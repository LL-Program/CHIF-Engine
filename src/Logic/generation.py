...
def generate_terrain(size, scale=50.0, octaves=6, persistence=0.5, lacunarity=2.0):
    ...
    for z in range(size):
        for x in range(size):
            ...
            heightmap[z, x] = height * 0.5 + 0.5  # Normalize to 0-1 range, but with less extreme values

    return heightmap

# Keep any existing functions in generation.py that you still need
# For example, if you still need the cursive and perm functions:
