...
#include "chifShaderCompiler_XBOX.h"
...
#include "chifShaderCompiler_PS5.h"
...
namespace chif::shadercompiler
{
		...
		InternalState_DXC(const std::string& modifier = "")
		{
			...
			if (dxcompiler != nullptr)
			{
				...
				if (DxcCreateInstance != nullptr)
				{
					...
					assert(SUCCEEDED(hr));
					chif::backlog::post("chif::shadercompiler: loaded " + library + " (version: " + std::to_string(major) + "." + std::to_string(minor) + ")");
				}
			}
			else
			{
				chif::backlog::post("chif::shadercompiler: could not load library " + library, chif::backlog::LogLevel::Error);
#ifdef PLATFORM_LINUX
				chif::backlog::post(dlerror(), chif::backlog::LogLevel::Error); // print dlopen() error detail: https://linux.die.net/man/3/dlerror
#endif // PLATFORM_LINUX
			}
	...
	void Compile_DXCompiler(const CompilerInput& input, CompilerOutput& output)
	{
		...
		chif::vector<uint8_t> shadersourcedata;
		if (!chif::helper::FileRead(input.shadersourcefilename, shadersourcedata))
		{
			return;
		}
		...
		{
			args.push_back(L"-D");
			chif::helper::StringConvert(x, args.emplace_back());
		}
		...
		if (input.format == ShaderFormat::HLSL6_XS)
		{
			chif::shadercompiler::xbox::AddArguments(input, args);
		}
#endif // SHADERCOMPILER_XBOX_INCLUDED

		// Entry point parameter:
		std::wstring wentry;
		chif::helper::StringConvert(input.entrypoint, wentry);
		args.push_back(L"-E");
		args.push_back(wentry.c_str());

		// Add source file name as last parameter. This will be displayed in error messages
		std::wstring wsource;
		chif::helper::StringConvert(chif::helper::GetFileNameFromPath(input.shadersourcefilename), wsource);
		...
		struct IncludeHandler : public IDxcIncludeHandler
		{
			...
			) override
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					chif::helper::StringConvert(pFilename, filename);
				}
				return hr;
			}
		...
		chif::vector<const wchar_t*> args_raw;
	...
	struct InternalState_D3DCompiler
	{
		...
		InternalState_D3DCompiler()
		{
			...
			if (d3dcompiler != nullptr)
			{
				...
				if (D3DCompile != nullptr)
				{
					chif::backlog::post("chif::shadercompiler: loaded d3dcompiler_47.dll");
				}
			}
		}
	};
	...
	void Compile_D3DCompiler(const CompilerInput& input, CompilerOutput& output)
	{
		...
		struct IncludeHandler : public ID3DInclude
		{
			const CompilerInput* input = nullptr;
			CompilerOutput* output = nullptr;
			chif::vector<chif::vector<uint8_t>> filedatas;
			...
			HRESULT Open(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID* ppData, UINT* pBytes) override
			{
				for (auto& x : input->include_directories)
				{
					...
					if (!chif::helper::FileExists(filename))
						continue;
					chif::vector<uint8_t>& filedata = filedatas.emplace_back();
					if (chif::helper::FileRead(filename, filedata))
					{
	...
	void Compile(const CompilerInput& input, CompilerOutput& output)
	{
		...
		case ShaderFormat::PS5:
			chif::shadercompiler::ps5::Compile(input, output);
			break;
	...
	{
#ifdef SHADERCOMPILER_ENABLED
		chif::helper::DirectoryCreate(chif::helper::GetDirectoryFromPath(shaderfilename));

		chif::Archive dependencyLibrary(chif::helper::ReplaceExtension(shaderfilename, shadermetaextension), false);
		if (dependencyLibrary.IsOpen())
		{
			std::string rootdir = dependencyLibrary.GetSourceDirectory();
			chif::vector<std::string> dependencies = output.dependencies;
			for (auto& x : dependencies)
			{
				chif::helper::MakePathRelative(rootdir, x);
			}
			dependencyLibrary << dependencies;
		}

		if (chif::helper::FileWrite(shaderfilename, output.shaderdata, output.shadersize))
		{
			return true;
		}
#endif // SHADERCOMPILER_ENABLED
	...
	bool IsShaderOutdated(const std::string& shaderfilename)
	{
		...
		chif::helper::MakePathAbsolute(filepath);
		if (!chif::helper::FileExists(filepath))
		{
			return true; // no shader file = outdated shader, apps can attempt to rebuild it
		}
		std::string dependencylibrarypath = chif::helper::ReplaceExtension(shaderfilename, shadermetaextension);
		if (!chif::helper::FileExists(dependencylibrarypath))
		{
			return false; // no metadata file = no dependency, up to date (for example packaged builds)
		}

		const uint64_t tim = chif::helper::FileTimestamp(filepath);

		chif::Archive dependencyLibrary(dependencylibrarypath);
		if (dependencyLibrary.IsOpen())
		{
			std::string rootdir = dependencyLibrary.GetSourceDirectory();
			...
			for (auto& x : dependencies)
			{
				...
				chif::helper::MakePathAbsolute(dependencypath);
				if (chif::helper::FileExists(dependencypath))
				{
					const uint64_t dep_tim = chif::helper::FileTimestamp(dependencypath);
	...
	std::mutex locker;
	chif::unordered_set<std::string> registered_shaders;
