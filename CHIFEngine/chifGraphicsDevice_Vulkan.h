...
#define CHIFENGINE_BUILD_VULKAN
...
namespace chif::graphics
{
	...
	protected:
		...
		chif::vector<VkQueueFamilyProperties2> queueFamilies;
		chif::vector<VkQueueFamilyVideoPropertiesKHR> queueFamiliesVideo;
		uint32_t graphicsFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t computeFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t copyFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t videoFamily = VK_QUEUE_FAMILY_IGNORED;
		chif::vector<uint32_t> families;
		...
		chif::vector<VkDynamicState> pso_dynamicStates;
		...
		struct CommandQueue
		{
			...
			chif::vector<SwapChain> swapchain_updates;
			chif::vector<VkSwapchainKHR> submit_swapchains;
			chif::vector<uint32_t> submit_swapChainImageIndices;
			chif::vector<VkSemaphoreSubmitInfo> submit_waitSemaphoreInfos;
			chif::vector<VkSemaphore> submit_signalSemaphores;
			chif::vector<VkSemaphoreSubmitInfo> submit_signalSemaphoreInfos;
			chif::vector<VkCommandBufferSubmitInfo> submit_cmds;
		...
		struct DescriptorBinder
		{
			...
			chif::vector<VkWriteDescriptorSet> descriptorWrites;
			chif::vector<VkDescriptorBufferInfo> bufferInfos;
			chif::vector<VkDescriptorImageInfo> imageInfos;
			chif::vector<VkBufferView> texelBufferViews;
			chif::vector<VkWriteDescriptorSetAccelerationStructureKHR> accelerationStructureViews;
		...
		chif::vector<VkSemaphore> semaphore_pool;
		...
		struct CommandList_Vulkan
		{
			...
			QUEUE_TYPE queue = {};
			uint32_t id = 0;
			chif::vector<std::pair<QUEUE_TYPE, VkSemaphore>> wait_queues;
			chif::vector<VkSemaphore> waits;
			chif::vector<VkSemaphore> signals;

			DescriptorBinder binder;
			DescriptorBinderPool binder_pools[BUFFERCOUNT];
			GPULinearAllocator frame_allocators[BUFFERCOUNT];

			chif::vector<std::pair<size_t, VkPipeline>> pipelines_worker;
			...
			chif::vector<SwapChain> prev_swapchains;
			bool dirty_pso = {};
			chif::vector<VkMemoryBarrier2> frame_memoryBarriers;
			chif::vector<VkImageMemoryBarrier2> frame_imageBarriers;
			chif::vector<VkBufferMemoryBarrier2> frame_bufferBarriers;
			chif::vector<VkAccelerationStructureGeometryKHR> accelerationstructure_build_geometries;
			chif::vector<VkAccelerationStructureBuildRangeInfoKHR> accelerationstructure_build_ranges;
			RenderPassInfo renderpass_info;
			chif::vector<VkImageMemoryBarrier2> renderpass_barriers_begin;
			chif::vector<VkImageMemoryBarrier2> renderpass_barriers_end;
		...
		chif::vector<std::unique_ptr<CommandList_Vulkan>> commandlists;
		uint32_t cmd_count = 0;
		chif::SpinLock cmd_locker;
		...
		mutable chif::unordered_map<size_t, PSOLayout> pso_layout_cache;
		mutable std::mutex pso_layout_cache_mutex;

		VkPipelineCache pipelineCache = VK_NULL_HANDLE;
		chif::unordered_map<size_t, VkPipeline> pipelines_global;
		...
		chif::vector<VkSampler> immutable_samplers;

	public:
		GraphicsDevice_Vulkan(chif::platform::window_type window, ValidationMode validationMode = ValidationMode::Disabled, GPUPreference preference = GPUPreference::Discrete);
		~GraphicsDevice_Vulkan() override;

		bool CreateSwapChain(const SwapChainDesc* desc, chif::platform::window_type window, SwapChain* swapchain) const override;
