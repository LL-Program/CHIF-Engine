...
#include "chifSheenLUT.h"
#include "chifShaderCompiler.h"
#include "chifTimer.h"
#include "chifUnorderedMap.h" // leave it here for shader dump!
#include "chifFont.h"
...
std::string SHADERPATH = chif::helper::GetCurrentPath() + "/shaders/";
...
static thread_local chif::vector<GPUBarrier> barrier_stack;
...
chif::vector<std::pair<XMFLOAT4X4, XMFLOAT4>> renderableBoxes;
chif::vector<std::pair<XMFLOAT4X4, XMFLOAT4>> renderableBoxes_depth;
chif::vector<std::pair<Sphere, XMFLOAT4>> renderableSpheres;
chif::vector<std::pair<Sphere, XMFLOAT4>> renderableSpheres_depth;
chif::vector<std::pair<Capsule, XMFLOAT4>> renderableCapsules;
chif::vector<std::pair<Capsule, XMFLOAT4>> renderableCapsules_depth;
chif::vector<RenderableLine> renderableLines;
chif::vector<RenderableLine> renderableLines_depth;
chif::vector<RenderableLine2D> renderableLines2D;
chif::vector<RenderablePoint> renderablePoints;
chif::vector<RenderablePoint> renderablePoints_depth;
chif::vector<RenderableTriangle> renderableTriangles_solid;
chif::vector<RenderableTriangle> renderableTriangles_wireframe;
chif::vector<RenderableTriangle> renderableTriangles_solid_depth;
chif::vector<RenderableTriangle> renderableTriangles_wireframe_depth;
chif::vector<uint8_t> debugTextStorage; // A stream of DebugText struct + text characters
chif::vector<PaintRadius> paintrads;
chif::vector<PaintTextureParams> painttextures;
chif::vector<PaintDecalParams> paintdecals;
chif::vector<const chif::VoxelGrid*> renderableVoxelgrids;
chif::vector<const chif::PathQuery*> renderablePathqueries;
chif::vector<const chif::TrailRenderer*> renderableTrails;

chif::SpinLock deferredMIPGenLock;
chif::vector<std::pair<Texture, bool>> deferredMIPGens;
chif::vector<std::pair<Texture, Texture>> deferredBCQueue;
...
struct RenderQueue
{
	chif::vector<RenderBatch> batches;
...
union ObjectRenderingVariant
{
	struct
	{
		uint32_t renderpass : 4;	// chif::enums::RENDERPASS
		uint32_t shadertype : 8;	// MaterialComponent::SHADERTYPE
		uint32_t blendmode : 4;		// chif::enums::BLENDMODE
		uint32_t cullmode : 2;		// chif::graphics::CullMode
...
{
	static chif::unordered_map<uint32_t, PipelineState> PSO_object[RENDERPASS_COUNT][MaterialComponent::SHADERTYPE_COUNT][OBJECT_MESH_SHADER_PSO_COUNT];
	return &PSO_object[variant.bits.renderpass][variant.bits.shadertype][variant.bits.mesh_shader][variant.value];
}
chif::jobsystem::context mesh_shader_ctx;
chif::jobsystem::context object_pso_job_ctx[RENDERPASS_COUNT][OBJECT_MESH_SHADER_PSO_COUNT];
PipelineState PSO_object_wire;
PipelineState PSO_object_wire_tessellation;
PipelineState PSO_object_wire_mesh_shader;

chif::jobsystem::context raytracing_ctx;
chif::jobsystem::context objectps_ctx;

chif::vector<CustomShader> customShaders;
...
)
{
	...
	{
		std::string ext = chif::helper::GetExtensionFromFileName(shaderbinaryfilename);
		shaderbinaryfilename = chif::helper::RemoveExtension(shaderbinaryfilename);
	...
	if (device != nullptr)
	{
		...
		{
			chif::backlog::post("shader dump doesn't contain shader: " + shaderbinaryfilename, chif::backlog::LogLevel::Error);
		}
#endif // SHADERDUMP_ENABLED
	}

	chif::shadercompiler::RegisterShader(shaderbinaryfilename);

	if (chif::shadercompiler::IsShaderOutdated(shaderbinaryfilename))
	{
		...
		chif::helper::MakePathAbsolute(sourcedir);
		input.include_directories.push_back(sourcedir);
		input.include_directories.push_back(sourcedir + chif::helper::GetDirectoryFromPath(filename));
		input.shadersourcefilename = chif::helper::ReplaceExtension(sourcedir + filename, "hlsl");
		...
		if (output.IsValid())
		{
			...
			{
				chif::backlog::post(output.error_message, chif::backlog::LogLevel::Warning);
			}
			chif::backlog::post("shader compiled: " + shaderbinaryfilename);
			return device->CreateShader(stage, output.shaderdata, output.shadersize, &shader);
		}
		else
		{
			chif::backlog::post("shader compile FAILED: " + shaderbinaryfilename + "\n" + output.error_message, chif::backlog::LogLevel::Error);
	...
	if (device != nullptr)
	{
		chif::vector<uint8_t> buffer;
		if (chif::helper::FileRead(shaderbinaryfilename, buffer))
		{
...
void LoadShaders()
{
	chif::jobsystem::Wait(raytracing_ctx);
	raytracing_ctx.priority = chif::jobsystem::Priority::Low;

	chif::jobsystem::Wait(objectps_ctx);
	objectps_ctx.priority = chif::jobsystem::Priority::Low;
	...
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_COMMON_TESSELLATION], "objectVS_common_tessellation.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_PREPASS_TESSELLATION], "objectVS_prepass_tessellation.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_PREPASS_ALPHATEST_TESSELLATION], "objectVS_prepass_alphatest_tessellation.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_SIMPLE_TESSELLATION], "objectVS_simple_tessellation.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_IMPOSTOR], "impostorVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_DIRECTIONAL], "volumetriclight_directionalVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_POINT], "volumetriclight_pointVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_SPOT], "volumetriclight_spotVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LIGHTVISUALIZER_SPOTLIGHT], "vSpotLightVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LIGHTVISUALIZER_POINTLIGHT], "vPointLightVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SPHERE], "sphereVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OCCLUDEE], "occludeeVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SKY], "skyVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOXELIZER], "objectVS_voxelizer.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOXEL], "voxelVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_FORCEFIELDVISUALIZER_POINT], "forceFieldPointVisualizerVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_FORCEFIELDVISUALIZER_PLANE], "forceFieldPlaneVisualizerVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_RAYTRACE_SCREEN], "raytrace_screenVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_POSTPROCESS], "postprocessVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LENSFLARE], "lensFlareVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_DDGI_DEBUG], "ddgi_debugVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SCREEN], "screenVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_PAINTDECAL], "paintdecalVS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP], "envMapVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP_SKY], "envMap_skyVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW], "shadowVS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_ALPHATEST], "shadowVS_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_TRANSPARENT], "shadowVS_transparent.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR], "impostorPS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_HOLOGRAM], "objectPS_hologram.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_DEBUG], "objectPS_debug.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PAINTRADIUS], "objectPS_paintradius.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_SIMPLE], "objectPS_simple.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS], "objectPS_prepass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_ALPHATEST], "objectPS_prepass_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_DEPTHONLY], "objectPS_prepass_depthonly.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_DEPTHONLY_ALPHATEST], "objectPS_prepass_depthonly_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_PREPASS], "impostorPS_prepass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_PREPASS_DEPTHONLY], "impostorPS_prepass_depthonly.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_SIMPLE], "impostorPS_simple.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_LIGHTVISUALIZER], "lightVisualizerPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_DIRECTIONAL], "volumetricLight_DirectionalPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_POINT], "volumetricLight_PointPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_SPOT], "volumetricLight_SpotPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP], "envMapPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP_SKY_STATIC], "envMap_skyPS_static.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP_SKY_DYNAMIC], "envMap_skyPS_dynamic.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_CAPTUREIMPOSTOR], "captureImpostorPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_CUBEMAP], "cubeMapPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VERTEXCOLOR], "vertexcolorPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SKY_STATIC], "skyPS_static.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SKY_DYNAMIC], "skyPS_dynamic.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SUN], "sunPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_ALPHATEST], "shadowPS_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_TRANSPARENT], "shadowPS_transparent.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_WATER], "shadowPS_water.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOXELIZER], "objectPS_voxelizer.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOXEL], "voxelPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_FORCEFIELDVISUALIZER], "forceFieldVisualizerPS.cso"); });
	
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_RAYTRACE_DEBUGBVH], "raytrace_debugbvhPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_DOWNSAMPLEDEPTHBUFFER], "downsampleDepthBuffer4xPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL], "upsample_bilateralPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_OUTLINE], "outlinePS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_LENSFLARE], "lensFlarePS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_DDGI_DEBUG], "ddgi_debugPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_UPSAMPLE], "volumetricCloud_upsamplePS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_COPY_DEPTH], "copyDepthPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_COPY_STENCIL_BIT], "copyStencilBitPS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_PAINTDECAL], "paintdecalPS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_VOXELIZER], "objectGS_voxelizer.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_VOXEL], "voxelGS.cso"); });

#ifdef PLATFORM_PS5
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_OBJECT_PRIMITIVEID_EMULATION], "objectGS_primitiveID_emulation.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_OBJECT_PRIMITIVEID_EMULATION_ALPHATEST], "objectGS_primitiveID_emulation_alphatest.cso"); });
#endif // PLATFORM_PS5

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LUMINANCE_PASS1], "luminancePass1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LUMINANCE_PASS2], "luminancePass2CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SHADINGRATECLASSIFICATION], "shadingRateClassificationCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SHADINGRATECLASSIFICATION_DEBUG], "shadingRateClassificationCS_DEBUG.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING], "lightCullingCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_DEBUG], "lightCullingCS_DEBUG.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_ADVANCED], "lightCullingCS_ADVANCED.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_ADVANCED_DEBUG], "lightCullingCS_ADVANCED_DEBUG.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_RESOLVEMSAADEPTHSTENCIL], "resolveMSAADepthStencilCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_OFFSETPREV], "vxgi_offsetprevCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_TEMPORAL], "vxgi_temporalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_SDF_JUMPFLOOD], "vxgi_sdf_jumpfloodCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_RESOLVE_DIFFUSE], "vxgi_resolve_diffuseCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_RESOLVE_SPECULAR], "vxgi_resolve_specularCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_TRANSMITTANCELUT], "skyAtmosphere_transmittanceLutCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_MULTISCATTEREDLUMINANCELUT], "skyAtmosphere_multiScatteredLuminanceLutCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_SKYVIEWLUT], "skyAtmosphere_skyViewLutCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_SKYLUMINANCELUT], "skyAtmosphere_skyLuminanceLutCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_CAMERAVOLUMELUT], "skyAtmosphere_cameraVolumeLutCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN2D_UNORM4], "generateMIPChain2DCS_unorm4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN2D_FLOAT4], "generateMIPChain2DCS_float4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN3D_UNORM4], "generateMIPChain3DCS_unorm4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN3D_FLOAT4], "generateMIPChain3DCS_float4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBE_UNORM4], "generateMIPChainCubeCS_unorm4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBE_FLOAT4], "generateMIPChainCubeCS_float4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBEARRAY_UNORM4], "generateMIPChainCubeArrayCS_unorm4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBEARRAY_FLOAT4], "generateMIPChainCubeArrayCS_float4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC1], "blockcompressCS_BC1.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC3], "blockcompressCS_BC3.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC4], "blockcompressCS_BC4.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC5], "blockcompressCS_BC5.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC6H], "blockcompressCS_BC6H.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC6H_CUBEMAP], "blockcompressCS_BC6H_cubemap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_FILTERENVMAP], "filterEnvMapCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_UNORM4], "copytexture2D_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_FLOAT4], "copytexture2D_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_UNORM4_BORDEREXPAND], "copytexture2D_unorm4_borderexpandCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_FLOAT4_BORDEREXPAND], "copytexture2D_float4_borderexpandCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKINNING], "skinningCS.cso"); });
	
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_PAINT_TEXTURE], "paint_textureCS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT1], "blur_gaussian_float1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT3], "blur_gaussian_float3CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT4], "blur_gaussian_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_UNORM1], "blur_gaussian_unorm1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_UNORM4], "blur_gaussian_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT1], "blur_gaussian_wide_float1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT3], "blur_gaussian_wide_float3CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT4], "blur_gaussian_wide_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_UNORM1], "blur_gaussian_wide_unorm1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_UNORM4], "blur_gaussian_wide_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT1], "blur_bilateral_float1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT3], "blur_bilateral_float3CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT4], "blur_bilateral_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_UNORM1], "blur_bilateral_unorm1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_UNORM4], "blur_bilateral_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT1], "blur_bilateral_wide_float1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT3], "blur_bilateral_wide_float3CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT4], "blur_bilateral_wide_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_UNORM1], "blur_bilateral_wide_unorm1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_UNORM4], "blur_bilateral_wide_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSAO], "ssaoCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_HBAO], "hbaoCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_PREPAREDEPTHBUFFERS1], "msao_preparedepthbuffers1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_PREPAREDEPTHBUFFERS2], "msao_preparedepthbuffers2CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_INTERLEAVE], "msao_interleaveCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO], "msaoCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE], "msao_blurupsampleCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_BLENDOUT], "msao_blurupsampleCS_blendout.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_PREMIN], "msao_blurupsampleCS_premin.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_PREMIN_BLENDOUT], "msao_blurupsampleCS_premin_blendout.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TILEMAXROUGHNESS_HORIZONTAL], "ssr_tileMaxRoughness_horizontalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TILEMAXROUGHNESS_VERTICAL], "ssr_tileMaxRoughness_verticalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_DEPTHHIERARCHY], "ssr_depthHierarchyCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE], "ssr_raytraceCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE_EARLYEXIT], "ssr_raytraceCS_earlyexit.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE_CHEAP], "ssr_raytraceCS_cheap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RESOLVE], "ssr_resolveCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TEMPORAL], "ssr_temporalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_UPSAMPLE], "ssr_upsampleCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_LIGHTSHAFTS], "lightShaftsCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_TILEMAXCOC_HORIZONTAL], "depthoffield_tileMaxCOC_horizontalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_TILEMAXCOC_VERTICAL], "depthoffield_tileMaxCOC_verticalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_NEIGHBORHOODMAXCOC], "depthoffield_neighborhoodMaxCOCCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_PREPASS], "depthoffield_prepassCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_PREPASS_EARLYEXIT], "depthoffield_prepassCS_earlyexit.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN], "depthoffield_mainCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN_EARLYEXIT], "depthoffield_mainCS_earlyexit.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN_CHEAP], "depthoffield_mainCS_cheap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_POSTFILTER], "depthoffield_postfilterCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_UPSAMPLE], "depthoffield_upsampleCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_TILEMAXVELOCITY_HORIZONTAL], "motionblur_tileMaxVelocity_horizontalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_TILEMAXVELOCITY_VERTICAL], "motionblur_tileMaxVelocity_verticalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_NEIGHBORHOODMAXVELOCITY], "motionblur_neighborhoodMaxVelocityCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR], "motionblurCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_EARLYEXIT], "motionblurCS_earlyexit.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_CHEAP], "motionblurCS_cheap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLOOMSEPARATE], "bloomseparateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE], "aerialPerspectiveCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE_CAPTURE], "aerialPerspectiveCS_capture.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE_CAPTURE_MSAA], "aerialPerspectiveCS_capture_MSAA.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHAPENOISE], "volumetricCloud_shapenoiseCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_DETAILNOISE], "volumetricCloud_detailnoiseCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_CURLNOISE], "volumetricCloud_curlnoiseCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_WEATHERMAP], "volumetricCloud_weathermapCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER], "volumetricCloud_renderCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER_CAPTURE], "volumetricCloud_renderCS_capture.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER_CAPTURE_MSAA], "volumetricCloud_renderCS_capture_MSAA.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_REPROJECT], "volumetricCloud_reprojectCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHADOW_RENDER], "volumetricCloud_shadow_renderCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHADOW_FILTER], "volumetricCloud_shadow_filterCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FXAA], "fxaaCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_TEMPORALAA], "temporalaaCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SHARPEN], "sharpenCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_TONEMAP], "tonemapCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UNDERWATER], "underwaterCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR_UPSCALING], "fsr_upscalingCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR_SHARPEN], "fsr_sharpenCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_AUTOGEN_REACTIVE_PASS], "ffx-fsr2/ffx_fsr2_autogen_reactive_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_COMPUTE_LUMINANCE_PYRAMID_PASS], "ffx-fsr2/ffx_fsr2_compute_luminance_pyramid_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_PREPARE_INPUT_COLOR_PASS], "ffx-fsr2/ffx_fsr2_prepare_input_color_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_RECONSTRUCT_PREVIOUS_DEPTH_PASS], "ffx-fsr2/ffx_fsr2_reconstruct_previous_depth_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_DEPTH_CLIP_PASS], "ffx-fsr2/ffx_fsr2_depth_clip_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_LOCK_PASS], "ffx-fsr2/ffx_fsr2_lock_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_ACCUMULATE_PASS], "ffx-fsr2/ffx_fsr2_accumulate_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_RCAS_PASS], "ffx-fsr2/ffx_fsr2_rcas_pass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_CHROMATIC_ABERRATION], "chromatic_aberrationCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_FLOAT1], "upsample_bilateral_float1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_UNORM1], "upsample_bilateral_unorm1CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_FLOAT4], "upsample_bilateral_float4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_UNORM4], "upsample_bilateral_unorm4CS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DOWNSAMPLE4X], "downsample4xCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_LINEARDEPTH], "lineardepthCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_NORMALSFROMDEPTH], "normalsfromdepthCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SCREENSPACESHADOW], "screenspaceshadowCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSGI_DEINTERLEAVE], "ssgi_deinterleaveCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSGI], "ssgiCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSGI_WIDE], "ssgiCS.cso", chif::graphics::ShaderModel::SM_5_0, { "WIDE" }); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSGI_UPSAMPLE], "ssgi_upsampleCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSGI_UPSAMPLE_WIDE], "ssgi_upsampleCS.cso", chif::graphics::ShaderModel::SM_5_0, { "WIDE" }); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_SPATIAL], "rtdiffuse_spatialCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_TEMPORAL], "rtdiffuse_temporalCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_UPSAMPLE], "rtdiffuse_upsampleCS.cso"); });

	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE], "rtdiffuseCS.cso", ShaderModel::SM_6_5); });

		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTREFLECTION], "rtreflectionCS.cso", ShaderModel::SM_6_5); });

		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW], "rtshadowCS.cso", ShaderModel::SM_6_5); });
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_TILECLASSIFICATION], "rtshadow_denoise_tileclassificationCS.cso"); });
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_FILTER], "rtshadow_denoise_filterCS.cso"); });
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_TEMPORAL], "rtshadow_denoise_temporalCS.cso"); });

		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO], "rtaoCS.cso", ShaderModel::SM_6_5); });
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO_DENOISE_TILECLASSIFICATION], "rtao_denoise_tileclassificationCS.cso"); });
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO_DENOISE_FILTER], "rtao_denoise_filterCS.cso"); });

	}

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_UPSAMPLE], "rtshadow_upsampleCS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_COVERAGE], "surfel_coverageCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_INDIRECTPREPARE], "surfel_indirectprepareCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_UPDATE], "surfel_updateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_GRIDOFFSETS], "surfel_gridoffsetsCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_BINNING], "surfel_binningCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_INTEGRATE], "surfel_integrateCS.cso"); });
	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS.cso"); });
	}

	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_RAYTRACE], "raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_RAYTRACE], "raytraceCS.cso"); });
	}

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_RESOLVE], "visibility_resolveCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_RESOLVE_MSAA], "visibility_resolveCS_MSAA.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_SKY], "visibility_skyCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_VELOCITY], "visibility_velocityCS.cso"); });

	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS.cso"); });
	}
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYALLOCATION], "ddgi_rayallocationCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_INDIRECTPREPARE], "ddgi_indirectprepareCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_UPDATE], "ddgi_updateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_UPDATE_DEPTH], "ddgi_updateCS_depth.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_BASECOLORMAP], "terrainVirtualTextureUpdateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_NORMALMAP], "terrainVirtualTextureUpdateCS_normalmap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_SURFACEMAP], "terrainVirtualTextureUpdateCS_surfacemap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_EMISSIVEMAP], "terrainVirtualTextureUpdateCS_emissivemap.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_MESHLET_PREPARE], "meshlet_prepareCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_IMPOSTOR_PREPARE], "impostor_prepareCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_TILEREQUESTS], "virtualTextureTileRequestsCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_TILEALLOCATE], "virtualTextureTileAllocateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_RESIDENCYUPDATE], "virtualTextureResidencyUpdateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_WIND], "windCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_YUV_TO_RGB], "yuv_to_rgbCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_WETMAP_UPDATE], "wetmap_updateCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_CAUSTICS], "causticsCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DEPTH_REPROJECT], "depth_reprojectCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DEPTH_PYRAMID], "depth_pyramidCS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTMAP_EXPAND], "lightmap_expandCS.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT], "objectHS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_PREPASS], "objectHS_prepass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_PREPASS_ALPHATEST], "objectHS_prepass_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_SIMPLE], "objectHS_simple.cso"); });

	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT], "objectDS.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_PREPASS], "objectDS_prepass.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_PREPASS_ALPHATEST], "objectDS_prepass_alphatest.cso"); });
	chif::jobsystem::Execute(ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_SIMPLE], "objectDS_simple.cso"); });

	chif::jobsystem::Dispatch(objectps_ctx, MaterialComponent::SHADERTYPE_COUNT, 1, [](chif::jobsystem::JobArgs args) {
	...
	if (device->CheckCapability(GraphicsDeviceCapability::MESH_SHADER))
	{
		...
		chif::jobsystem::Wait(mesh_shader_ctx);
		mesh_shader_ctx.priority = chif::jobsystem::Priority::Low;

		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) {
		...
		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_OBJECT], "objectMS.cso"); });
		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_OBJECT_PREPASS], "objectMS_prepass.cso"); });
		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_OBJECT_PREPASS_ALPHATEST], "objectMS_prepass_alphatest.cso"); });

		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_SHADOW], "shadowMS.cso"); });
		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_SHADOW_ALPHATEST], "shadowMS_alphatest.cso"); });
		chif::jobsystem::Execute(mesh_shader_ctx, [](chif::jobsystem::JobArgs args) { LoadShader(ShaderStage::MS, shaders[MSTYPE_SHADOW_TRANSPARENT], "shadowMS_transparent.cso"); });
	}

	chif::jobsystem::Dispatch(ctx, MaterialComponent::SHADERTYPE_COUNT, 1, [](chif::jobsystem::JobArgs args) {
	...
	chif::jobsystem::Dispatch(ctx, RENDERPASS_COUNT, 1, [](chif::jobsystem::JobArgs args) {
	...
	chif::jobsystem::Dispatch(ctx, LightComponent::LIGHTTYPE_COUNT, 1, [](chif::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	chif::jobsystem::Execute(raytracing_ctx, [](chif::jobsystem::JobArgs args) {
	...
	chif::jobsystem::Dispatch(ctx, SKYRENDERING_COUNT, 1, [](chif::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	chif::jobsystem::Dispatch(ctx, DEBUGRENDERING_COUNT, 1, [](chif::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	for (uint32_t renderPass = 0; renderPass < RENDERPASS_COUNT; ++renderPass)
	{
		for (uint32_t mesh_shader = 0; mesh_shader <= (device->CheckCapability(GraphicsDeviceCapability::MESH_SHADER) ? 1u : 0u); ++mesh_shader)
		{
			...
			chif::jobsystem::Wait(object_pso_job_ctx[renderPass][mesh_shader]);
			object_pso_job_ctx[renderPass][mesh_shader].priority = chif::jobsystem::Priority::Low;
			for (uint32_t shaderType = 0; shaderType < MaterialComponent::SHADERTYPE_COUNT; ++shaderType)
			{
				chif::jobsystem::Execute(object_pso_job_ctx[renderPass][mesh_shader], [=](chif::jobsystem::JobArgs args) {
...
{
	if (chif::jobsystem::IsBusy(raytracing_ctx))
		return true;
	if (chif::jobsystem::IsBusy(objectps_ctx))
		return true;
	if (chif::jobsystem::IsBusy(mesh_shader_ctx))
		return true;
	for (uint32_t renderPass = 0; renderPass < RENDERPASS_COUNT; ++renderPass)
	{
		for (uint32_t mesh_shader = 0; mesh_shader <= (IsMeshShaderAllowed() ? 1u : 0u); ++mesh_shader)
		{
			if (chif::jobsystem::IsBusy(object_pso_job_ctx[renderPass][mesh_shader]))
			{
				return true;
			}
		}
...
const GPUBuffer& GetIndexBufferForQuads(uint32_t max_quad_count)
{
	...
	if (required_max_index < 65536u)
	{
		...
		if (!indexBufferForQuads16.IsValid() || indexBufferForQuads16.desc.size / indexBufferForQuads16.desc.stride < required_index_count)
		{
			...
			device->SetName(&indexBufferForQuads16, "chif::renderer::indexBufferForQuads16bit");
		}
		return indexBufferForQuads16;
	}

	// 32-bit request below:
	max_quad_count = chif::math::GetNextPowerOfTwo(max_quad_count); // reduce allocations by making larger fitting allocations
	const size_t required_index_count = max_quad_count * 6u;
	...
	if (!indexBufferForQuads32.IsValid() || indexBufferForQuads32.desc.size / indexBufferForQuads32.desc.stride < required_index_count)
	{
		...
		device->SetName(&indexBufferForQuads32, "chif::renderer::indexBufferForQuads32bit");
	}
...
void ReloadShaders()
{
	...
	chif::eventhandler::FireEvent(chif::eventhandler::EVENT_RELOAD_SHADERS, 0);
}
...
void Initialize()
{
	...
	static chif::eventhandler::Handle handle2 = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
	LoadShaders();

	chif::backlog::post("chif::renderer Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
...
inline void CreateDirLightShadowCams(const LightComponent& light, CameraComponent camera, SHCAM* shcams, size_t shcam_count, const chif::rectpacker::Rect& shadow_rect)
{
...
void Workaround(const int bug , CommandList cmd)
{
	if (bug == 1)
	{
		...
		chif::jobsystem::Wait(object_pso_job_ctx[RENDERPASS_VOXELIZE][OBJECT_MESH_SHADER_PSO_DISABLED]);
		ObjectRenderingVariant variant = {};
...
)
{
	...
	chif::jobsystem::Wait(object_pso_job_ctx[renderPass][OBJECT_MESH_SHADER_PSO_DISABLED]);
	...
	if (mesh_shader)
	{
		...
		chif::jobsystem::Wait(object_pso_job_ctx[renderPass][OBJECT_MESH_SHADER_PSO_ENABLED]);
	}
...
void UpdateVisibility(Visibility& vis)
{
	...
	chif::jobsystem::context ctx;
	auto range = chif::profiler::BeginRangeCPU("Frustum Culling");
	...
	if (vis.flags & Visibility::ALLOW_LIGHTS)
	{
		...
		chif::jobsystem::Dispatch(ctx, light_loop, groupSize, [&](chif::jobsystem::JobArgs args) {
	...
	if (vis.flags & Visibility::ALLOW_OBJECTS)
	{
		...
		chif::jobsystem::Dispatch(ctx, object_loop, groupSize, [&](chif::jobsystem::JobArgs args) {
			...
			if ((aabb.layerMask & vis.layerMask) && vis.frustum.CheckBoxFast(aabb))
			{
				...
				if ((vis.flags & Visibility::ALLOW_REQUEST_REFLECTION) && object.IsRequestPlanarReflection() && !occluded)
				{
					...
					float dist = chif::math::DistanceEstimated(vis.camera->Eye, object.center);
					vis.locker.lock();
	...
	if (vis.flags & Visibility::ALLOW_EMITTERS)
	{
		chif::jobsystem::Execute(ctx, [&](chif::jobsystem::JobArgs args) {
			...
			{
				const chif::EmittedParticleSystem& emitter = vis.scene->emitters[i];
	...
	if (vis.flags & Visibility::ALLOW_HAIRS)
	{
		chif::jobsystem::Execute(ctx, [&](chif::jobsystem::JobArgs args) {
			...
			{
				const chif::HairParticleSystem& hair = vis.scene->hairs[i];
				...
				{
					const float dist = chif::math::Distance(vis.camera->Eye, hair.aabb.getCenter());
	...
	{
		auto range = chif::profiler::BeginRangeCPU("Shadowmap packing");
		...
		while (iterative_scaling > 0.03f)
		{
			...
			for (uint32_t lightIndex : vis.visibleLights)
			{
				...
				const float dist = chif::math::Distance(vis.camera->Eye, light.position);
			...
			if (!vis.shadow_packer.rects.empty())
			{
				if (vis.shadow_packer.pack(8192))
				{
					for (auto& rect : vis.shadow_packer.rects)
					{
						...
						chif::rectpacker::Rect& lightrect = vis.visibleLightShadowRects[lightIndex];
						const LightComponent& light = vis.scene->lights[lightIndex];
						if (rect.was_packed)
						{
							lightrect = rect;
	...
	chif::profiler::EndRange(range); // Frustum Culling
}
...
)
{
	...
	frameCB.giboost_packed = chif::math::pack_half2(GetGIBoost(), 0);
	...
	frameCB.texture_random64x64_index = device->GetDescriptorIndex(chif::texturehelper::getRandom64x64(), SubresourceType::SRV);
	frameCB.texture_bluenoise_index = device->GetDescriptorIndex(chif::texturehelper::getBlueNoise(), SubresourceType::SRV);
	...
	frameCB.entity_culling_count = 0;
	{
		...
		for (uint32_t lightIndex : vis.visibleLights)
		{
			...
			const chif::rectpacker::Rect& shadow_rect = vis.visibleLightShadowRects[lightIndex];
...
)
{
	...
	auto prof_updatebuffer_cpu = chif::profiler::BeginRangeCPU("Update Buffers (CPU)");
	auto prof_updatebuffer_gpu = chif::profiler::BeginRangeGPU("Update Buffers (GPU)", cmd);
	...
	chif::profiler::EndRange(prof_updatebuffer_cpu);
	chif::profiler::EndRange(prof_updatebuffer_gpu);
	...
	{
		auto range = chif::profiler::BeginRangeGPU("Wind", cmd);
	...
	{
		auto range = chif::profiler::BeginRangeGPU("Caustics", cmd);
...

	{
		...
		auto range = chif::profiler::BeginRangeGPU("Skinning and Morph", cmd);
		...
		chif::profiler::EndRange(range); // Skinning and Morph
	...
	static thread_local chif::vector<HairParticleSystem::UpdateGPUItem> hair_updates;
	if (!vis.visibleHairs.empty())
	{
		auto range = chif::profiler::BeginRangeGPU("HairParticles - Simulate", cmd);
		for (uint32_t hairIndex : vis.visibleHairs)
		{
			const chif::HairParticleSystem& hair = vis.scene->hairs[hairIndex];
	...
	if (vis.scene->impostors.GetCount() > 0 && vis.scene->objects.GetCount() > 0 && vis.scene->impostorBuffer.IsValid())
	{
		...
		auto range = chif::profiler::BeginRangeGPU("Impostor prepare", cmd);
	...
	if(vis.scene->instanceArraySize > 0 && vis.scene->meshletBuffer.IsValid())
	{
		...
		auto range = chif::profiler::BeginRangeGPU("Meshlet prepare", cmd);
...
)
{
	...
	{
		auto range = chif::profiler::BeginRangeGPU("EmittedParticles - Simulate", cmd);
		for (uint32_t emitterIndex : vis.visibleEmitters)
		{
			const chif::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
...
)
{
	if (!vis.scene->weather.IsOceanEnabled() || !vis.scene->ocean.IsValid())
		return;
	bool occluded = false;
	if (vis.flags & chif::renderer::Visibility::ALLOW_OCCLUSION_CULLING)
	{
		occluded = vis.scene->ocean.IsOccluded();
	}
	if (!occluded)
	{
		auto range = chif::profiler::BeginRangeGPU("Ocean - Simulate", cmd);
...
void UpdateRaytracingAccelerationStructures(const Scene& scene, CommandList cmd)
{
	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		...
		// BLAS:
		{
			auto rangeCPU = chif::profiler::BeginRangeCPU("BLAS Update (CPU)");
			auto range = chif::profiler::BeginRangeGPU("BLAS Update (GPU)", cmd);
			...
			{
				const chif::HairParticleSystem& hair = scene.hairs[i];
			...
			{
				const chif::EmittedParticleSystem& emitter = scene.emitters[i];
			...
			device->EventEnd(cmd);
			chif::profiler::EndRange(range);
			chif::profiler::EndRange(rangeCPU);
		}

		// TLAS:
		{
			auto rangeCPU = chif::profiler::BeginRangeCPU("TLAS Update (CPU)");
			auto range = chif::profiler::BeginRangeGPU("TLAS Update (GPU)", cmd);
...
void OcclusionCulling_Render(const CameraComponent& camera, const Visibility& vis, CommandList cmd)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Occlusion Culling Render", cmd);
	...
	chif::profiler::EndRange(range); // Occlusion Culling Render
}
...
)
{
	auto range = distortion ?
		chif::profiler::BeginRangeGPU("EmittedParticles - Render (Distortion)", cmd) :
		chif::profiler::BeginRangeGPU("EmittedParticles - Render", cmd);
	...
	if (emitterCount > 0)
	{
		...
		static thread_local chif::vector<uint32_t> emitterSortingHashes;
		emitterSortingHashes.resize(emitterCount);
		for (size_t i = 0; i < emitterCount; ++i)
		{
			const uint32_t emitterIndex = vis.visibleEmitters[i];
			const chif::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
			float distance = chif::math::DistanceEstimated(emitter.center, vis.camera->Eye);
		...
		for (size_t i = 0; i < emitterCount; ++i)
		{
			...
			if (distortion && emitter.shaderType == chif::EmittedParticleSystem::SOFT_DISTORTION)
			{
				emitter.Draw(material, cmd);
			}
			else if (!distortion && (emitter.shaderType == chif::EmittedParticleSystem::SOFT || emitter.shaderType == chif::EmittedParticleSystem::SOFT_LIGHTING || emitter.shaderType == chif::EmittedParticleSystem::SIMPLE || IsWireRender()))
			{
				emitter.Draw(material, cmd);
			}
		}
...
)
{
	...
	static thread_local chif::vector<uint64_t> distance_sorter;
	distance_sorter.clear();
	for (size_t i = 0; i < scene.sprites.GetCount(); ++i)
	{
		const chif::Sprite& sprite = scene.sprites[i];
		if (sprite.IsHidden())
			continue;
		...
		{
			sorter.bits.distance = XMConvertFloatToHalf(chif::math::DistanceEstimated(transform->GetPosition(), camera.Eye));
		}
	...
	if (!distortion)
	{
		...
		{
			const chif::SpriteFont& font = scene.fonts[i];
			if (font.IsHidden())
				continue;
			DistanceSorter sorter = {};
			sorter.bits.id = uint32_t(i);
	...
	for (auto& x : distance_sorter)
	{
		...
		case SPRITE:
		{
			const chif::Sprite& sprite = scene.sprites[sorter.bits.id];
			chif::image::Params params = sprite.params;
			...
			if (transform != nullptr)
			{
				...
				{
					float scale = 0.05f * chif::math::Distance(transform->GetPosition(), camera.Eye);
			...
			chif::image::Draw(sprite.GetTexture(), params, cmd);
		}
		break;
		case FONT:
		{
			const chif::SpriteFont& font = scene.fonts[sorter.bits.id];
			chif::font::Params params = font.params;
			...
			params.customProjection = &M;
			chif::font::Draw(font.GetText().c_str(), font.GetCurrentTextLength(), params, cmd);
		}
		break;
		}
	}
		...
		for (int type = LightComponent::POINT; type < LightComponent::LIGHTTYPE_COUNT; ++type)
		{
			...
			for (uint32_t lightIndex : vis.visibleLights)
			{
				...
				if (light.GetType() == type && light.IsVisualizerEnabled())
				{
					...
					{
						const float sphere_volume = std::max(1.0f, chif::math::SphereVolume(light.radius));
	...
	for (int type = 0; type < LightComponent::LIGHTTYPE_COUNT; ++type)
	{
		...
		for (size_t i = 0; i < vis.visibleLights.size(); ++i)
		{
			...
			case LightComponent::SPOT:
			{
				...
				uint packed = chif::math::pack_half2(std::pow(std::sin(light.outerConeAngle), 2.0f), std::pow(std::cos(light.outerConeAngle), 2.0f));
	...
	for (uint32_t lightIndex : vis.visibleLights)
	{
		...
		if (!light.lensFlareRimTextures.empty())
		{
			...
			if (light.GetType() == LightComponent::DIRECTIONAL)
			{
				...
				{
					device->BindResource(chif::texturehelper::getWhite(), 0, cmd);
				}
				else
				{
...
)
{
	...
	if ((!vis.visibleLights.empty() || vis.scene->weather.rain_amount > 0) && shadowMapAtlas.IsValid())
	{
		...
		auto range_cpu = chif::profiler::BeginRangeCPU("Shadowmap Rendering");
		auto range_gpu = chif::profiler::BeginRangeGPU("Shadowmap Rendering", cmd);
		...
		chif::profiler::EndRange(range_gpu);
		chif::profiler::EndRange(range_cpu);
		device->EventEnd(cmd);
	}
}
...
)
{
	...
	if (opaque || transparent)
	{
		...
		for (uint32_t instanceIndex : vis.visibleObjects)
		{
			...
			const float distance = chif::math::Distance(vis.camera->Eye, object.center);
...
)
{
	...
	{
		const XMFLOAT4 colliderColor = chif::Color(88, 218, 83, 255);
	...
	{
		const XMFLOAT4 springDebugColor = chif::Color(255, 70, 165, 255);
		for (size_t i = 0; i < scene.springs.GetCount(); ++i)
		{
			...
			if (transform != nullptr)
			{
				...
				chif::renderer::DrawLine(line, false);
			}
			chif::primitive::Sphere sphere;
			sphere.center = spring.currentTail;
			sphere.radius = spring.hitRadius;
			chif::renderer::DrawSphere(sphere, springDebugColor, false);
		}
	}
		...
		)
	{
		...
		auto draw_triangle = [&](chif::vector<RenderableTriangle>& dbg) {
			if (dbg.empty())
				return;
			MiscCB sb;
			XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
	...
	if(!renderableLines.empty() || !renderableLines_depth.empty())
	{
		...
		auto draw_line = [&](chif::vector<RenderableLine>& dbg) {
			if (dbg.empty())
				return;
			MiscCB sb;
			XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
	...
	if(!renderablePoints.empty() || !renderablePoints_depth.empty())
	{
		...
		auto draw_point = [&](chif::vector<RenderablePoint>& dbg) {
			if (dbg.empty())
				return;
			MiscCB sb;
	...
	if(!renderableBoxes.empty() || !renderableBoxes_depth.empty())
	{
		...
		auto draw_box = [&](chif::vector<std::pair<XMFLOAT4X4, XMFLOAT4>>& dbg) {
			if (dbg.empty())
				return;
	...
	if (!renderableSpheres.empty() || !renderableSpheres_depth.empty())
	{
		...
		auto draw_sphere = [&](chif::vector<std::pair<Sphere, XMFLOAT4>>& dbg) {
			if (dbg.empty())
				return;
	...
	if (!renderableCapsules.empty() || !renderableCapsules_depth.empty())
	{
		...
		auto draw_capsule = [&](chif::vector<std::pair<Capsule, XMFLOAT4>>& dbg) {
			if (dbg.empty())
				return;
			MiscCB sb;
			XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
	...
	if (!debugTextStorage.empty())
	{
		...
		static thread_local chif::vector<DebugTextSorter> sorted_texts;
		sorted_texts.clear();
		size_t offset = 0;
		while(offset < debugTextStorage.size())
		{
			...
			offset += x.text_len + 1;
			x.distance = chif::math::Distance(x.params.position, camera.Eye);
		...
		for (auto& x : sorted_texts)
		{
			...
			params.color = chif::Color::fromFloat4(x.params.color);
			params.intensity = x.params.color.w > 1 ? x.params.color.w : 1;
			params.h_align = chif::font::WIFALIGN_CENTER;
			params.v_align = chif::font::WIFALIGN_CENTER;
			params.softness = 0.0f;
			params.shadowColor = chif::Color::Black();
			...
			chif::font::Draw(x.text, x.text_len, params, cmd);
		}
	...
	const Texture* weatherMapSecond)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Volumetric Clouds Shadow", cmd);
...
void ComputeSkyAtmosphereTextures(CommandList cmd)
{
	...
	auto range = chif::profiler::BeginRangeGPU("SkyAtmosphere Textures", cmd);
...
void RefreshEnvProbes(const Visibility& vis, CommandList cmd)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Environment Probe Refresh", cmd);
	...
	auto render_probe = [&](const EnvironmentProbeComponent& probe, const AABB& probe_aabb) {
		...
		static chif::unordered_map<uint32_t, Texture> render_textures;
		static std::mutex locker;
		{
			...
			if (!envrenderingDepthBuffer.IsValid())
			{
				...
				info += "\n\tMemory = " + chif::helper::GetMemorySizeText(ComputeTextureMemorySizeInBytes(desc)) + "\n";
				chif::backlog::post(info);
			}
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("VXGI - Voxelize", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("VXGI - Resolve", cmd);
...
{
	auto range = chif::profiler::BeginRangeGPU("Entity Culling", cmd);
...
void BlockCompress(const Texture& texture_src, const Texture& texture_bc, CommandList cmd, uint32_t dst_slice_offset)
{
	...
	Texture bc_raw_dest;
	{
		...
		if (!bc_raw->IsValid() || bc_raw->desc.width < desc.width || bc_raw->desc.height < desc.height || bc_raw->desc.array_size < desc.array_size)
		{
			...
			bc_raw_desc.width = chif::math::GetNextPowerOfTwo(bc_raw_desc.width);
			bc_raw_desc.height = chif::math::GetNextPowerOfTwo(bc_raw_desc.height);
			...
			info += "\n\tMemory = " + chif::helper::GetMemorySizeText(total_size) + "\n";
			chif::backlog::post(info);
		}
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("RayTraceScene", cmd);

	const TextureDesc& desc = output.GetDesc();

	BindCommonResources(cmd);

	const XMFLOAT4& halton = chif::math::GetHaltonSequence(accumulation_sample);
	RaytracingCB cb = {};
...
void RefreshLightmaps(const Scene& scene, CommandList cmd)
{
	...
	{
		auto range = chif::profiler::BeginRangeGPU("Lightmap Processing", cmd);
		...
		for (uint32_t requestIndex = 0; requestIndex < lightmap_request_count; ++requestIndex)
		{
			...
			if (object.IsLightmapRenderRequested())
			{
				...
				cb.xTraceUserData.x = raytraceBounceCount;
				XMFLOAT4 halton = chif::math::GetHaltonSequence(object.lightmapIterationCount); // for jittering the rasterization (good for eliminating atlas border artifacts)
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Luminance", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Bloom", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("ComputeShadingRateClassification", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Visibility_Prepare", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Visibility_Surface", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Visibility_Surface_Reduced", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Visibility_Shade", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Visibility_Velocity", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("SurfelGI - Coverage", cmd);
	...
	chif::profiler::EndRange(prof_range);
	device->EventEnd(cmd);
}
void SurfelGI(
	const SurfelGIResources& res,
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("SurfelGI", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("DDGI", cmd);
	...
	// Raytracing:
	{
		...
		MiscCB cb = {};
		float angle = chif::random::GetRandom(0.0f, 1.0f) * XM_2PI;
		XMVECTOR axis = XMVectorSet(
			chif::random::GetRandom(-1.0f, 1.0f),
			chif::random::GetRandom(-1.0f, 1.0f),
			chif::random::GetRandom(-1.0f, 1.0f),
			0
		);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("SSAO", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("HBAO", cmd);
	...
	)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("MSAO", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("RTAO", cmd);
...
)
{
	...
	auto profilerRange = chif::profiler::BeginRangeGPU("RTDiffuse", cmd);
	...
	chif::profiler::EndRange(profilerRange);
	device->EventEnd(cmd);
}
...
)
{
	...
	auto profilerRange = chif::profiler::BeginRangeGPU("SSGI", cmd);
...
)
{
	...
	auto profilerRange = chif::profiler::BeginRangeGPU("RTReflection", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("SSR", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("RTShadow", cmd);
...
)
{
	...
	auto prof_range = chif::profiler::BeginRangeGPU("ScreenSpaceShadow", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("LightShafts", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Depth of Field", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Outline", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("MotionBlur", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Aerial Perspective", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Volumetric Clouds", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Volumetric Clouds Upsample", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("FXAA", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Temporal AA Resolve", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Postprocess_FSR", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Postprocess_FSR2", cmd);
	...
	if (resetAccumulation)
	{
		...
		uint32_t clear_lock_pk = chif::math::Pack_R11G11B10_FLOAT(XMFLOAT3(clearValuesLockStatus[0], clearValuesLockStatus[1], clearValuesLockStatus[2]));
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU("Underwater", cmd);
...
)
{
	...
	auto range = chif::profiler::BeginRangeGPU(debug_name, cmd);
...
Ray GetPickRay(long cursorX, long cursorY, const chif::Canvas& canvas, const CameraComponent& camera)
{
...
chif::Resource CreatePaintableTexture(uint32_t width, uint32_t height, uint32_t mips, chif::Color initialColor)
{
	TextureDesc desc;
	desc.width = width;
	desc.height = height;
	...
	chif::vector<chif::Color> data(desc.width * desc.height);
