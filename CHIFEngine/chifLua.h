...
namespace chif::lua
{
	...
	inline bool CompileFile(const std::string& filename, chif::vector<uint8_t>& dst) { return CompileFile(filename.c_str(), dst); }
	// Compiles LUA source code text into binary LUA code
	bool CompileText(const char* script, chif::vector<uint8_t>& dst);
	inline bool CompileText(const std::string& script, chif::vector<uint8_t>& dst) { return CompileText(script.c_str(), dst); }
