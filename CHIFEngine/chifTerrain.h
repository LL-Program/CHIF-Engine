...
{
	template <>
	struct hash<chif::terrain::Chunk>
	{
		inline size_t operator()(const chif::terrain::Chunk& chunk) const
		{
...
namespace chif::terrain
{
	...
	struct VirtualTextureAtlas
	{
		struct Map
		{
			...
			chif::graphics::Texture texture_raw_block;
		};
		Map maps[4];
		...
		chif::vector<PhysicalTile> physical_tiles;

		struct Residency
		{
			chif::graphics::Texture feedbackMap;
			chif::graphics::Texture residencyMap;
			chif::graphics::GPUBuffer requestBuffer;
			chif::graphics::GPUBuffer allocationBuffer;
			chif::graphics::GPUBuffer allocationBuffer_CPU_readback[chif::graphics::GraphicsDevice::GetBufferCount()];
			chif::graphics::GPUBuffer pageBuffer;
			chif::graphics::GPUBuffer pageBuffer_CPU_upload[chif::graphics::GraphicsDevice::GetBufferCount()];
			bool data_available_CPU[chif::graphics::GraphicsDevice::GetBufferCount()] = {};
		...
		chif::unordered_map<uint32_t, chif::vector<std::shared_ptr<Residency>>> free_residencies; // per resolution residencies
	...
	struct VirtualTexture
	{
		...
		chif::vector<VirtualTextureAtlas::Tile> tiles;
		...
		chif::vector<AllocationRequest> allocation_requests;
		...
		mutable chif::vector<UpdateRequest> update_requests;
		chif::graphics::Texture blendmap;
	};

	struct BlendmapLayer
	{
		chif::vector<uint8_t> pixels;
	};

	struct ChunkData
	{
		chif::ecs::Entity entity = chif::ecs::INVALID_ENTITY;
		chif::ecs::Entity grass_entity = chif::ecs::INVALID_ENTITY;
		chif::ecs::Entity props_entity = chif::ecs::INVALID_ENTITY;
		const XMFLOAT3* mesh_vertex_positions = nullptr;
		float prop_density_current = 1;
		chif::HairParticleSystem grass;
		float grass_density_current = 1;
		chif::vector<BlendmapLayer> blendmap_layers;
		chif::graphics::Texture blendmap;
		chif::primitive::Sphere sphere;
		XMFLOAT3 position = XMFLOAT3(0, 0, 0);
		bool visible = true;
		std::shared_ptr<VirtualTexture> vt;
		chif::vector<uint16_t> heightmap_data;
		chif::graphics::Texture heightmap;
	...
	struct Prop
	{
		chif::vector<uint8_t> data; // serialized component data storage
	...
	struct Terrain
	{
		...
		chif::ecs::Entity terrainEntity = chif::ecs::INVALID_ENTITY;
		chif::ecs::Entity chunkGroupEntity = chif::ecs::INVALID_ENTITY;
		chif::scene::Scene* scene = nullptr;
		chif::vector<chif::ecs::Entity> materialEntities = {};
		chif::ecs::Entity grassEntity = chif::ecs::INVALID_ENTITY;
		chif::scene::WeatherComponent weather;
		chif::HairParticleSystem grass_properties;
		chif::scene::MaterialComponent grass_material;
		chif::unordered_map<Chunk, ChunkData> chunks;
		Chunk center_chunk = {};
		chif::noise::Perlin perlin_noise;
		chif::vector<Prop> props;
		...
		chif::vector<VirtualTexture*> virtual_textures_in_use;
		chif::graphics::Sampler sampler;
		VirtualTextureAtlas atlas;

		int chunk_buffer_range = 3; // how many chunks to upload to GPU in X and Z directions
		chif::graphics::GPUBuffer chunk_buffer;
		...
		chif::vector<std::shared_ptr<Modifier>> modifiers;
		chif::vector<Modifier*> modifiers_to_remove;
		...
		void Generation_Update(const chif::scene::CameraComponent& camera);
		...
		void UpdateVirtualTexturesGPU(chif::graphics::CommandList cmd) const;
		void CopyVirtualTexturePageStatusGPU(chif::graphics::CommandList cmd) const;
		void AllocateVirtualTextureTileRequestsGPU(chif::graphics::CommandList cmd) const;
		void WritebackTileRequestsGPU(chif::graphics::CommandList cmd) const;
	...
	private:
		chif::vector<chif::scene::MaterialComponent> materials; // temp storage allocation
	};
	...
	struct Modifier
	{
		...
		constexpr void Blend(float& height, float value)
		{
			...
			case BlendMode::Normal:
				height = chif::math::Lerp(height, value, weight);
				break;
	...
	struct VoronoiModifier : public Modifier
	{
		...
		void Apply(const XMFLOAT2& world_pos, float& height) override
		{
			...
			chif::noise::voronoi::Result res = chif::noise::voronoi::compute(p.x, p.y, (float)seed);
