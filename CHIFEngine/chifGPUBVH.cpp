...
namespace chif
{
	...
	void GPUBVH::Update(const chif::scene::Scene& scene)
	{
	...
	void GPUBVH::Build(const Scene& scene, CommandList cmd) const
	{
		...
		auto range = chif::profiler::BeginRangeGPU("BVH Rebuild", cmd);
		...
		chif::gpusortlib::Sort(primitiveCount, primitiveMortonBuffer, primitiveCounterBuffer, 0, primitiveIDBuffer, cmd);
		device->EventEnd(cmd);
		...
		chif::profiler::EndRange(range); // BVH rebuild
	...
	namespace GPUBVH_Internal
	{
		void LoadShaders()
		{
			chif::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PRIMITIVES], "bvh_primitivesCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_HIERARCHY], "bvh_hierarchyCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PROPAGATEAABB], "bvh_propagateaabbCS.cso");
		}
	}
	...
	void GPUBVH::Initialize()
	{
		...
		static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { GPUBVH_Internal::LoadShaders(); });
		GPUBVH_Internal::LoadShaders();

		chif::backlog::post("chif::GPUBVH Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
}
