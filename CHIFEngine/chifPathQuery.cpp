...
namespace chif
{
	...
	{
		const chif::vector<XMFLOAT3>& results = result_path_goal_to_start_simplified.empty() ? result_path_goal_to_start : result_path_goal_to_start_simplified;
		if (results.size() < 2)
	...
	namespace PathQuery_internal
	{
		...
		static void LoadShaders()
		{
			...
			desc.dss = chif::renderer::GetDepthStencilState(chif::enums::DSSTYPE_DEPTHREAD);
			...
			desc.rs = chif::renderer::GetRasterizerState(chif::enums::RSTYPE_FRONT);
			desc.bs = chif::renderer::GetBlendState(chif::enums::BSTYPE_ADDITIVE);
	...
	void PathQuery::debugdraw(const XMFLOAT4X4& ViewProjection, CommandList cmd) const
	{
		...
		if (debug_voxels)
		{
			...
			if (results.size() >= 2)
			{
				...
				{
					const chif::vector<XMFLOAT3>& srcarray = i < result_path_goal_to_start.size() ? result_path_goal_to_start : result_path_goal_to_start_simplified;
		...
		if (results.size() >= 2)
		{
			...
			static XMFLOAT4 color0 = chif::Color(10, 10, 20, 255);
			static XMFLOAT4 color1 = chif::Color(70, 150, 170, 255);
			...
			for (int i = 0; i < count; ++i)
			{
				...
				for (uint32_t j = 0; j < segment_resolution; ++j)
				{
...
#if 1
					XMVECTOR P = cap ? XMVectorLerp(P1, P2, t) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t, curve_tension);
					XMVECTOR P_prev = cap ? XMVectorLerp(P0, P1, t - resolution_rcp) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t - resolution_rcp, curve_tension);
					XMVECTOR P_next = cap ? XMVectorLerp(P1, P2, t + resolution_rcp) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t + resolution_rcp, curve_tension);
#else
					...
					Vertex vert;
					vert.color = chif::math::Lerp(color0, color1, saturate(std::sin(segmenti + debugtimer) * 0.5f + 0.5f));
