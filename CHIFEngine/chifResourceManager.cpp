...
namespace chif
{
	...
	struct ResourceInternal
	{
		...
		chif::graphics::GPUBuffer tile_pool;
		chif::graphics::Texture texture_feedback;
		chif::graphics::Texture texture_residency;
	};

	const chif::vector<uint8_t>& Resource::GetFileData() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
		return resourceinternal->filedata;
	}
	const chif::graphics::Texture& Resource::GetTexture() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
		return resourceinternal->texture;
	}
	const chif::audio::Sound& Resource::GetSound() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
	...
	const chif::video::Video& Resource::GetVideo() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
	...
	void Resource::SetFileData(const chif::vector<uint8_t>& data)
	{
		if (internal_state == nullptr)
		{
	...
	void Resource::SetFileData(chif::vector<uint8_t>&& data)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	void Resource::SetTexture(const chif::graphics::Texture& texture, int srgb_subresource)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	void Resource::SetSound(const chif::audio::Sound& sound)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	void Resource::SetVideo(const chif::video::Video& video)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	namespace resourcemanager
	{
		...
		static const chif::unordered_map<std::string, DataType> types = {
		...
		{
			std::string ext = chif::helper::toUpper(chif::helper::GetExtensionFromFileName(name));
			DataType type;
			...
			case DataType::IMAGE:
			{
				...
				if (!ext.compare("KTX2"))
				{
					...
					if (transcoder.init(filedata, (uint32_t)filesize))
					{
						...
						if (transcoder.start_transcoding())
						{
							...
							chif::vector<uint8_t> transcoded_data(transcoded_data_size);

							chif::vector<SubresourceData> InitData;
								...
								for (uint32_t face = 0; face < faces; ++face)
								{
									for (uint32_t mip = 0; mip < levels; ++mip)
									{
										...
										if (transcoder.get_image_level_info(level_info, mip, layer, face))
										{
											...
											{
												chif::backlog::post("KTX2 transcoding error while loading image!", chif::backlog::LogLevel::Error);
												assert(0);
											}
										}
										else
										{
											chif::backlog::post("KTX2 transcoding error while loading image level info!", chif::backlog::LogLevel::Error);
											assert(0);
										}
									}
								}
								...
								if (transcoder.start_transcoding(filedata, (uint32_t)filesize))
								{
									...
									for (uint32_t mip = 0; mip < desc.mip_levels; ++mip)
									{
										...
										if (transcoder.get_image_level_info(filedata, (uint32_t)filesize, level_info, 0, mip))
										{
											...
											{
												chif::backlog::post("BASIS transcoding error while loading image!", chif::backlog::LogLevel::Error);
												assert(0);
											}
										}
										else
										{
											chif::backlog::post("BASIS transcoding error while loading image level info!", chif::backlog::LogLevel::Error);
											assert(0);
										}
									}
				...
				else if (!ext.compare("DDS"))
				{
					...
					if (header.is_valid())
					{
						...
						chif::vector<SubresourceData> initdata_heap;
					...
					else
					{
						if (!has_flag(flags, Flags::IMPORT_COLORGRADINGLUT) && stbi_is_16_bit_from_memory(filedata, (int)filesize))
						{
							...
							case 3:
							{
								...
								chif::Color16* color4 = (chif::Color16*)malloc(width * height * sizeof(chif::Color16));
								for (int i = 0; i < width * height; ++i)
								{
						...
						else
						{
							...
							case 3:
							{
								...
								const Color3* color3 = (const Color3*)rgba;
								chif::Color* color4 = (chif::Color*)malloc(width * height * sizeof(chif::Color));
								for (int i = 0; i < width * height; ++i)
								{
									color4[i].setR(color3[i].r);
									color4[i].setG(color3[i].g);
					...
					if (rgba != nullptr)
					{
						...
						if (has_flag(flags, Flags::IMPORT_COLORGRADINGLUT))
						{
							if (desc.type != TextureDesc::Type::TEXTURE_2D ||
								...
								format != Format::R8G8B8A8_UNORM)
							{
								chif::helper::messageBox("The Dimensions must be 256 x 16 for color grading LUT and format must be RGB or RGBA!", "Error");
							}
							else
							{
						...
						else
						{
							...
							chif::renderer::AddDeferredMIPGen(resource->texture, true);
							...
							if (has_flag(flags, Flags::IMPORT_BLOCK_COMPRESSED))
							{
								...
								chif::renderer::AddDeferredBlockCompression(uncompressed_src, resource->texture);
							}
						}
					}
					free(rgba);
			...
			case DataType::SOUND:
			{
				success = chif::audio::CreateSound(filedata, filesize, &resource->sound);
			}
			break;
			...
			case DataType::SCRIPT:
			{
				...
				resource->script_hash = chif::helper::string_hash(resource->script.c_str());
				success = true;
			}
			break;
			...
			case DataType::VIDEO:
			{
				success = chif::video::CreateVideo(filedata, filesize, &resource->video);
			}
			break;

			case DataType::FONTSTYLE:
			{
				resource->font_style = chif::font::AddFontStyle(name, filedata, filesize, true);
		...
		)
		{
			...
			{
				timestamp = chif::helper::FileTimestamp(container_filename);
			}
			else
			{
				timestamp = chif::helper::FileTimestamp(name);
			}
			...
			if (filedata == nullptr || filesize == 0)
			{
				...
				{
					if (!chif::helper::FileRead(resource->container_filename, resource->filedata, resource->container_filesize, resource->container_fileoffset))
					{
						resource.reset();
						return Resource();
					}
		...
		chif::jobsystem::context streaming_ctx;
		chif::vector<std::shared_ptr<ResourceInternal>> streaming_texture_jobs;
		...
		chif::vector<StreamingTextureReplace> streaming_texture_replacements;
		...
		void UpdateStreamingResources(float dt)
		{
			...
			for (auto& x : resources)
			{
				...
				if (resource != nullptr && resource->texture.IsValid() && has_flag(resource->flags, Flags::STREAMING))
				{
					...
					if (chif::math::float_equal(min_lod_clamp_absolute_next, resource->streaming_texture.min_lod_clamp_absolute))
						continue;
			...
			if (chif::jobsystem::IsBusy(streaming_ctx))
			{
				locker.unlock();
				return;
			}
			...
			streaming_ctx.priority = chif::jobsystem::Priority::Streaming;
			chif::jobsystem::Execute(streaming_ctx, [](chif::jobsystem::JobArgs args) {
				for(auto& resource : streaming_texture_jobs)
				{
					...
					if (desc.mip_levels <= resource->streaming_texture.mip_count)
					{
						...
						static chif::vector<uint8_t> streaming_file; // make this static to not reallocate for each file loading
		...
		bool CheckResourcesOutdated()
		{
			...
			for (auto& x : resources)
			{
				...
				uint64_t timestamp = chif::helper::FileTimestamp(resourceinternal->filename);
		...
		void ReloadOutdatedResources()
		{
			...
			for (auto& x : resources)
			{
				...
				{
					chif::vector<uint8_t> filedata;
					if (chif::helper::FileRead(resourceinternal->filename, filedata))
					{
						if (resourceinternal->streaming_texture.mip_count > 1)
							chif::jobsystem::Wait(streaming_ctx); // reloading a resource that is potentially streaming needs to wait for current streaming job to end
						if (LoadResourceDirectly(resourceinternal->filename, resourceinternal->flags, filedata.data(), filedata.size(), resourceinternal.get()))
						{
							...
							chif::backlog::post("[resourcemanager] reload success: " + resourceinternal->filename);
						}
						else
						{
							chif::backlog::post("[resourcemanager] reload failure - LoadResourceDirectly returned false: " + resourceinternal->filename, chif::backlog::LogLevel::Error);
						}
					}
					else
					{
						chif::backlog::post("[resourcemanager] reload failure - file data could not be read: " + resourceinternal->filename, chif::backlog::LogLevel::Error);
					}
				}
			}
		}

		void Serialize_READ(chif::Archive& archive, ResourceSerializer& seri)
		{
			...
			chif::vector<TempResource> temp_resources;
			temp_resources.resize(serializable_count);

			chif::jobsystem::context ctx;
			ctx.priority = chif::jobsystem::Priority::Low;
			...
			for (size_t i = 0; i < serializable_count; ++i)
			{
				...
				chif::jobsystem::Execute(ctx, [i, &temp_resources, &seri, &archive, file_offset](chif::jobsystem::JobArgs args) {
			...
			else
			{
				...
				for (auto& name : resource_names)
				{
					...
					if (resource != nullptr)
					{
						...
						chif::helper::MakePathRelative(archive.GetSourceDirectory(), name);

						if (resource->filedata.empty())
						{
							chif::helper::FileRead(
								resource->container_filename,
								resource->filedata,
