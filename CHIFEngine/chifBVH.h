...
namespace chif
{
	...
	struct BVH
	{
		...
		chif::vector<uint8_t> allocation;
		Node* nodes = nullptr;
		...
		void Build(const chif::primitive::AABB* aabbs, uint32_t aabb_count)
		{
			node_count = 0;
			if (aabb_count == 0)
				return;
			...
			{
				node.aabb = chif::primitive::AABB::Merge(node.aabb, aabbs[i]);
				leaf_indices[i] = i;
			}
			Subdivide(0, aabbs);
		}

		void Subdivide(uint32_t nodeIndex, const chif::primitive::AABB* leaf_aabb_data)
		{
		...
		void UpdateNodeBounds(uint32_t nodeIndex, const chif::primitive::AABB* leaf_aabb_data)
		{
			Node& node = nodes[nodeIndex];
			node.aabb = {};
			for (uint32_t i = 0; i < node.count; ++i)
			{
				...
				node.aabb = chif::primitive::AABB::Merge(node.aabb, leaf_aabb_data[index]);
			}
		}
