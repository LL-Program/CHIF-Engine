...
namespace chif::physics
{
	...
	namespace jolt
	{
		...
		struct SoftBody
		{
			...
			chif::vector<Neighbors> physicsNeighbors;
		...
		RigidBody& GetRigidBody(chif::scene::RigidBodyPhysicsComponent& physicscomponent)
		{
		...
		SoftBody& GetSoftBody(chif::scene::SoftBodyPhysicsComponent& physicscomponent)
		{
			if (physicscomponent.physicsobject == nullptr)
			{
		...
		)
		{
			...
			{
				chif::backlog::post("AddRigidBody failed: convex hull physics requested, but no MeshComponent provided!", chif::backlog::LogLevel::Error);
				return;
			}
			break;
			...
			{
				chif::backlog::post("AddRigidBody failed: triangle mesh physics requested, but no MeshComponent provided!", chif::backlog::LogLevel::Error);
				return;
			}
			break;
			}
			...
			if (!shape_result.IsValid())
			{
				...
				chif::backlog::post(text, chif::backlog::LogLevel::Error);
				return;
			}
			else
			{
				...
				{
					chif::backlog::post("AddRigidBody failed: body couldn't be created! This could mean that there are too many physics objects.", chif::backlog::LogLevel::Error);
					return;
				}
		...
		)
		{
			...
			{
				chif::backlog::post("AddSoftBody failed: physics faces are empty, this means generating physics mesh has failed, try to change settings.", chif::backlog::LogLevel::Error);
				return;
			}
			...
			if (physicsobject.bodyID.IsInvalid())
			{
				chif::backlog::post("AddSoftBody failed: body couldn't be created! This could mean that there are too many physics objects.", chif::backlog::LogLevel::Error);
				return;
			}
		}
		...
		struct Ragdoll
		{
			...
			Ragdoll(Scene& scene, HumanoidComponent& humanoid, Entity humanoidEntity, float scale)
			{
				...
				chif::renderer::SetGameSpeed(0.1f);
				...
				for (int c = 0; c < BODYPART_COUNT; ++c)
				{
					...
					if (entityA == INVALID_ENTITY)
					{
						chif::backlog::post("Ragdoll creation aborted because humanoid has a missing required bone.", chif::backlog::LogLevel::Warning);
						return;
					}
					...
					case BODYPART_RIGHT_UPPER_ARM:
					case BODYPART_RIGHT_LOWER_ARM:
						physicsobject.capsule = chif::primitive::Capsule(XMFLOAT3(-capsule_height * 0.5f - capsule_radius, 0, 0), XMFLOAT3(capsule_height * 0.5f + capsule_radius, 0, 0), capsule_radius);
						rtshape_settings.mRotation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * JPH_PI).Normalized();
						break;
					default:
						physicsobject.capsule = chif::primitive::Capsule(XMFLOAT3(0, -capsule_height * 0.5f - capsule_radius, 0), XMFLOAT3(0, capsule_height * 0.5f + capsule_radius, 0), capsule_radius);
						break;
					}
	...
	void Initialize()
	{
		...
		char text[256] = {};
		snprintf(text, arraysize(text), "chif::physics Initialized [Jolt Physics %d.%d.%d] (%d ms)", JPH_VERSION_MAJOR, JPH_VERSION_MINOR, JPH_VERSION_PATCH, (int)std::round(timer.elapsed()));
	...
	)
	{
		...
		auto range = chif::profiler::BeginRangeCPU("Physics");

		PhysicsScene& physics_scene = GetPhysicsScene(scene);
		physics_scene.physics_system.SetGravity(cast(scene.weather.gravity));

		// System will register rigidbodies to objects:
		chif::jobsystem::Dispatch(ctx, (uint32_t)scene.rigidbodies.GetCount(), 64, [&scene, &physics_scene](chif::jobsystem::JobArgs args) {
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&scene, &physics_scene](chif::jobsystem::JobArgs args) {
			...
			if (physicscomponent.physicsobject != nullptr)
			{
				...
				for (size_t ind = 0; ind < physicscomponent.physicsToGraphicsVertexMapping.size(); ++ind)
				{
					...
					{
						XMFLOAT3 position = mesh->vertex_positions[graphicsInd];
						XMVECTOR P = armature == nullptr ? XMLoadFloat3(&position) : chif::scene::SkinVertex(*mesh, *armature, graphicsInd);
						P = XMVector3Transform(P, worldMatrix);
						XMStoreFloat3(&position, P);
		...
		// Ragdoll management:
		chif::jobsystem::Dispatch(ctx, (uint32_t)scene.humanoids.GetCount(), 1, [&scene, &physics_scene](chif::jobsystem::JobArgs args) {
			...
			if (humanoid.ragdoll != nullptr)
			{
				...
				if (!chif::math::float_equal(ragdoll.scale, scale))
				{
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&scene, &physics_scene](chif::jobsystem::JobArgs args) {
			...
			physicscomponent.aabb = chif::primitive::AABB();
...
#if 0
				scene.locker.lock();
				chif::renderer::DrawAxis(W, 0.05f, false);
				scene.locker.unlock();
#endif

				physicscomponent.aabb._min = chif::math::Min(physicscomponent.aabb._min, p0);
				physicscomponent.aabb._max = chif::math::Max(physicscomponent.aabb._max, p0);
			}
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)scene.humanoids.GetCount(), 1, [&scene, &physics_scene](chif::jobsystem::JobArgs args) {
			...
			humanoid.ragdoll_bounds = chif::primitive::AABB();
			int caps = 0;
			...
			for (auto& rb : ragdoll.rigidbodies)
			{
				...
				humanoid.ragdoll_bounds = chif::primitive::AABB::Merge(humanoid.ragdoll_bounds, bp.capsule.getAABB());

#if 0
				scene.locker.lock();
				chif::renderer::DrawCapsule(bp.capsule, XMFLOAT4(1, 1, 1, 1), false);
				scene.locker.unlock();
#endif
			...
			chif::renderer::DrawBox(mat, XMFLOAT4(1, 1, 1, 1), false);
			scene.locker.unlock();
#endif
		});
		...
		if (IsDebugDrawEnabled())
		{
			class JoltDebugRenderer : public DebugRendererSimple
			{
				void DrawLine(RVec3Arg inFrom, RVec3Arg inTo, ColorArg inColor) override
				{
					...
					line.color_start = line.color_end = chif::Color(inColor.r, inColor.g, inColor.b, inColor.a);
				...
				void DrawText3D(RVec3Arg inPosition, const string_view& inString, ColorArg inColor = JPH::Color::sWhite, float inHeight = 0.5f) override
				{
					...
					params.scaling = 0.6f;
					params.flags |= chif::renderer::DebugTextParams::CAMERA_FACING;
					params.flags |= chif::renderer::DebugTextParams::CAMERA_SCALING;
					chif::renderer::DrawDebugText(inString.data(), params);
				}
			};
		...
		chif::profiler::EndRange(range); // Physics
	}
	...
	)
	{
		...
		switch (state)
		{
		case chif::physics::ActivationState::Active:
			body_interface.ActivateBody(physicsobject.bodyID);
			break;
		case chif::physics::ActivationState::Inactive:
	...
	)
	{
		...
		else
		{
			...
			internal_state->pick_distance = chif::math::Distance(ray.origin, result.position);
