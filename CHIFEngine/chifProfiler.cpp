...
namespace chif::profiler
{
	...
	chif::Color background_color = chif::Color(20, 20, 20, 230);
	chif::Color text_color = chif::Color::White();
	...
	chif::unordered_map<size_t, Range> ranges;
	...
	void BeginFrame()
	{
		...
		if (!initialized)
		{
			...
			{
				chif::backlog::post("[chif::profiler] Superluminal Performance API loaded");
			}
#endif // PERFORMANCEAPI_ENABLED
		}
	...
	range_id BeginRangeCPU(const char* name)
	{
		...
		range_id id = chif::helper::string_hash(name);
		...
		{
			chif::helper::hash_combine(id, differentiator++);
		}
	...
	void LoadShaders()
	{
		...
		desc.rs = chif::renderer::GetRasterizerState(chif::enums::RSTYPE_WIRE_SMOOTH);
	...
	chif::unordered_map<std::string, Hits> time_cache_cpu;
	chif::unordered_map<std::string, Hits> time_cache_gpu;
	void DrawData(
	...
	)
	{
		...
		chif::font::Params params = chif::font::Params(x, y + (graph_size.y + graph_padding_y) * 2, chif::font::WIFONTSIZE_DEFAULT - 6, chif::font::WIFALIGN_LEFT, chif::font::WIFALIGN_TOP, text_color);

		// Background:
		chif::image::Params fx;
		fx.pos.x = x - 10;
		fx.pos.y = y - 10;
		fx.siz.x = std::max(graph_size.x, (float)chif::font::TextWidth(ss.str(), params)) + 200;
		fx.siz.y = (float)chif::font::TextHeight(ss.str(), params) + (graph_size.y + graph_padding_y) * 2 + 20;
		...
		chif::font::Draw(ss.str(), params, cmd);


		// Graph:
		{
			GraphicsDevice* device = chif::graphics::GetDevice();
			chif::graphics::GraphicsDevice::MemoryUsage gpu_memory_usage = device->GetMemoryUsage();
			chif::helper::MemoryUsage cpu_memory_usage = chif::helper::GetMemoryUsage();

			static bool shaders_loaded = false;
			if (!shaders_loaded)
			{
				shaders_loaded = true;
				static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
				LoadShaders();
			}
			...
			chif::font::Draw("16.6 ms", params, cmd);
			...
			params.color = chif::Color::fromFloat4(XMFLOAT4(1, 0.2f, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = chif::Color::fromFloat4(XMFLOAT4(0.2f, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = chif::Color::fromFloat4(XMFLOAT4(1, 0.2f, 1, 1));
			ss.str("");
			ss.clear();
			...
			params.color = chif::Color::fromFloat4(XMFLOAT4(1, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			chif::font::Draw("60 frames", params, cmd);
			params.position.x = x + graph_size.x;
			chif::font::Draw("current frame", params, cmd);
