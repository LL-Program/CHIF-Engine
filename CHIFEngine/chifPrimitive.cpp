...
namespace chif::primitive
{
	...
	{
		return AABB(chif::math::Min(a.getMin(), b.getMin()), chif::math::Max(a.getMax(), b.getMax()));
	}
	void AABB::Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri)
	{
	...
	{
		float distsq = chif::math::DistanceSquared(XMLoadFloat3(&center), P);
		float radiussq = radius * radius;
		return distsq < radiussq;
	}
	bool Sphere::intersects(const XMFLOAT3& P) const
	{
		float distsq = chif::math::DistanceSquared(center, P);
		float radiussq = radius * radius;
		return distsq < radiussq;
	}
	bool Sphere::intersects(const AABB& b) const
	{
		...
		XMFLOAT3 closestPointInAabb = chif::math::Min(chif::math::Max(center, min), max);
		float distanceSquared = chif::math::DistanceSquared(closestPointInAabb, center);
	...
	{
		dist = chif::math::Distance(center, b.center);
	...
	bool Sphere::intersects(const Capsule& b, float& dist) const
	{
		...
		dist = chif::math::GetPointSegmentDistance(C, A, B);
		dist = dist - radius - b.radius;
		return dist < 0;
	}
	bool Sphere::intersects(const Capsule& b, float& dist, XMFLOAT3& direction) const
	{
		...
		B += N * b.radius;
		XMVECTOR C = XMLoadFloat3(&center);
		XMVECTOR D = C - chif::math::ClosestPointOnLineSegment(A, B, C);
	...
	bool Capsule::intersects(const Capsule& other, XMFLOAT3& position, XMFLOAT3& incident_normal, float& penetration_depth) const
	{
		...
		XMVECTOR bestB = chif::math::ClosestPointOnLineSegment(b_A, b_B, bestA);

		// Now do the same for capsule A segment:
		bestA = chif::math::ClosestPointOnLineSegment(a_A, a_B, bestB);
	...
	bool Capsule::intersects(const Ray& ray, float& dist, XMFLOAT3& direction) const
	{
		...
		XMVECTOR P = chif::math::ClosestPointOnLineSegment(A, B, I);
	...
	bool Capsule::intersects(const XMFLOAT3& point) const
	{
		...
		XMVECTOR C = chif::math::ClosestPointOnLineSegment(A, B, P);
	...
	bool Plane::intersects(const Sphere& b, float& dist, XMFLOAT3& direction) const
	{
		...
		dist = chif::math::GetPlanePointDistance(XMLoadFloat3(&origin), XMLoadFloat3(&normal), C);
		direction = normal;
		if (dist < 0)
		{
			direction.x *= -1;
	...
	bool Plane::intersects(const Capsule& b, float& dist, XMFLOAT3& direction) const
	{
		...
		else
		{
			...
			C = chif::math::ClosestPointOnLineSegment(A, B, LinePlaneIntersection);
		}

		dist = chif::math::GetPlanePointDistance(O, N, C);
	...
	{
		return chif::math::Collision2D(pos, siz, b.pos, b.siz);
	}
