...
namespace chif::scene
{
	...
	void Scene::Update(float dt)
	{
		...
		if (dt > 0)
		{
			...
			{
				chif::terrain::Terrain& terrain = terrains[i];
		...
		if(chif::renderer::GetOcclusionCullingEnabled() && !chif::renderer::GetFreezeCullingCameraEnabled())
		{
		...
		if (dt > 0)
		{
			...
			chif::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [this](chif::jobsystem::JobArgs args) {
			...
			chif::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
			...
			chif::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
		...
		chif::physics::RunPhysicsUpdateSystem(ctx, *this, dt);

		RunTransformUpdateSystem(ctx);

		chif::jobsystem::Wait(ctx); // dependencies
		...
		if (chif::renderer::GetDDGIEnabled())
		{
			ddgi.frame_index++;
...

		{
			...
			{
				chif::VoxelGrid& voxelgrid = voxel_grids[voxelgridIndex];
		...
		shaderscene.weather.sun_color = chif::math::pack_half3(weather.sunColor);
		shaderscene.weather.sun_direction = chif::math::pack_half3(weather.sunDirection);
		shaderscene.weather.most_important_light_index = weather.most_important_light_index;
		shaderscene.weather.ambient = chif::math::pack_half3(weather.ambient);
		...
		shaderscene.weather.horizon = chif::math::pack_half3(weather.horizon);
		shaderscene.weather.zenith = chif::math::pack_half3(weather.zenith);
	...
	void Scene::Merge(Scene& other)
	{
		...
		collider_deinterleaved_data.reserve(
			sizeof(chif::primitive::AABB) * colliders.GetCount() +
			sizeof(ColliderComponent) * colliders.GetCount() +
			sizeof(ColliderComponent) * colliders.GetCount()
		);
		aabb_colliders_cpu = (chif::primitive::AABB*)collider_deinterleaved_data.data();
	...
	Entity Scene::Instantiate(Scene& prefab, bool attached)
	{
		...
		archive = chif::Archive(prefab.optimized_instatiation_data.GetData(), prefab.optimized_instatiation_data.GetSize());
		...
		chif::jobsystem::Wait(seri.ctx); // wait for completion of component serializations background threads here

		Merge(tmp);

		char text[64] = {};
		snprintf(text, arraysize(text), "Scene::Instantiate took %.2f ms", timer.elapsed_milliseconds());
		chif::backlog::post(text);

		return rootEntity;
	}
	void Scene::FindAllEntities(chif::unordered_set<chif::ecs::Entity>& entities) const
	{
		for (auto& entry : componentLibrary.entries)
		{
	...
	void Scene::Entity_Remove(Entity entity, bool recursive, bool keep_sorted)
	{
		if (recursive)
		{
			chif::vector<Entity> entities_to_remove;
	...
	bool Scene::Entity_IsDescendant(chif::ecs::Entity entity, chif::ecs::Entity ancestor) const
	{
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			sound.soundResource = chif::resourcemanager::Load(filename);
			chif::audio::CreateSoundInstance(&sound.soundResource.GetSound(), &sound.soundinstance);
		}
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			video.videoResource = chif::resourcemanager::Load(filename);
			chif::video::CreateVideoInstance(&video.videoResource.GetVideo(), &video.videoinstance);
		}
	...
	void Scene::RunAnimationUpdateSystem(chif::jobsystem::context& ctx)
	{
		auto range = chif::profiler::BeginRangeCPU("Animations");

		chif::jobsystem::Wait(animation_dependency_scan_workload);

		chif::jobsystem::Dispatch(ctx, (uint32_t)animation_queue_count, 1, [&](chif::jobsystem::JobArgs args) {
				...
				for (const AnimationComponent::AnimationChannel& channel : animation.channels)
				{
					...
					else
					{
						...
						case AnimationComponent::AnimationSampler::Mode::STEP:
						{
							...
							const int key = chif::math::InverseLerp(timeLeft, timeRight, animation.timer) > 0.5f ? keyRight : keyLeft;
						...
						case AnimationComponent::AnimationSampler::Mode::LINEAR:
						{
							...
							case AnimationComponent::AnimationChannel::PathDataType::Float:
							{
								...
								float vAnim = chif::math::Lerp(vLeft, vRight, t);
					...
					const bool isRootBone = (animation.IsRootMotion() && animation.rootMotionBone != chif::ecs::INVALID_ENTITY && (target_transform == transforms.GetComponent(animation.rootMotionBone)));
					...
					if (target_mesh != nullptr)
					{
						...
						{
							target_mesh->morph_targets[j].weight = chif::math::Lerp(target_mesh->morph_targets[j].weight, animation.morph_weights_temp[j], t);
						}
					}
					...
					if (target_light != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
						{
							target_light->color = chif::math::Lerp(target_light->color, interpolator.f3, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
						{
							target_light->intensity = chif::math::Lerp(target_light->intensity, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
						{
							target_light->range = chif::math::Lerp(target_light->range, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
						{
							target_light->innerConeAngle = chif::math::Lerp(target_light->innerConeAngle, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
						{
							target_light->outerConeAngle = chif::math::Lerp(target_light->outerConeAngle, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_sound != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
						{
							target_sound->volume = chif::math::Lerp(target_sound->volume, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_emitter != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
						{
							target_emitter->count = chif::math::Lerp(target_emitter->count, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_camera != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
						{
							target_camera->fov = chif::math::Lerp(target_camera->fov, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
						{
							target_camera->focal_length = chif::math::Lerp(target_camera->focal_length, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
						{
							target_camera->aperture_size = chif::math::Lerp(target_camera->aperture_size, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
						{
							target_camera->aperture_shape = chif::math::Lerp(target_camera->aperture_shape, interpolator.f2, t);
						}
						break;
						default:
							break;
					...
					if (target_material != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
						{
							target_material->baseColor = chif::math::Lerp(target_material->baseColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
						{
							target_material->emissiveColor = chif::math::Lerp(target_material->emissiveColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
						{
							target_material->roughness = chif::math::Lerp(target_material->roughness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
						{
							target_material->metalness = chif::math::Lerp(target_material->metalness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
						{
							target_material->reflectance = chif::math::Lerp(target_material->reflectance, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
						{
							target_material->texMulAdd = chif::math::Lerp(target_material->texMulAdd, interpolator.f4, t);
						}
						break;
						default:
							break;
	...
	void Scene::RunTransformUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)transforms.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {

			TransformComponent& transform = transforms[args.jobIndex];
			transform.UpdateTransform();
		});
	}
	void Scene::RunHierarchyUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)hierarchy.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunExpressionUpdateSystem(chif::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < expressions.GetCount(); ++i)
		{
			...
			if (expression_mastering.blink_timer >= 1)
			{
				...
				if (blink >= 0 && blink < expression_mastering.expressions.size())
				{
					...
					float blink_index = std::floor(chif::math::Lerp(0, (float)expression_mastering.blink_count, (expression_mastering.blink_timer - 1) / all_blink_length));
					float blink_trim = 1 + one_blink_length * blink_index;
					float blink_state = chif::math::InverseLerp(0, one_blink_length, expression_mastering.blink_timer - blink_trim);
					if (blink_state < 0.5f)
					{
						// closing
						expression.weight = chif::math::Lerp(0, 1, saturate(blink_state * 2));
					}
					else
					{
						// opening
						expression.weight = chif::math::Lerp(1, 0, saturate((blink_state - 0.5f) * 2));
					}
					if (expression_mastering.blink_timer >= 1 + all_blink_length)
					{
						expression.weight = 0;
						expression_mastering.blink_timer = -chif::random::GetRandom(0.0f, 1.0f);
					}
					expression.SetDirty();
				}
			}
			...
			if (expression_mastering.look_timer == 0)
			{
				...
				float vertical = chif::random::GetRandom(-1.0f, 1.0f);
				float horizontal = chif::random::GetRandom(-1.0f, 1.0f);
			...
			if (expression_mastering.look_timer >= 1)
			{
				...
				for (int idx = 0; idx<arraysize(looks); ++idx)
				{
					...
					if (look >= 0 && look < expression_mastering.expressions.size())
					{
						...
						float look_state = chif::math::InverseLerp(0, expression_mastering.look_length * expression_mastering.look_frequency, expression_mastering.look_timer - 1);
						if (look_state < 0.25f)
						{
							expression.weight = chif::math::Lerp(0, weight, saturate(look_state * 4));
						}
						else
						{
							expression.weight = chif::math::Lerp(weight, 0, saturate((look_state - 0.75f) * 4));
						}
						expression.SetDirty();
					}
				}
				...
				{
					expression_mastering.look_timer = -chif::random::GetRandom(0.0f, 1.0f);
				}
			}
			...
			if(voice_playing || expression_mastering.IsForceTalkingEnabled())
			{
				...
				for (int phoneme = (int)ExpressionComponent::Preset::Aa; phoneme <= (int)ExpressionComponent::Preset::Oh; phoneme++)
				{
					if (phoneme != (int)expression_mastering.talking_phoneme) // don't allow to select the current phoneme next
					{
						...
						expression.weight = chif::math::Lerp(expression.weight, 0, 0.4f); // fade out unused
						expression.SetDirty();
					}
				}
				...
				if (voice_playing)
				{
					...
					chif::audio::SampleInfo info = chif::audio::GetSampleInfo(&sound->soundResource.GetSound());
					uint32_t sample_frequency = info.sample_rate * info.channel_count;
					uint64_t current_sample = chif::audio::GetTotalSamplesPlayed(&sound->soundinstance);
					if (sound->IsLooped())
					{
					...
					if (voice > 0.1f)
					{
						expression.weight = chif::math::Lerp(expression.weight, 1, strength);
					}
					else
					{
						expression.weight = chif::math::Lerp(expression.weight, 0, strength);
					}
				}
				else
				{
				...
				if (prev_slope < 0 && curr_slope > 0)
				{
					...
					expression_mastering.talking_phoneme = unused_phonemes[chif::random::GetRandom(0, (int)arraysize(unused_phonemes) - 1)];
				}
			...
			else if (expression_mastering._flags & ExpressionComponent::TALKING_ENDED)
			{
				...
				for (auto& phoneme : phonemes)
				{
					...
					expression.weight = chif::math::Lerp(expression.weight, 0, 0.4f);
					expression.SetDirty();
			...
			for (ExpressionComponent::Expression& expression : expression_mastering.expressions)
			{
				...
				for (const ExpressionComponent::Expression::MorphTargetBinding& morph_target_binding : expression.morph_target_bindings)
				{
					...
					{
						MeshComponent::MorphTarget& morph_target = mesh->morph_targets[morph_target_binding.index];
						morph_target.weight = chif::math::Lerp(morph_target.weight, morph_target_binding.weight, blend);
					}
				}
			}
		}
	}
	void Scene::RunProceduralAnimationUpdateSystem(chif::jobsystem::context& ctx)
	{
		auto range = chif::profiler::BeginRangeCPU("Procedural Animations");
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)characters.GetCount(), 1, [&](chif::jobsystem::JobArgs args) {
			...
			if (character.IsFootPlacementEnabled() && character.ground_intersect)
			{
				...
				character.root_offset = chif::math::Lerp(character.root_offset, diff, 0.1f);
			}
			else
			{
				character.root_offset = chif::math::Lerp(character.root_offset, 0.0f, 0.1f);
			}
			...
			if (inverse_kinematics.Contains(character.left_foot))
			{
				...
				ik.target_position = chif::math::Lerp(ik.target_position, left_pos, 0.6f);
			}
			if (inverse_kinematics.Contains(character.right_foot))
			{
				...
				ik.target_position = chif::math::Lerp(ik.target_position, right_pos, 0.6f);
			}
			...
			if (ik_foot != INVALID_ENTITY && inverse_kinematics.Contains(ik_foot))
			{
				...
				ik_pos.y += 0.16f;
				ik.target_position = chif::math::Lerp(ik.target_position, ik_pos, 0.6f);
#if 0
		...
		chif::jobsystem::Dispatch(ctx,(uint32_t)inverse_kinematics.GetCount(),1,[this, &recompute_hierarchy](chif::jobsystem::JobArgs args){
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)humanoids.GetCount(), 1, [this, &recompute_hierarchy](chif::jobsystem::JobArgs args) {
			...
			for (auto& source : sources)
			{
				...
				if (boneIndex < transforms_temp.size())
				{
					...
					if (humanoid.IsLookAtEnabled())
					{
						...
						const float angle_horizontal = chif::math::GetAngle(FORWARD, TARGET_HORIZONTAL, UP, source.rotation_max->x);
						const float angle_vertical = chif::math::GetAngle(FORWARD, TARGET_VERTICAL, SIDE, source.rotation_max->y);
						...
						chif::renderer::DebugTextParams textparams;
						textparams.flags |= chif::renderer::DebugTextParams::CAMERA_FACING;
						textparams.flags |= chif::renderer::DebugTextParams::CAMERA_SCALING;
						textparams.position = humanoid.lookAt;
						textparams.scaling = 0.8f;
						chif::renderer::DrawDebugText(text.c_str(), textparams);
#endif
					}
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)colliders.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
		...
		// Springs:
		chif::jobsystem::Wait(spring_dependency_scan_workload);
		if (dt > 0)
		{
			chif::jobsystem::Dispatch(ctx, (uint32_t)spring_queues.size(), 1, [this](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunArmatureUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), 1, [&](chif::jobsystem::JobArgs args) {
			...
			for (Entity boneEntity : armature.boneCollection)
			{
				...
				_min = chif::math::Min(_min, boneAABB._min);
				_max = chif::math::Max(_max, boneAABB._max);
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)softbodies.GetCount(), 1, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunMeshUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
			...
			if (geometryArrayMapped != nullptr)
			{
				...
				if (chif::renderer::IsMeshShaderAllowed())
				{
				...
				for (auto& subset : mesh.subsets)
				{
					...
					if (chif::renderer::IsMeshShaderAllowed() && subsetIndex < mesh.cluster_ranges.size())
					{
	...
	void Scene::RunMaterialUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)materials.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunImpostorUpdateSystem(chif::jobsystem::context& ctx)
	{
	...
	void Scene::RunObjectUpdateSystem(chif::jobsystem::context& ctx)
	{
		...
		parallel_bounds.resize((size_t)chif::jobsystem::DispatchGroupCount((uint32_t)objects.GetCount(), small_subtask_groupsize));
		
		chif::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
			...
			if (!chif::renderer::GetFreezeCullingCameraEnabled())
			{
			...
			if (object.meshID != INVALID_ENTITY && meshes.Contains(object.meshID) && transforms.Contains(entity))
			{
				...
				{
					const float distsq = chif::math::DistanceSquared(camera.Eye, object.center);
				...
				union SortBits
				{
					struct
					{
						...
						uint32_t blendmode : chif::enums::BLENDMODE_COUNT;
				...
				inst.color = chif::math::pack_half4(object.color);
				inst.emissive = chif::math::pack_half3(XMFLOAT3(object.emissiveColor.x * object.emissiveColor.w, object.emissiveColor.y * object.emissiveColor.w, object.emissiveColor.z * object.emissiveColor.w));
				...
				inst.alphaTest_size = chif::math::pack_half2(XMFLOAT2(1 - object.alphaRef, size));
				inst.SetUserStencilRef(object.userStencilRef);
				inst.rimHighlight = chif::math::pack_half4(XMFLOAT4(object.rimHighlightColor.x * object.rimHighlightColor.w, object.rimHighlightColor.y * object.rimHighlightColor.w, object.rimHighlightColor.z * object.rimHighlightColor.w, object.rimHighlightFalloff));
				...
				if (TLAS_instancesMapped != nullptr)
				{
					...
					{
						instance.instance_mask &= ~chif::renderer::raytracing_inclusion_mask_shadow;
					}
					if (object.IsNotVisibleInReflections())
					{
						instance.instance_mask &= ~chif::renderer::raytracing_inclusion_mask_reflection;
					}
				...
				if (object.IsLightmapRenderRequested() && dt > 0)
				{
					...
					{
						object.lightmapWidth = chif::math::GetNextPowerOfTwo(object.lightmapWidth + 1) / 2;
						object.lightmapHeight = chif::math::GetNextPowerOfTwo(object.lightmapHeight + 1) / 2;

						// align to BC6 block size:
						object.lightmapWidth = chif::graphics::AlignTo(object.lightmapWidth, 4u);
						object.lightmapHeight = chif::graphics::AlignTo(object.lightmapHeight, 4u);
				...
				if (!object.lightmapTextureData.empty() && !object.lightmap.IsValid())
				{
					...
					chif::texturehelper::CreateTexture(object.lightmap, object.lightmapTextureData.data(), object.lightmapWidth, object.lightmapHeight, object.lightmap.desc.format);
					device->SetName(&object.lightmap, "lightmap");
				}
	...
	void Scene::RunCameraUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)cameras.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunDecalUpdateSystem(chif::jobsystem::context& ctx)
	{
	...
	void Scene::RunProbeUpdateSystem(chif::jobsystem::context& ctx)
	{
	...
	void Scene::RunForceUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)forces.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunLightUpdateSystem(chif::jobsystem::context& ctx)
	{
		aabb_lights.resize(lights.GetCount());

		chif::jobsystem::Dispatch(ctx, (uint32_t)lights.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunParticleUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)hairs.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
			...
			inst.emissive = chif::math::pack_half3(XMFLOAT3(1, 1, 1));
			inst.color = chif::math::pack_half4(XMFLOAT4(1, 1, 1, 1));
			...
			XMFLOAT4X4 remapMatrix = chif::math::IDENTITY_MATRIX;
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)emitters.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
			...
			if (TLAS_instancesMapped != nullptr)
			{
				...
				for (int i = 0; i < arraysize(instance.transform); ++i)
				{
					for (int j = 0; j < arraysize(instance.transform[i]); ++j)
					{
						instance.transform[i][j] = chif::math::IDENTITY_MATRIX.m[j][i];
					}
				}
				instance.instance_id = (uint32_t)instanceIndex;
	...
	void Scene::RunWeatherUpdateSystem(chif::jobsystem::context& ctx)
	{
		if (weathers.GetCount() > 0)
		{
			...
			if (!chif::renderer::GetFreezeCullingCameraEnabled() && weather.IsOceanEnabled())
			{
		...
		if (weather.rain_amount > 0)
		{
			...
			rainEmitter._flags |= chif::EmittedParticleSystem::FLAG_USE_RAIN_BLOCKER;
			rainEmitter.shaderType = chif::EmittedParticleSystem::PARTICLESHADERTYPE::SOFT_LIGHTING;
			...
			rainEmitter.count = chif::math::Lerp(0, (float)target_max_particle_count, weather.rain_amount);
			rainEmitter.life = 1;
			...
			{
				Texture gradientTex = chif::texturehelper::CreateGradientTexture(
					chif::texturehelper::GradientType::Circular,
					64, 64,
					XMFLOAT2(0.5f, 0.5f), XMFLOAT2(0.5f, 0),
					chif::texturehelper::GradientFlags::Smoothstep | chif::texturehelper::GradientFlags::Inverse
				);
				Texture gradientTexBC;
				TextureDesc desc = gradientTex.GetDesc();
				desc.format = Format::BC4_UNORM;
				desc.swizzle = { chif::graphics::ComponentSwizzle::ONE,chif::graphics::ComponentSwizzle::ONE,chif::graphics::ComponentSwizzle::ONE,chif::graphics::ComponentSwizzle::R };
				bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
				assert(success);
				chif::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
			...
			{
				Texture gradientTex = chif::texturehelper::CreateLensDistortionNormalMap(
					32, 32
				);
				Texture gradientTexBC;
				TextureDesc desc = gradientTex.GetDesc();
				desc.format = Format::BC5_UNORM;
				desc.swizzle = { chif::graphics::ComponentSwizzle::R,chif::graphics::ComponentSwizzle::G,chif::graphics::ComponentSwizzle::ONE,chif::graphics::ComponentSwizzle::ONE };
				bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
				assert(success);
				chif::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
	...
	void Scene::RunSoundUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::audio::SoundInstance3D instance3D;
		...
		for (size_t i = 0; i < sounds.GetCount(); ++i)
		{
			...
			if (!sound.IsDisable3D())
			{
				...
				if (transform != nullptr)
				{
					...
					chif::audio::Update3D(&sound.soundinstance, instance3D);
				}
			}
			if (sound.IsPlaying())
			{
				chif::audio::Play(&sound.soundinstance);
			}
			else
			{
				chif::audio::Stop(&sound.soundinstance);
			}
			chif::audio::SetVolume(sound.volume, &sound.soundinstance);
		}
	}
	void Scene::RunVideoUpdateSystem(chif::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < videos.GetCount(); ++i)
		{
			...
			if (video.IsPlaying())
			{
				video.videoinstance.flags |= chif::video::VideoInstance::Flags::Playing;
			}
			else
			{
				video.videoinstance.flags &= ~chif::video::VideoInstance::Flags::Playing;
			...
			if (video.IsLooped())
			{
				video.videoinstance.flags |= chif::video::VideoInstance::Flags::Looped;
			}
			else
			{
				video.videoinstance.flags &= ~chif::video::VideoInstance::Flags::Looped;
			...
			video.videoinstance.flags |= chif::video::VideoInstance::Flags::Mipmapped;

		}
	}
	void Scene::RunScriptUpdateSystem(chif::jobsystem::context& ctx)
	{
		if (dt == 0)
			return; // not allowed to be run when dt == 0 as it could be on separate thread!
		auto range = chif::profiler::BeginRangeCPU("Script Components");
		for (size_t i = 0; i < scripts.GetCount(); ++i)
		{
			...
			if (script.IsPlaying())
			{
				if (script.resource.IsValid() && (script.script.empty() || script.script_hash != script.resource.GetScriptHash()))
				{
					...
					chif::lua::AttachScriptParameters(str, script.filename, chif::lua::GeneratePID(), "local function GetEntity() return " + std::to_string(entity) + "; end;", "");
					chif::lua::CompileText(str, script.script);
				}
				if (!script.script.empty())
				{
					chif::lua::RunBinaryData(script.script.data(), script.script.size(), script.filename.c_str());
				}
	...
	void Scene::RunSpriteUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)sprites.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunFontUpdateSystem(chif::jobsystem::context& ctx)
	{
		chif::jobsystem::Dispatch(ctx, (uint32_t)fonts.GetCount(), small_subtask_groupsize, [&](chif::jobsystem::JobArgs args) {
	...
	void Scene::RunCharacterUpdateSystem(chif::jobsystem::context& ctx)
	{
		if (dt == 0)
			return;
		...
		chif::jobsystem::Dispatch(ctx, (uint32_t)characters.GetCount(), 1, [&](chif::jobsystem::JobArgs args) {
			...
			if (character.IsActive())
			{
				...
				// Smooth facing:
				character.facing = chif::math::Lerp(character.facing, character.facing_next, dt * 5);
				...
				if (character.pathfinding_thread)
				{
					...
					if (character.process_goal && character.voxelgrid != nullptr && !chif::jobsystem::IsBusy(character.pathfinding_thread->ctx))
					{
						character.process_goal = false;
						character.pathfinding_thread->ctx.priority = chif::jobsystem::Priority::Low;
						chif::jobsystem::Execute(character.pathfinding_thread->ctx, [&](chif::jobsystem::JobArgs args) {
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			...
			for (size_t objectIndex = 0; objectIndex < objectCount; ++objectIndex)
			{
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex)
				{
					...
					float distance;
					XMFLOAT2 bary;
					if (chif::math::RayTriangleIntersects(rayOrigin_local, rayDirection_local, p0, p1, p2, distance, bary))
					{
						...
						distance = chif::math::Distance(pos, rayOrigin);
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex, bool doubleSided)
				{
					...
					else
					{
						...
						else
						{
							...
							// Edge 0,1
							XMVECTOR Point1 = chif::math::ClosestPointOnLineSegment(p0, p1, LinePlaneIntersection);

							// Edge 1,2
							XMVECTOR Point2 = chif::math::ClosestPointOnLineSegment(p1, p2, LinePlaneIntersection);

							// Edge 2,0
							XMVECTOR Point3 = chif::math::ClosestPointOnLineSegment(p2, p0, LinePlaneIntersection);
					...
					XMVECTOR Center = chif::math::ClosestPointOnLineSegment(A, B, ReferencePoint);
					...
					// Edge 0,1
					XMVECTOR Point1 = chif::math::ClosestPointOnLineSegment(p0, p1, Center);
					...
					// Edge 1,2
					XMVECTOR Point2 = chif::math::ClosestPointOnLineSegment(p1, p2, Center);
					...
					// Edge 2,0
					XMVECTOR Point3 = chif::math::ClosestPointOnLineSegment(p2, p0, Center);
	...
	void Scene::VoxelizeObject(size_t objectIndex, chif::VoxelGrid& grid, bool subtract, uint32_t lod)
	{
		if (objectIndex >= objects.GetCount() || objectIndex >= aabb_objects.size())
			return;
		if (aabb_objects[objectIndex].intersects(grid.get_aabb()) == chif::primitive::AABB::OUTSIDE)
			return;
		const ObjectComponent& object = objects[objectIndex];
		const MeshComponent* mesh = meshes.GetComponent(object.meshID);
		if (mesh == nullptr)
	...
	void Scene::VoxelizeScene(chif::VoxelGrid& voxelgrid, bool subtract, uint32_t filterMask, uint32_t layerMask, uint32_t lod)
	{
		chif::jobsystem::context ctx;
		if ((filterMask & FILTER_COLLIDER))
		{
			for (size_t i = 0; i < collider_count_cpu; ++i)
			{
				...
				case ColliderComponent::Shape::Sphere:
				{
					...
					chif::jobsystem::Execute(ctx, [&voxelgrid, subtract, sphere](chif::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Capsule:
				{
					...
					// TODO: fix heap allocating lambda capture!
					chif::jobsystem::Execute(ctx, [&voxelgrid, subtract, capsule](chif::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Plane:
				{
					...
					// TODO: fix heap allocating lambda capture!
					chif::jobsystem::Execute(ctx, [&voxelgrid, subtract, P0, P1, P2, P3](chif::jobsystem::JobArgs args) {
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			for (size_t i = 0; i < objects.GetCount(); ++i)
			{
				...
				if ((layerMask & aabb.layerMask) == 0)
					continue;
				// TODO: fix heap allocating lambda capture!
				chif::jobsystem::Execute(ctx, [this, &voxelgrid, subtract, lod, i](chif::jobsystem::JobArgs args) {
	...
	void Scene::PutWaterRipple(const std::string& image, const XMFLOAT3& pos)
	{
		...
		img.params.pos = pos;
		img.params.rotation = (chif::random::GetRandom(0, 1000) * 0.001f) * 2 * 3.1415f;
		img.params.siz = XMFLOAT2(1, 1);
		img.params.quality = chif::image::QUALITY_ANISOTROPIC;
	...
	{
		chif::Sprite img;
		img.textureResource.SetTexture(*chif::texturehelper::getWaterRipple());
		img.params.enableExtractNormalMap();
		img.params.blendFlag = BLENDMODE_ADDITIVE;
		img.anim.fad = 0.01f;
		img.anim.scaleX = 0.1f;
	...
	XMVECTOR SkinVertex(const MeshComponent& mesh, const chif::vector<ShaderTransform>& boneData, uint32_t index, XMVECTOR* N)
	{
	...
	{
		chif::Archive archive(fileName, true);
		if (!archive.IsOpen())
			return;
	...
	PickResult Pick(const chif::primitive::Ray& ray, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(ray, filterMask, layerMask, lod);
	}
	SceneIntersectSphereResult SceneIntersectSphere(const chif::primitive::Sphere& sphere, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(sphere, filterMask, layerMask, lod);
	}
	SceneIntersectCapsuleResult SceneIntersectCapsule(const chif::primitive::Capsule& capsule, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
	...
	XMMATRIX Scene::GetRestPose(chif::ecs::Entity entity) const
	{
	...
	void Scene::ScanAnimationDependencies()
	{
		...
		chif::jobsystem::Execute(animation_dependency_scan_workload, [&](chif::jobsystem::JobArgs args) {
			auto range = chif::profiler::BeginRangeCPU("Animation Dependencies");
	...
	{
		chif::jobsystem::Execute(spring_dependency_scan_workload, [this](chif::jobsystem::JobArgs args){
			auto range = chif::profiler::BeginRangeCPU("Spring Dependencies");
			spring_queues.clear();
	...
	void Scene::UpdateSpringsTopDownRecursive(SpringComponent* parent_spring, SpringComponent& spring)
	{
		...
		chif::primitive::Sphere tail_sphere;
...
#if 0
		// Debug axis:
		static chif::SpinLock dbglocker;
		chif::renderer::RenderableLine line;
		line.color_start = line.color_end = XMFLOAT4(1, 1, 0, 1);
	...
	void Scene::UpdateHumanoidFacings()
	{
		...
		for (size_t i = 0; i < humanoids.GetCount(); ++i)
		{
			...
			if (humanoid.knee_bending == 0)
			{
				...
				if (transform != nullptr)
				{
					XMFLOAT3 roll_pitch_yaw = chif::math::QuaternionToRollPitchYaw(transform->rotation_local);
