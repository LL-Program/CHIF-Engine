...
namespace vulkan_internal
{
	...
	//	These are also defined in chif::shadercompiler.cpp as hard coded compiler arguments for SPIRV, so they need to be the same
	enum
	{
	...
	constexpr VkStencilOp _ConvertStencilOp(StencilOp value)
	{
		switch (value)
		{
		case chif::graphics::StencilOp::KEEP:
			return VK_STENCIL_OP_KEEP;
		case chif::graphics::StencilOp::ZERO:
			return VK_STENCIL_OP_ZERO;
		case chif::graphics::StencilOp::REPLACE:
			return VK_STENCIL_OP_REPLACE;
		case chif::graphics::StencilOp::INCR_SAT:
			return VK_STENCIL_OP_INCREMENT_AND_CLAMP;
		case chif::graphics::StencilOp::DECR_SAT:
			return VK_STENCIL_OP_DECREMENT_AND_CLAMP;
		case chif::graphics::StencilOp::INVERT:
			return VK_STENCIL_OP_INVERT;
		case chif::graphics::StencilOp::INCR:
			return VK_STENCIL_OP_INCREMENT_AND_WRAP;
		case chif::graphics::StencilOp::DECR:
	...
	constexpr VkImageAspectFlags _ConvertImageAspect(ImageAspect value)
	{
		...
		{
		default:
		case chif::graphics::ImageAspect::COLOR:
			return VK_IMAGE_ASPECT_COLOR_BIT;
		case chif::graphics::ImageAspect::DEPTH:
			return VK_IMAGE_ASPECT_DEPTH_BIT;
		case chif::graphics::ImageAspect::STENCIL:
			return VK_IMAGE_ASPECT_STENCIL_BIT;
		case chif::graphics::ImageAspect::LUMINANCE:
			return VK_IMAGE_ASPECT_PLANE_0_BIT;
		case chif::graphics::ImageAspect::CHROMINANCE:
	...
	constexpr VkComponentSwizzle _ConvertComponentSwizzle(ComponentSwizzle value)
	{
		...
		case chif::graphics::ComponentSwizzle::R:
			return VK_COMPONENT_SWIZZLE_R;
		case chif::graphics::ComponentSwizzle::G:
			return VK_COMPONENT_SWIZZLE_G;
		case chif::graphics::ComponentSwizzle::B:
			return VK_COMPONENT_SWIZZLE_B;
		case chif::graphics::ComponentSwizzle::A:
			return VK_COMPONENT_SWIZZLE_A;
		case chif::graphics::ComponentSwizzle::ZERO:
			return VK_COMPONENT_SWIZZLE_ZERO;
		case chif::graphics::ComponentSwizzle::ONE:
	...
	bool checkExtensionSupport(const char* checkExtension, const chif::vector<VkExtensionProperties>& available_extensions)
	{
	...
	bool ValidateLayers(const chif::vector<const char*>& required,
		const chif::vector<VkLayerProperties>& available)
	{
		...
		void* user_data)
	{
		...
		if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
		{
            ...
            ss += "\n";
			chif::helper::DebugOut(ss, chif::helper::DebugLevel::Warning);
		}
		else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
		{
			...
			ss += callback_data->pMessage;
            ss += "\n";
			chif::helper::DebugOut(ss, chif::helper::DebugLevel::Error);
		}
	...
	{
		return chif::helper::GetCurrentPath() + "/pso_cache_vulkan";
	}
	...
	struct Buffer_Vulkan
	{
		...
		chif::vector<BufferSubresource> subresources_srv;
		chif::vector<BufferSubresource> subresources_uav;
	...
	struct Texture_Vulkan
	{
		...
		chif::vector<TextureSubresource> subresources_srv;
		chif::vector<TextureSubresource> subresources_uav;
		chif::vector<TextureSubresource> subresources_rtv;
		chif::vector<TextureSubresource> subresources_dsv;

		chif::vector<SubresourceData> mapped_subresources;
	...
	struct Shader_Vulkan
	{
		...
		chif::vector<VkDescriptorSetLayoutBinding> layoutBindings;
		chif::vector<VkImageViewType> imageViewTypes;

		chif::vector<BindingUsage> bindlessBindings;
		chif::vector<VkDescriptorSet> bindlessSets;
		uint32_t bindlessFirstSet = 0;

		VkPushConstantRange pushconstants = {};

		VkDeviceSize uniform_buffer_sizes[DESCRIPTORBINDER_CBV_COUNT] = {};
		chif::vector<uint32_t> uniform_buffer_dynamic_slots;
	...
	struct BVH_Vulkan
	{
		...
		chif::vector<VkAccelerationStructureGeometryKHR> geometries;
		chif::vector<uint32_t> primitiveCounts;
	...
	struct SwapChain_Vulkan
	{
		...
		chif::vector<VkImage> swapChainImages;
		chif::vector<VkImageView> swapChainImageViews;
		...
		chif::vector<VkSemaphore> swapchainAcquireSemaphores;
	...
	struct VideoDecoder_Vulkan
	{
		...
		chif::vector<VmaAllocation> allocations;
	...
	)
	{
		...
		chif::vector<VkSurfaceFormatKHR> swapchain_formats(formatCount);
		...
		chif::vector<VkPresentModeKHR> swapchain_presentModes(presentModeCount);
...
using namespace vulkan_internal;
	...
	GraphicsDevice_Vulkan::CopyAllocator::CopyCMD GraphicsDevice_Vulkan::CopyAllocator::allocate(uint64_t staging_size)
	{
		...
		if (!cmd.IsValid())
		{
			...
			uploaddesc.size = chif::math::GetNextPowerOfTwo(staging_size);
	...
	void GraphicsDevice_Vulkan::pso_validate(CommandList cmd)
	{
		...
		if (it == pipelines_global.end())
		{
			...
			if (pipeline == VK_NULL_HANDLE)
			{
				...
				chif::vector<VkVertexInputBindingDescription> bindings;
				chif::vector<VkVertexInputAttributeDescription> attributes;
	...
	// Engine functions
	GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(chif::platform::window_type window, ValidationMode validationMode_, GPUPreference preference)
	{
		chif::Timer timer;
		...
		chif::unordered_map<uint32_t, std::shared_ptr<std::mutex>> queue_lockers;
		...
		if (res != VK_SUCCESS)
		{
			chif::helper::messageBox("volkInitialize failed! ERROR: " + std::to_string(res), "Error!");
			chif::platform::Exit();
		}
		...
		appInfo.engineVersion = VK_MAKE_VERSION(chif::version::GetMajor(), chif::version::GetMinor(), chif::version::GetRevision());
		...
		assert(res == VK_SUCCESS);
		chif::vector<VkLayerProperties> availableInstanceLayers(instanceLayerCount);
		...
		assert(res == VK_SUCCESS);
		chif::vector<VkExtensionProperties> availableInstanceExtensions(extensionCount);
		res = vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, availableInstanceExtensions.data());
		assert(res == VK_SUCCESS);

		chif::vector<const char*> instanceLayers;
		chif::vector<const char*> instanceExtensions;
...
#elif SDL2
		{
			...
			chif::vector<const char *> extensionNames_sdl(extensionCount);
		...
		if (validationMode != ValidationMode::Disabled)
		{
			...
			static const chif::vector<const char*> validationLayerPriorityList[] =
			{
		...
		// Create instance:
		{
			...
			if (res != VK_SUCCESS)
			{
				chif::helper::messageBox("vkCreateInstance failed! ERROR: " + std::to_string(res), "Error!");
				chif::platform::Exit();
			}
		...
		// Enumerating and creating devices:
		{
			...
			{
				assert(0);
				chif::helper::messageBox("Failed to find GPU with Vulkan support!");
				chif::platform::Exit();
			}

			chif::vector<VkPhysicalDevice> devices(deviceCount);
			res = vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data());
			assert(res == VK_SUCCESS);

			const chif::vector<const char*> required_deviceExtensions = {
				VK_KHR_SWAPCHAIN_EXTENSION_NAME,
			};
			chif::vector<const char*> enabled_deviceExtensions;
			...
			auto checkPhysicalDeviceAndFillProperties2 = [&](VkPhysicalDevice dev) {
				...
				assert(res == VK_SUCCESS);
				chif::vector<VkExtensionProperties> available_deviceExtensions(extensionCount);
			...
			{
				assert(0);
				chif::helper::messageBox("Failed to find a suitable GPU!");
				chif::platform::Exit();
			}
			...
			chif::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
			chif::unordered_set<uint32_t> uniqueQueueFamilies = { graphicsFamily,copyFamily,computeFamily };
			if (videoFamily != VK_QUEUE_FAMILY_IGNORED)
			{
			...
			if (res != VK_SUCCESS)
			{
				chif::helper::messageBox("vkCreateDevice failed! ERROR: " + std::to_string(res), "Error!");
				chif::platform::Exit();
			}
		...
		if (res != VK_SUCCESS)
		{
			chif::helper::messageBox("vmaCreateAllocator failed! ERROR: " + std::to_string(res), "Error!");
			chif::platform::Exit();
		}
		...
		if (res != VK_SUCCESS)
		{
			chif::helper::messageBox("Failed to create Vulkan external memory allocator, ERROR: " + std::to_string(res), "Error!");
			chif::platform::Exit();
		}
		...
		for (uint32_t fr = 0; fr < BUFFERCOUNT; ++fr)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (res != VK_SUCCESS)
				{
					chif::helper::messageBox("vkCreateFence[FRAME] failed! ERROR: " + std::to_string(res), "Error!");
					chif::platform::Exit();
				}
			}
		}
		...
		// Pipeline Cache
		{
			...
			chif::vector<uint8_t> pipelineData;
			if (!chif::helper::FileRead(get_shader_cache_path(), pipelineData))
			{
				pipelineData.clear();
			}
		...
		chif::backlog::post("Created GraphicsDevice_Vulkan (" + std::to_string((int)std::round(timer.elapsed())) + " ms)\nAdapter: " + adapterName);
	}
	GraphicsDevice_Vulkan::~GraphicsDevice_Vulkan()
	{
		...
		if (pipelineCache != VK_NULL_HANDLE)
		{
			...
			chif::vector<uint8_t> data(size);
			res = vkGetPipelineCacheData(device, pipelineCache, &size, data.data());
			assert(res == VK_SUCCESS);

			// Write pipeline cache data to a file in binary format
			chif::helper::FileWrite(get_shader_cache_path(), data.data(), size);
	...
	bool GraphicsDevice_Vulkan::CreateSwapChain(const SwapChainDesc* desc, chif::platform::window_type window, SwapChain* swapchain) const
	{
...
#else
#error CHIFENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
#endif // _WIN32
		}
	...
	bool GraphicsDevice_Vulkan::CreateTexture(const TextureDesc* desc, const SubresourceData* initial_data, Texture* texture, const GPUResource* alias, uint64_t alias_offset) const
	{
			...
			)
		{
			...
			chif::vector<VkVideoFormatPropertiesKHR> video_format_properties(format_property_count);
		...
		if (has_flag(texture->desc.misc_flags, ResourceMiscFlag::SPARSE))
		{
			...
			chif::vector<VkSparseImageMemoryRequirements> sparse_requirements(sparse_requirement_count);
		...
		if (initial_data != nullptr)
		{
			...
			chif::vector<VkBufferImageCopy> copyRegions;
	...
	bool GraphicsDevice_Vulkan::CreateShader(ShaderStage stage, const void* shadercode, size_t shadercode_size, Shader* shader) const
	{
...

		{
			...
			chif::vector<SpvReflectDescriptorBinding*> bindings(binding_count);
			result = spvReflectEnumerateDescriptorBindings(
			...
			chif::vector<SpvReflectBlockVariable*> pushconstants(push_count);
			...
			if (stage == ShaderStage::CS || stage == ShaderStage::LIB)
			{
				...
				{
					chif::helper::hash_combine(internal_state->binding_hash, x.binding);
					chif::helper::hash_combine(internal_state->binding_hash, x.descriptorCount);
					chif::helper::hash_combine(internal_state->binding_hash, x.descriptorType);
					chif::helper::hash_combine(internal_state->binding_hash, x.stageFlags);
					chif::helper::hash_combine(internal_state->binding_hash, internal_state->imageViewTypes[i++]);
				}
				for (auto& x : internal_state->bindlessBindings)
				{
					chif::helper::hash_combine(internal_state->binding_hash, x.used);
					chif::helper::hash_combine(internal_state->binding_hash, x.binding.binding);
					chif::helper::hash_combine(internal_state->binding_hash, x.binding.descriptorCount);
					chif::helper::hash_combine(internal_state->binding_hash, x.binding.descriptorType);
					chif::helper::hash_combine(internal_state->binding_hash, x.binding.stageFlags);
				}
				chif::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.offset);
				chif::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.size);
				chif::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.stageFlags);

				pso_layout_cache_mutex.lock();
				if (pso_layout_cache[internal_state->binding_hash].pipelineLayout == VK_NULL_HANDLE)
				{
					chif::vector<VkDescriptorSetLayout> layouts;
	...
	bool GraphicsDevice_Vulkan::CreatePipelineState(const PipelineStateDesc* desc, PipelineState* pso, const RenderPassInfo* renderpass_info) const
	{
		...
		chif::helper::hash_combine(internal_state->hash, desc->ms);
		chif::helper::hash_combine(internal_state->hash, desc->as);
		chif::helper::hash_combine(internal_state->hash, desc->vs);
		chif::helper::hash_combine(internal_state->hash, desc->ps);
		chif::helper::hash_combine(internal_state->hash, desc->hs);
		chif::helper::hash_combine(internal_state->hash, desc->ds);
		chif::helper::hash_combine(internal_state->hash, desc->gs);
		chif::helper::hash_combine(internal_state->hash, desc->il);
		chif::helper::hash_combine(internal_state->hash, desc->rs);
		chif::helper::hash_combine(internal_state->hash, desc->bs);
		chif::helper::hash_combine(internal_state->hash, desc->dss);
		chif::helper::hash_combine(internal_state->hash, desc->pt);
		chif::helper::hash_combine(internal_state->hash, desc->sample_mask);
	...
	bool GraphicsDevice_Vulkan::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		chif::vector<VkPipelineShaderStageCreateInfo> stages;
		...
		chif::vector<VkRayTracingShaderGroupCreateInfoKHR> groups;
	...
	bool GraphicsDevice_Vulkan::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		chif::vector<StdVideoH264PictureParameterSet> pps_array_h264(desc->pps_count);
		chif::vector<StdVideoH264ScalingLists> scalinglist_array_h264(desc->pps_count);
		...
		chif::vector<StdVideoH264SequenceParameterSet> sps_array_h264(desc->sps_count);
		chif::vector<StdVideoH264SequenceParameterSetVui> vui_array_h264(desc->sps_count);
		chif::vector<StdVideoH264HrdParameters> hrd_array_h264(desc->sps_count);
		...
		chif::vector<VkVideoSessionMemoryRequirementsKHR> requirements(requirement_count);
		...
		chif::vector<VkBindVideoSessionMemoryInfoKHR> bind_session_memory_infos(requirement_count);
	...
	CommandList GraphicsDevice_Vulkan::BeginCommandList(QUEUE_TYPE queue)
	{
		...
		if (commandlist.GetCommandBuffer() == VK_NULL_HANDLE)
		{
			...
			for (uint32_t buffer = 0; buffer < BUFFERCOUNT; ++buffer)
			{
				...
				switch (queue)
				{
				case chif::graphics::QUEUE_GRAPHICS:
					poolInfo.queueFamilyIndex = graphicsFamily;
					break;
				case chif::graphics::QUEUE_COMPUTE:
					poolInfo.queueFamilyIndex = computeFamily;
					break;
				case chif::graphics::QUEUE_COPY:
					poolInfo.queueFamilyIndex = copyFamily;
					break;
				case chif::graphics::QUEUE_VIDEO_DECODE:
	...
	{
		thread_local chif::vector<VkBindSparseInfo> sparse_infos;
		struct DataPerBind
		{
			...
			chif::vector<VkSparseMemoryBind> memory_binds;
			chif::vector<VkSparseImageMemoryBind> image_memory_binds;
		};
		thread_local chif::vector<DataPerBind> sparse_binds;
	...
	void GraphicsDevice_Vulkan::BindVertexBuffers(const GPUBuffer *const* vertexBuffers, uint32_t slot, uint32_t count, const uint32_t* strides, const uint64_t* offsets, CommandList cmd)
	{
		...
		for (uint32_t i = 0; i < count; ++i)
		{
			chif::helper::hash_combine(hash, strides[i]);
	...
	void GraphicsDevice_Vulkan::BindPipelineState(const PipelineState* pso, CommandList cmd)
	{
		...
		else
		{
			...
			chif::helper::hash_combine(pipeline_hash, internal_state->hash);
			chif::helper::hash_combine(pipeline_hash, commandlist.renderpass_info.get_hash());
