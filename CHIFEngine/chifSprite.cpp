...
namespace chif
{
	...
	Sprite::Sprite(const std::string& newTexture, const std::string& newMask)
	{
		if (!newTexture.empty())
		{
			...
			textureResource = chif::resourcemanager::Load(newTexture);
		}
		if (!newMask.empty())
		{
			maskName = newMask;
			maskResource = chif::resourcemanager::Load(newMask);
	...
	void Sprite::Draw(CommandList cmd) const
	{
		if (IsHidden())
			return;
		chif::image::Draw(GetTexture(), params, cmd);
	}
	...
	void Sprite::Update(float dt)
	{
		...
		if (anim.wobbleAnim.amount.x > 0 || anim.wobbleAnim.amount.y > 0)
		{
			...
			chif::image::Params default_params; // contains corners in idle positions
			for (int i = 0; i < 4; ++i)
			{
	...
	const chif::graphics::Texture* Sprite::GetTexture() const
	{
		return textureResource.IsValid() ? &textureResource.GetTexture() : chif::texturehelper::getWhite();
	}

	void Sprite::Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri)
	{
			...
			if (!textureName.empty() || !maskName.empty())
			{
				chif::jobsystem::Execute(seri.ctx, [&](chif::jobsystem::JobArgs args) {
					if (!textureName.empty())
					{
						...
						textureResource = chif::resourcemanager::Load(textureName);
					}
					if (!maskName.empty())
					{
						maskName = dir + maskName;
						maskResource = chif::resourcemanager::Load(maskName);
					}
				});
			}
		}
		else
		{
			...
			archive << chif::helper::GetPathRelative(dir, maskName);
