...
namespace chif::backlog
{
	...
	float scroll = 0;
	int historyPos = 0;
	chif::font::Params font_params;
	chif::SpinLock logLock;
	Texture backgroundTex;
	bool refitscroll = false;
	chif::gui::TextInputField inputField;
	chif::gui::Button toggleButton;
	chif::gui::GUI GUI;
	...
	void write_logfile()
	{
		static const std::string filename = chif::helper::GetCurrentPath() + "/log.txt";
		std::string text = getText(); // will lock mutex
		chif::helper::FileWrite(filename, (const uint8_t*)text.c_str(), text.length());
	}
	...
	void Update(const chif::Canvas& canvas, float dt)
	{
		if (!locked)
		{
			if (chif::input::Press(chif::input::KEYBOARD_BUTTON_HOME) && !GUI.IsTyping())
			{
				Toggle();
			}
			...
			if (isActive())
			{
				if (chif::input::Press(chif::input::KEYBOARD_BUTTON_UP))
				{
					historyPrev();
				}
				if (chif::input::Press(chif::input::KEYBOARD_BUTTON_DOWN))
				{
					historyNext();
				}
				if (chif::input::Down(chif::input::KEYBOARD_BUTTON_PAGEUP))
				{
					Scroll(1000.0f * dt);
				}
				if (chif::input::Down(chif::input::KEYBOARD_BUTTON_PAGEDOWN))
				{
					Scroll(-1000.0f * dt);
				}

				Scroll(chif::input::GetPointer().z * 20);
				...
				if (!created)
				{
					...
					inputField.OnInputAccepted([](chif::gui::EventArgs args) {
						historyPos = 0;
						post(args.sValue);
						LogEntry entry;
					...
					chif::Color theme_color_idle = chif::Color(30, 40, 60, 200);
					chif::Color theme_color_focus = chif::Color(70, 150, 170, 220);
					chif::Color theme_color_active = chif::Color::White();
					chif::Color theme_color_deactivating = chif::Color::lerp(theme_color_focus, chif::Color::White(), 0.5f);
					inputField.SetColor(theme_color_idle); // all states the same, it's gonna be always active anyway
					inputField.font.params.color = chif::Color(160, 240, 250, 255);
					inputField.font.params.shadowColor = chif::Color::Transparent();

					toggleButton.Create("V");
					toggleButton.OnClick([](chif::gui::EventArgs args) {
						Toggle();
						});
					toggleButton.SetColor(theme_color_idle, chif::gui::IDLE);
					toggleButton.SetColor(theme_color_focus, chif::gui::FOCUS);
					toggleButton.SetColor(theme_color_active, chif::gui::ACTIVE);
					toggleButton.SetColor(theme_color_deactivating, chif::gui::DEACTIVATING);
					toggleButton.SetShadowRadius(5);
					toggleButton.SetShadowColor(chif::Color(80, 140, 180, 100));
					toggleButton.font.params.color = chif::Color(160, 240, 250, 255);
					toggleButton.font.params.rotation = XM_PI;
					toggleButton.font.params.size = 24;
					toggleButton.font.params.scaling = 3;
					toggleButton.font.params.shadowColor = chif::Color::Transparent();
				...
				if (inputField.GetState() != chif::gui::ACTIVE)
				{
		...
		pos = chif::math::Clamp(pos, -canvas.GetLogicalHeight(), 0);
	...
	)
	{
		...
		if (!backgroundTex.IsValid())
		{
			...
			chif::texturehelper::CreateTexture(backgroundTex, colorData, 1, 2);
			device->SetName(&backgroundTex, "chif::backlog::backgroundTex");
		}

		chif::image::Params fx = chif::image::Params((float)canvas.GetLogicalWidth(), (float)canvas.GetLogicalHeight());
		fx.pos = XMFLOAT3(0, pos, 0);
		fx.opacity = chif::math::Lerp(1, 0, -pos / canvas.GetLogicalHeight());
		if (colorspace != ColorSpace::SRGB)
		{
			fx.enableLinearOutputMapping(9);
		}
		chif::image::Draw(&backgroundTex, fx, cmd);

		chif::image::Params inputbg;
		inputbg.color = chif::Color(80, 140, 180, 200);
		...
		chif::image::Draw(nullptr, inputbg, cmd);
	...
	)
	{
		...
		chif::font::SetCanvas(canvas); // always set here as it can be called from outside...
		chif::font::Params params = font_params;
		params.cursor = {};
		if (refitscroll)
		{
			float textheight = chif::font::TextHeight(getTextWithoutLock(), params);
		...
		for (auto& x : entries)
		{
			...
			params.cursor = chif::font::Draw(x.text, params, cmd);
		}
	...
	void post(const char* input, LogLevel level)
	{
		...
		// This is explicitly scoped for scoped_lock!
		{
			...
			default:
			case LogLevel::Default:
				chif::helper::DebugOut(str, chif::helper::DebugLevel::Normal);
				break;
			case LogLevel::Warning:
				chif::helper::DebugOut(str, chif::helper::DebugLevel::Warning);
				break;
			case LogLevel::Error:
				chif::helper::DebugOut(str, chif::helper::DebugLevel::Error);
				break;
			}
