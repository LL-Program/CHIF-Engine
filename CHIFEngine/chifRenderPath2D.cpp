...
namespace chif
{
	...
	void RenderPath2D::Render() const
	{
		...
		chif::image::SetCanvas(*this);
		chif::font::SetCanvas(*this);

		chif::renderer::ProcessDeferredTextureRequests(cmd);

		if (GetGUIBlurredBackground() != nullptr)
		{
			chif::image::SetBackground(*GetGUIBlurredBackground());
		}
			...
			for (auto& x : layers)
			{
				for (auto& y : x.items)
				{
					if (y.type == RenderItem2D::TYPE::SPRITE &&
						y.sprite != nullptr &&
						y.sprite->params.stencilComp != chif::image::STENCILMODE_DISABLED)
					{
						y.sprite->Draw(cmd);
					}
				}
		...
		if (GetDepthStencil() != nullptr)
		{
			if (rtStenciled.IsValid())
			{
				...
				if (rtStenciled.GetDesc().sample_count > 1)
				{
					chif::image::Draw(&rtStenciled_resolved, fx, cmd);
				}
				else
				{
					chif::image::Draw(&rtStenciled, fx, cmd);
				}
				device->EventEnd(cmd);
			}
			else
		...
		for (auto& x : layers)
		{
			for (auto& y : x.items)
			{
				...
				default:
				case RenderItem2D::TYPE::SPRITE:
					if (y.sprite != nullptr && y.sprite->params.stencilComp == chif::image::STENCILMODE_DISABLED)
					{
						y.sprite->Draw(cmd);
					}
					break;
	...
	void RenderPath2D::Compose(CommandList cmd) const
	{
		...
		chif::image::Draw(&GetRenderResult(), fx, cmd);

		RenderPath::Compose(cmd);
	}


	void RenderPath2D::AddSprite(chif::Sprite* sprite, const std::string& layer)
	{
		for (auto& x : layers)
		{
	...
	void RenderPath2D::RemoveSprite(chif::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetSpriteOrder(chif::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::AddFont(chif::SpriteFont* font, const std::string& layer)
	{
		for (auto& x : layers)
		{
			if (!x.name.compare(layer))
	...
	void RenderPath2D::RemoveFont(chif::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetFontOrder(chif::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::SetSpriteOrder(chif::Sprite* sprite, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::SetFontOrder(chif::SpriteFont* font, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::CleanLayers()
	{
		for (auto& x : layers)
		{
			...
			chif::vector<RenderItem2D> itemsToRetain(0);
			for (auto& y : x.items)
			{
