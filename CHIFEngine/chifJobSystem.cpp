...
#ifdef PLATFORM_PS5
#include "chifJobSystem_PS5.h"
#endif // PLATFORM_PS5

namespace chif::jobsystem
{
	struct Job
	{
	...
	struct PriorityResources
	{
		...
		chif::vector<std::thread> threads;
		...
		for (int prio = 0; prio < int(Priority::Count); ++prio)
		{
			...
			for (uint32_t threadID = 0; threadID < res.numThreads; ++threadID)
			{
				...
				if (priority == Priority::High)
				{
					...
					std::wstring wthreadname = L"chif::job_" + std::to_wstring(threadID);
				...
				else if (priority == Priority::Low)
				{
					...
					std::wstring wthreadname = L"chif::job_lo_" + std::to_wstring(threadID);
					HRESULT hr = SetThreadDescription(handle, wthreadname.c_str());
					assert(SUCCEEDED(hr));
				}
				else if (priority == Priority::Streaming)
				{
					...
					std::wstring wthreadname = L"chif::job_st_" + std::to_wstring(threadID);
					HRESULT hr = SetThreadDescription(handle, wthreadname.c_str());
					assert(SUCCEEDED(hr));
				}
...
#define handle_error_en(en, msg) \
               do { errno = en; perror(msg); } while (0)
				...
				if (priority == Priority::High)
				{
					std::string thread_name = "chif::job_" + std::to_string(threadID);
				...
				{
					std::string thread_name = "chif::job_lo_" + std::to_string(threadID);
					ret = pthread_setname_np(handle, thread_name.c_str());
					if (ret != 0)
						handle_error_en(ret, std::string(" pthread_setname_np[" + std::to_string(threadID) + ']').c_str());
				...
				{
					std::string thread_name = "chif::job_st_" + std::to_string(threadID);
					ret = pthread_setname_np(handle, thread_name.c_str());
					if (ret != 0)
						handle_error_en(ret, std::string(" pthread_setname_np[" + std::to_string(threadID) + ']').c_str());
					// priority is set in the worker function
