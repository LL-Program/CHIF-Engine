...
namespace chif::video
{
			...
			for (uint32_t ntrack = 0; ntrack < mp4.track_count; ntrack++)
			{
				...
				if (track.handler_type == MP4D_HANDLER_TYPE_VIDE)
				{
					...
					case MP4_OBJECT_TYPE_HEVC:
						chif::helper::messageBox("H265 (HEVC) video format is not supported yet!", "Error!");
						return false;
					default:
						chif::helper::messageBox("Unknown video format!", "Error!");
						return false;
					}
...
#ifdef DEBUG_DUMP_H264
					chif::helper::FileWrite("dump.h264", dump.data(), dump.size());
#endif // DEBUG_DUMP_H264
					...
					assert(success);
					device->SetName(&video->data_stream, "chif::Video::data_stream");
				}
				else if (track.handler_type == MP4D_HANDLER_TYPE_SOUN)
				{
					chif::backlog::post("Audio from video file is not implemented yet!");
				}
			}
		}
		else
		{
			chif::helper::messageBox("MP4 parsing failure!", "Error!");
			return false;
		}
		MP4D_close(&mp4);
		return success;
	...
	void ResolveVideoToRGB(VideoInstance* instance, CommandList cmd)
	{
		...
		if (has_flag(instance->flags, VideoInstance::Flags::Mipmapped))
		{
			chif::renderer::GenerateMipChain(output.texture, chif::renderer::MIPGENFILTER_LINEAR, cmd);
		}
