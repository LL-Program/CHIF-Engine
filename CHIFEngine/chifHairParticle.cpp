...
namespace chif
{
	...
	void HairParticleSystem::CreateFromMesh(const chif::scene::MeshComponent& mesh)
	{
	...
	void HairParticleSystem::CreateRenderData()
	{
		...
		if (particleCount > 0)
		{
			...
			primitiveBuffer = chif::renderer::GetIndexBufferForQuads(particleCount);
		}
		...
		if (!vertex_lengths.empty())
		{
			auto pack_lengths = [&](void* dest) {
				...
				{
					uint8_t packed = uint8_t(chif::math::Clamp(vertex_lengths[i], 0, 1) * 255.0f);
	...
	)
	{
		...
		for (uint32_t i = 0; i < itemCount; ++i)
		{
			...
			chif::renderer::PushBarrier(GPUBarrier::Buffer(&hair.constantBuffer, ResourceState::COPY_DST, ResourceState::CONSTANT_BUFFER));
			...
			chif::renderer::PushBarrier(GPUBarrier::Buffer(&hair.generalBuffer, ResourceState::COPY_DST, ResourceState::UNORDERED_ACCESS));
		...
		for (uint32_t i = 0; i < itemCount; ++i)
		{
			...
			chif::renderer::PushBarrier(GPUBarrier::Buffer(&hair.generalBuffer, ResourceState::UNORDERED_ACCESS, ResourceState::INDIRECT_ARGUMENT | ResourceState::INDEX_BUFFER | ResourceState::SHADER_RESOURCE));
		}
	...
	void HairParticleSystem::InitializeGPUDataIfNeeded(chif::graphics::CommandList cmd)
	{
	...
	void HairParticleSystem::Draw(const MaterialComponent& material, chif::enums::RENDERPASS renderPass, CommandList cmd) const
	{
	...
	void HairParticleSystem::Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri)
	{
	...
	namespace HairParticleSystem_Internal
	{
		void LoadShaders()
		{
			chif::renderer::LoadShader(ShaderStage::VS, vs, "hairparticleVS.cso");

			chif::renderer::LoadShader(ShaderStage::PS, ps_simple, "hairparticlePS_simple.cso");
			chif::renderer::LoadShader(ShaderStage::PS, ps_prepass, "hairparticlePS_prepass.cso");
			chif::renderer::LoadShader(ShaderStage::PS, ps_prepass_depthonly, "hairparticlePS_prepass_depthonly.cso");
			chif::renderer::LoadShader(ShaderStage::PS, ps_shadow, "hairparticlePS_shadow.cso");
			chif::renderer::LoadShader(ShaderStage::PS, ps, "hairparticlePS.cso");

			chif::renderer::LoadShader(ShaderStage::CS, cs_simulate, "hairparticle_simulateCS.cso");
	...
	void HairParticleSystem::Initialize()
	{
		...
		static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { HairParticleSystem_Internal::LoadShaders(); });
		HairParticleSystem_Internal::LoadShaders();

		chif::backlog::post("chif::HairParticleSystem Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
