...
#ifdef PLATFORM_PS5
#include "chifInput_PS5.h"
#endif // PLATFORM_PS5

namespace chif::input
{
#ifdef _WIN32
...
#define KEY_UP(vk_code) (!KEY_DOWN(vk_code))
	...
	chif::platform::window_type window = nullptr;
	chif::Canvas canvas;
	...
	chif::vector<Controller> controllers;
	...
	void Initialize()
	{
		...
		chif::input::rawinput::Initialize();
		chif::input::sdlinput::Initialize();

#ifdef PLATFORM_PS5
		chif::input::ps5::Initialize();
#endif // PLATFORM_PS5
		...
		chif::backlog::post("chif::input Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		initialized.store(true);
	}

	void Update(chif::platform::window_type _window, chif::Canvas _canvas)
	{
		window = _window;
		canvas = _canvas;
		if (!initialized.load())
		...
		auto range = chif::profiler::BeginRangeCPU("Input");

		chif::input::xinput::Update();
		chif::input::rawinput::Update();
		chif::input::sdlinput::Update();

#ifdef PLATFORM_PS5
		chif::input::ps5::Update();
#endif // PLATFORM_PS5

#if defined(_WIN32) && !defined(PLATFORM_XBOX)
		chif::input::rawinput::GetMouseState(&mouse); // currently only the relative data can be used from this
		chif::input::rawinput::GetKeyboardState(&keyboard); 
...
#elif SDL2
		chif::input::sdlinput::GetMouseState(&mouse);
		mouse.position.x /= canvas.GetDPIScaling();
		mouse.position.y /= canvas.GetDPIScaling();
		chif::input::sdlinput::GetKeyboardState(&keyboard);
		//TODO controllers
		//TODO touch
#endif
		...
		for (int i = 0; i < chif::input::xinput::GetMaxControllerCount(); ++i)
		{
			if (chif::input::xinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
		...
		for (int i = 0; i < chif::input::rawinput::GetMaxControllerCount(); ++i)
		{
			if (chif::input::rawinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (int i = 0; i < chif::input::sdlinput::GetMaxControllerCount(); ++i)
		{
			if (chif::input::sdlinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (int i = 0; i < chif::input::ps5::GetMaxControllerCount(); ++i)
		{
			if (chif::input::ps5::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (auto& controller : controllers)
		{
			...
			switch (controller.deviceType)
			{
				case Controller::XINPUT:
					connected = chif::input::xinput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
				case Controller::RAWINPUT:
					connected = chif::input::rawinput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
				case Controller::SDLINPUT:
					connected = chif::input::sdlinput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
#ifdef PLATFORM_PS5
				case Controller::PS5:
					connected = chif::input::ps5::GetControllerState(&controller.state, controller.deviceIndex);
					break;
#endif // PLATFORM_PS5
		...
		if (Press(MOUSE_BUTTON_LEFT))
		{
			...
			if (elapsed < 0.5 && chif::math::Distance(doubleclick_prevpos, pos) < 5)
			{
				double_click = true;
			}
	...
	bool Down(BUTTON button, int playerindex)
	{
		...
		else if (playerindex == 0) // keyboard or mouse
		{
			...
			switch (button)
			{
			case chif::input::MOUSE_BUTTON_LEFT:
				if (mouse.left_button_press) 
					return true;
				return false;
			case chif::input::MOUSE_BUTTON_RIGHT:
				if (mouse.right_button_press) 
					return true;
				return false;
			case chif::input::MOUSE_BUTTON_MIDDLE:
				if (mouse.middle_button_press) 
					return true;
				return false;
#ifdef _WIN32
			case chif::input::KEYBOARD_BUTTON_UP:
				keycode = VK_UP;
				break;
			case chif::input::KEYBOARD_BUTTON_DOWN:
				keycode = VK_DOWN;
				break;
			case chif::input::KEYBOARD_BUTTON_LEFT:
				keycode = VK_LEFT;
				break;
			case chif::input::KEYBOARD_BUTTON_RIGHT:
				keycode = VK_RIGHT;
				break;
			case chif::input::KEYBOARD_BUTTON_SPACE:
				keycode = VK_SPACE;
				break;
			case chif::input::KEYBOARD_BUTTON_RSHIFT:
				keycode = VK_RSHIFT;
				break;
			case chif::input::KEYBOARD_BUTTON_LSHIFT:
				keycode = VK_LSHIFT;
				break;
			case chif::input::KEYBOARD_BUTTON_F1: 
				keycode = VK_F1;
				break;
			case chif::input::KEYBOARD_BUTTON_F2:
				keycode = VK_F2;
				break;
			case chif::input::KEYBOARD_BUTTON_F3:
				keycode = VK_F3;
				break;
			case chif::input::KEYBOARD_BUTTON_F4:
				keycode = VK_F4;
				break;
			case chif::input::KEYBOARD_BUTTON_F5:
				keycode = VK_F5;
				break;
			case chif::input::KEYBOARD_BUTTON_F6:
				keycode = VK_F6;
				break;
			case chif::input::KEYBOARD_BUTTON_F7:
				keycode = VK_F7;
				break;
			case chif::input::KEYBOARD_BUTTON_F8:
				keycode = VK_F8;
				break;
			case chif::input::KEYBOARD_BUTTON_F9:
				keycode = VK_F9;
				break;
			case chif::input::KEYBOARD_BUTTON_F10:
				keycode = VK_F10;
				break;
			case chif::input::KEYBOARD_BUTTON_F11:
				keycode = VK_F11;
				break;
			case chif::input::KEYBOARD_BUTTON_F12:
				keycode = VK_F12;
				break;
			case chif::input::KEYBOARD_BUTTON_ENTER:
				keycode = VK_RETURN;
				break;
			case chif::input::KEYBOARD_BUTTON_ESCAPE:
				keycode = VK_ESCAPE;
				break;
			case chif::input::KEYBOARD_BUTTON_HOME:
				keycode = VK_HOME;
				break;
			case chif::input::KEYBOARD_BUTTON_LCONTROL:
				keycode = VK_LCONTROL;
				break;
			case chif::input::KEYBOARD_BUTTON_RCONTROL:
				keycode = VK_RCONTROL;
				break;
			case chif::input::KEYBOARD_BUTTON_INSERT:
				keycode = VK_INSERT;
				break;
			case chif::input::KEYBOARD_BUTTON_DELETE:
				keycode = VK_DELETE;
				break;
			case chif::input::KEYBOARD_BUTTON_BACKSPACE:
				keycode = VK_BACK;
				break;
			case chif::input::KEYBOARD_BUTTON_PAGEDOWN:
				keycode = VK_NEXT;
				break;
			case chif::input::KEYBOARD_BUTTON_PAGEUP:
				keycode = VK_PRIOR;
				break;
	...
	void SetControllerFeedback(const ControllerFeedback& data, int playerindex)
	{
		if (playerindex < (int)controllers.size())
		{
			...
			{
				chif::input::xinput::SetControllerFeedback(data, controller.deviceIndex);
			}
			else if (controller.deviceType == Controller::RAWINPUT)
			{
				chif::input::rawinput::SetControllerFeedback(data, controller.deviceIndex);
			}
			else if (controller.deviceType == Controller::SDLINPUT)
			{
				chif::input::sdlinput::SetControllerFeedback(data, controller.deviceIndex);
			}
#ifdef PLATFORM_PS5
			else if (controller.deviceType == Controller::PS5)
			{
				chif::input::ps5::SetControllerFeedback(data, controller.deviceIndex);
			}
#endif // PLATFORM_PS5
		}
	}
	...
	const chif::vector<Touch>& GetTouches()
	{
		return touches;
	}
	...
	void SetCursorFromFile(CURSOR cursor, const char* filename)
	{
		...
		chif::helper::StringConvert(filename, wfilename);
	...
	ShortReturnString ButtonToString(BUTTON button, CONTROLLER_PREFERENCE preference)
	{
		...
		if (preference == CONTROLLER_PREFERENCE_PLAYSTATION)
		{
			switch (button)
			{
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_SQUARE: return "■";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_CROSS: return "✖";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_CIRCLE: return "●";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_TRIANGLE: return "▲";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_L1: return "L1";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_L2: return "L2";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_R1: return "R1";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_R2: return "R2";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_L3: return "L3";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_R3: return "R3";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_SHARE: return "Share";
			case chif::input::GAMEPAD_BUTTON_PLAYSTATION_OPTION: return "Option";
			default:
				break;
			}
		}
		...
		if (preference == CONTROLLER_PREFERENCE_XBOX)
		{
			switch (button)
			{
			case chif::input::GAMEPAD_BUTTON_XBOX_X: return "(X)";
			case chif::input::GAMEPAD_BUTTON_XBOX_A: return "(A)";
			case chif::input::GAMEPAD_BUTTON_XBOX_B: return "(B)";
			case chif::input::GAMEPAD_BUTTON_XBOX_Y: return "(Y)";
			case chif::input::GAMEPAD_BUTTON_XBOX_L1: return "L1";
			case chif::input::GAMEPAD_BUTTON_XBOX_LT: return "LT";
			case chif::input::GAMEPAD_BUTTON_XBOX_R1: return "R1";
			case chif::input::GAMEPAD_BUTTON_XBOX_RT: return "RT";
			case chif::input::GAMEPAD_BUTTON_XBOX_L3: return "L3";
			case chif::input::GAMEPAD_BUTTON_XBOX_R3: return "R3";
			case chif::input::GAMEPAD_BUTTON_XBOX_BACK: return "Back";
			case chif::input::GAMEPAD_BUTTON_XBOX_START: return "Start";
			default:
				break;
			}
		}
		...
		switch (button)
		{
		case chif::input::GAMEPAD_BUTTON_UP: return "Dpad ↑";
		case chif::input::GAMEPAD_BUTTON_LEFT: return "Dpad ←";
		case chif::input::GAMEPAD_BUTTON_DOWN: return "Dpad ↓";
		case chif::input::GAMEPAD_BUTTON_RIGHT: return "Dpad →";
		case chif::input::GAMEPAD_BUTTON_1: return "Gamepad 1";
		case chif::input::GAMEPAD_BUTTON_2: return "Gamepad 2";
		case chif::input::GAMEPAD_BUTTON_3: return "Gamepad 3";
		case chif::input::GAMEPAD_BUTTON_4: return "Gamepad 4";
		case chif::input::GAMEPAD_BUTTON_5: return "Gamepad 5";
		case chif::input::GAMEPAD_BUTTON_6: return "Gamepad 6";
		case chif::input::GAMEPAD_BUTTON_7: return "Gamepad 7";
		case chif::input::GAMEPAD_BUTTON_8: return "Gamepad 8";
		case chif::input::GAMEPAD_BUTTON_9: return "Gamepad 9";
		case chif::input::GAMEPAD_BUTTON_10: return "Gamepad 10";
		case chif::input::GAMEPAD_BUTTON_11: return "Gamepad 11";
		case chif::input::GAMEPAD_BUTTON_12: return "Gamepad 12";
		case chif::input::GAMEPAD_BUTTON_13: return "Gamepad 13";
		case chif::input::GAMEPAD_BUTTON_14: return "Gamepad 14";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_UP: return "Left Stick ↑";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_LEFT: return "Left Stick ←";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_DOWN: return "Left Stick ↓";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_RIGHT: return "Left Stick →";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_UP: return "Right Stick ↑";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_LEFT: return "Right Stick ←";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_DOWN: return "Right Stick ↓";
		case chif::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_RIGHT: return "Right Stick →";
		case chif::input::GAMEPAD_ANALOG_TRIGGER_L_AS_BUTTON: return "Left Trigger";
		case chif::input::GAMEPAD_ANALOG_TRIGGER_R_AS_BUTTON: return "Right Trigger";
		case chif::input::MOUSE_BUTTON_LEFT: return "Left Mouse Button";
		case chif::input::MOUSE_BUTTON_RIGHT: return "Right Mouse Button";
		case chif::input::MOUSE_BUTTON_MIDDLE: return "Middle Mouse Button";
		case chif::input::MOUSE_SCROLL_AS_BUTTON_UP: return "Mouse Wheel ↑";
		case chif::input::MOUSE_SCROLL_AS_BUTTON_DOWN: return "Mouse Wheel ↓";
		case chif::input::KEYBOARD_BUTTON_UP: return "↑";
		case chif::input::KEYBOARD_BUTTON_DOWN: return "↓";
		case chif::input::KEYBOARD_BUTTON_LEFT: return "←";
		case chif::input::KEYBOARD_BUTTON_RIGHT: return "→";
		case chif::input::KEYBOARD_BUTTON_SPACE: return "Space";
		case chif::input::KEYBOARD_BUTTON_RSHIFT: return "Right Shift";
		case chif::input::KEYBOARD_BUTTON_LSHIFT: return "Left Shift";
		case chif::input::KEYBOARD_BUTTON_F1: return "F1";
		case chif::input::KEYBOARD_BUTTON_F2: return "F2";
		case chif::input::KEYBOARD_BUTTON_F3: return "F3";
		case chif::input::KEYBOARD_BUTTON_F4: return "F4";
		case chif::input::KEYBOARD_BUTTON_F5: return "F5";
		case chif::input::KEYBOARD_BUTTON_F6: return "F6";
		case chif::input::KEYBOARD_BUTTON_F7: return "F7";
		case chif::input::KEYBOARD_BUTTON_F8: return "F8";
		case chif::input::KEYBOARD_BUTTON_F9: return "F9";
		case chif::input::KEYBOARD_BUTTON_F10: return "F10";
		case chif::input::KEYBOARD_BUTTON_F11: return "F11";
		case chif::input::KEYBOARD_BUTTON_F12: return "F12";
		case chif::input::KEYBOARD_BUTTON_ENTER: return "Enter";
		case chif::input::KEYBOARD_BUTTON_ESCAPE: return "Escape";
		case chif::input::KEYBOARD_BUTTON_HOME: return "Home";
		case chif::input::KEYBOARD_BUTTON_RCONTROL: return "Right Control";
		case chif::input::KEYBOARD_BUTTON_LCONTROL: return "Left Control";
		case chif::input::KEYBOARD_BUTTON_DELETE: return "Delete";
		case chif::input::KEYBOARD_BUTTON_BACKSPACE: return "Backspace";
		case chif::input::KEYBOARD_BUTTON_PAGEDOWN: return "Page Down";
		case chif::input::KEYBOARD_BUTTON_PAGEUP: return "Page Up";
		case chif::input::KEYBOARD_BUTTON_NUMPAD0: return "Numpad 0";
		case chif::input::KEYBOARD_BUTTON_NUMPAD1: return "Numpad 1";
		case chif::input::KEYBOARD_BUTTON_NUMPAD2: return "Numpad 2";
		case chif::input::KEYBOARD_BUTTON_NUMPAD3: return "Numpad 3";
		case chif::input::KEYBOARD_BUTTON_NUMPAD4: return "Numpad 4";
		case chif::input::KEYBOARD_BUTTON_NUMPAD5: return "Numpad 5";
		case chif::input::KEYBOARD_BUTTON_NUMPAD6: return "Numpad 6";
		case chif::input::KEYBOARD_BUTTON_NUMPAD7: return "Numpad 7";
		case chif::input::KEYBOARD_BUTTON_NUMPAD8: return "Numpad 8";
		case chif::input::KEYBOARD_BUTTON_NUMPAD9: return "Numpad 9";
		case chif::input::KEYBOARD_BUTTON_MULTIPLY: return "*";
		case chif::input::KEYBOARD_BUTTON_ADD: return "+";
		case chif::input::KEYBOARD_BUTTON_SEPARATOR: return "Separator";
		case chif::input::KEYBOARD_BUTTON_SUBTRACT: return "-";
		case chif::input::KEYBOARD_BUTTON_DECIMAL: return "Decimal";
		case chif::input::KEYBOARD_BUTTON_DIVIDE: return "/";
		case chif::input::KEYBOARD_BUTTON_TAB: return "Tab";
		case chif::input::KEYBOARD_BUTTON_TILDE: return "Tilde";
		case chif::input::KEYBOARD_BUTTON_INSERT: return "Insert";
		case chif::input::KEYBOARD_BUTTON_ALT: return "Alt";
		case chif::input::KEYBOARD_BUTTON_ALTGR: return "Alt Gr";
		default:
			break;
		}
