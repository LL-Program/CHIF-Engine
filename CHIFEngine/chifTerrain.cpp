...
namespace chif::terrain
{
	...
	struct Generator
	{
		chif::scene::Scene scene; // The background generation thread can safely add things to this, it will be merged into the main scene when it is safe to do so
		chif::jobsystem::context workload;
		std::atomic_bool cancelled{ false };
	};

	chif::jobsystem::context virtual_texture_ctx;
	...
	void VirtualTextureAtlas::Residency::init(uint32_t resolution)
	{
		...
		if (resolution > SVT_TILE_SIZE)
		{
			...
			chif::vector<uint32_t> default_data(td.width * td.height);
			chif::vector<SubresourceData> initdata(lod_count);
...

			{
				...
				chif::vector<uint32_t> data(tile_count);
			...
			}
			{
				...
				chif::vector<uint32_t> data(tile_count + 1);
	...
	{
		Generation_Cancel();
		chif::jobsystem::Wait(virtual_texture_ctx);
	}
	...
	void Terrain::Generation_Update(const CameraComponent& camera)
	{
		...
		for (chif::ecs::Entity entity : materialEntities)
		{
		...
		for (auto it = chunks.begin(); it != chunks.end();)
		{
			...
			if (chif::renderer::GetOcclusionCullingEnabled())
			{
			...
			{
				chif::HairParticleSystem* grass = scene->hairs.GetComponent(chunk_data.grass_entity);
				if (grass != nullptr)
				{
		...
		if (chunk_buffer.desc.size < required_chunk_buffer_size)
		{
			...
			assert(success);
			device->SetName(&chunk_buffer, "chif::terrain::Terrain::chunk_buffer");
		}

		// Start the generation on a background thread and keep it running until the next frame
		chif::jobsystem::Execute(generator->workload, [=](chif::jobsystem::JobArgs args) {
			...
			auto request_chunk = [&](int offset_x, int offset_z)
			{
				...
				if (it == chunks.end() || it->second.entity == INVALID_ENTITY)
				{
					...
					object.filterMask |= chif::enums::FILTER_NAVIGATION_MESH;
					object.filterMask |= chif::enums::FILTER_TERRAIN;
					...
					chif::HairParticleSystem grass = grass_properties;
					...
					chif::jobsystem::Dispatch(ctx, vertexCount, chunk_width, [&](chif::jobsystem::JobArgs args) {
						...
						for (int i = 0; i < arraysize(corners); ++i)
						{
							...
							height = chif::math::Lerp(bottomLevel, topLevel, height);
						...
						float region_low_altitude = region2 == 0 ? 1 : smoothstep(0.0f, region2, chif::math::InverseLerp(0, bottomLevel, height));
						float region_high_altitude = region3 == 0 ? 1 : smoothstep(0.0f, region3, chif::math::InverseLerp(0, topLevel, height));
					...
					chif::jobsystem::Wait(ctx); // wait until chunk's vertex buffer is fully generated
					...
					chif::jobsystem::Wait(ctx); // wait until mesh.CreateRenderData() async task finishes
				...
				if (dist <= grass_chunk_dist && IsGrassEnabled())
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (chunk_data.grass_entity == INVALID_ENTITY && chunk_data.grass.meshID != INVALID_ENTITY)
						{
							...
							chif::HairParticleSystem& grass = generator->scene.hairs.Create(chunk_data.grass_entity);
				...
				if (dist <= prop_generation)
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (prop_density > 0 && chunk_data.props_entity == INVALID_ENTITY && chunk_data.mesh_vertex_positions != nullptr)
						{
							...
							chif::random::RNG rng((uint32_t)chunk.compute_hash() ^ seed);
							...
							for (const auto& prop : props)
							{
								...
								for (int i = 0; i < gen_count; ++i)
								{
									...
									XMFLOAT4 region0 = chif::Color(chunk_data.blendmap_layers[0].pixels[ind0], chunk_data.blendmap_layers[1].pixels[ind0], chunk_data.blendmap_layers[2].pixels[ind0], chunk_data.blendmap_layers[3].pixels[ind0]);
									XMFLOAT4 region1 = chif::Color(chunk_data.blendmap_layers[0].pixels[ind1], chunk_data.blendmap_layers[1].pixels[ind1], chunk_data.blendmap_layers[2].pixels[ind1], chunk_data.blendmap_layers[3].pixels[ind1]);
									XMFLOAT4 region2 = chif::Color(chunk_data.blendmap_layers[0].pixels[ind2], chunk_data.blendmap_layers[1].pixels[ind2], chunk_data.blendmap_layers[2].pixels[ind2], chunk_data.blendmap_layers[3].pixels[ind2]);
									weight_norm(region0);
									weight_norm(region1);
									weight_norm(region2);
									...
									{
										chif::Archive archive = chif::Archive(prop.data.data(), prop.data.size());
										EntitySerializer seri;
										Entity entity = generator->scene.Entity_Serialize(
											archive,
											seri,
											INVALID_ENTITY,
											chif::scene::Scene::EntitySerializeFlags::RECURSIVE |
											chif::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
										);
										...
										transform->translation_local.y += chif::math::Lerp(prop.min_y_offset, prop.max_y_offset, rng.next_float());
										const float scaling = chif::math::Lerp(prop.min_size, prop.max_size, rng.next_float());
	...
	void Terrain::Generation_Cancel()
	{
		...
		chif::jobsystem::Wait(generator->workload); // waits until generation thread exits
	...
	void Terrain::CreateChunkRegionTexture(ChunkData& chunk_data)
	{
		...
		if (!chunk_data.heightmap.IsValid() && !chunk_data.heightmap_data.empty())
		{
			...
			assert(success);
			device->SetName(&chunk_data.heightmap, "chif::terrain::ChunkData::heightmap");
		}
		...
		if (!chunk_data.blendmap.IsValid() || chunk_data.blendmap.desc.array_size != (uint32_t)chunk_data.blendmap_layers.size())
		{
			...
			chif::vector<SubresourceData> data(chunk_data.blendmap_layers.size());
			...
			assert(success);
			device->SetName(&chunk_data.blendmap, "chif::terrain::ChunkData::blendmap");
		}
	}
	...
	{
		chif::jobsystem::Wait(virtual_texture_ctx);
		virtual_texture_ctx.priority = chif::jobsystem::Priority::Low;
		...
		chif::jobsystem::Execute(virtual_texture_ctx, [this](chif::jobsystem::JobArgs args) {
	...
	void Terrain::UpdateVirtualTexturesGPU(CommandList cmd) const
	{
		...
		auto range = chif::profiler::BeginRangeGPU("Terrain - UpdateVirtualTexturesGPU", cmd);

		device->EventBegin("Update Residency Maps", cmd);
		device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_VIRTUALTEXTURE_RESIDENCYUPDATE), cmd);
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			for (uint32_t map_type = 0; map_type < arraysize(atlas.maps); map_type++)
			{
				...
				{
				case MaterialComponent::BASECOLORMAP:
					device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_BASECOLORMAP), cmd);
					...
					break;
				case MaterialComponent::NORMALMAP:
					device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_NORMALMAP), cmd);
					...
					break;
				case MaterialComponent::SURFACEMAP:
					device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_SURFACEMAP), cmd);
					...
					break;
				case MaterialComponent::EMISSIVEMAP:
					device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_EMISSIVEMAP), cmd);
	...
	void Terrain::AllocateVirtualTextureTileRequestsGPU(CommandList cmd) const
	{
...

		{
			...
			device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_VIRTUALTEXTURE_TILEREQUESTS), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
...

		{
			...
			device->BindComputeShader(chif::renderer::GetShader(chif::enums::CSTYPE_VIRTUALTEXTURE_TILEALLOCATE), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
	...
	void Terrain::WritebackTileRequestsGPU(CommandList cmd) const
	{
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			chif::renderer::PushBarrier(GPUBarrier::Buffer(&vt->residency->allocationBuffer, ResourceState::COPY_SRC, ResourceState::COPY_DST));
		}

		chif::renderer::FlushBarriers(cmd);
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			static chif::unordered_map<uint32_t, VirtualTextureAtlas::Residency> clear_residencies;
			...
			chif::renderer::PushBarrier(GPUBarrier::Image(&vt->residency->residencyMap, ResourceState::COPY_DST, ResourceState::COPY_SRC));
		}
		chif::renderer::FlushBarriers(cmd);
		device->EventEnd(cmd);
	}
	...
	void Terrain::Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri)
	{
		Generation_Cancel();
		...
		const uint64_t grass_version = seri.GetVersion("chif::scene::Scene::hairs");
		const uint64_t material_version = seri.GetVersion("chif::scene::Scene::materials");
		const uint64_t weather_version = seri.GetVersion("chif::scene::Scene::weathers");
		...
		if (archive.IsReadMode())
		{
			...
			for (size_t i = 0; i < props.size(); ++i)
			{
				...
				if (terrain_version >= 1)
				{
					...
					if (!prop.data.empty())
					{
						...
						Scene tmp_scene;
						chif::Archive tmp_archive = chif::Archive(prop.data.data(), prop.data.size());
				...
				else
				{
					...
					tmp_scene.Entity_Serialize(
						...
						seri,
						object_entity,
						chif::scene::Scene::EntitySerializeFlags::RECURSIVE | chif::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
					);
			...
			for (size_t i = 0; i < count; ++i)
			{
				...
				{
					chif::vector<chif::Color> blendmap4;
					archive >> blendmap4;
		...
		else
		{
			...
			chif::scene::MaterialComponent materials[MATERIAL_COUNT];
			...
			chif::jobsystem::Wait(seri.ctx); // wait for material CreateRenderData() that was asynchronously launched in Serialize()!
			...
			ComponentManager<MaterialComponent>* scene_materials = library.Get<MaterialComponent>("chif::scene::Scene::materials");
			...
			ComponentManager<NameComponent>* scene_names = library.Get<NameComponent>("chif::scene::Scene::names");
		...
		if (terrain_version < 4)
		{
			...
			ComponentManager<chif::HairParticleSystem>* scene_hairs = library.Get<chif::HairParticleSystem>("chif::scene::Scene::hairs");
