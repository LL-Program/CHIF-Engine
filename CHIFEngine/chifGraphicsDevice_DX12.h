...
#ifdef PLATFORM_XBOX
#include "chifGraphicsDevice_DX12_XBOX.h"
#else
...
namespace chif::graphics
{
	...
	protected:
		...
		struct CommandQueue
		{
			...
			chif::vector<ID3D12CommandList*> submit_cmds;
		...
		chif::vector<Semaphore> semaphore_pool;
		...
		struct CommandList_DX12
		{
			...
			QUEUE_TYPE queue = {};
			uint32_t id = 0;
			chif::vector<std::pair<QUEUE_TYPE, Semaphore>> wait_queues;
			chif::vector<Semaphore> waits;
			chif::vector<Semaphore> signals;

			DescriptorBinder binder;
			GPULinearAllocator frame_allocators[BUFFERCOUNT];

			chif::vector<D3D12_RESOURCE_BARRIER> frame_barriers;
			struct Discard
			{
				ID3D12Resource* resource = nullptr;
				D3D12_DISCARD_REGION region = {};
			};
			chif::vector<Discard> discards;
			D3D_PRIMITIVE_TOPOLOGY prev_pt = {};
			chif::vector<std::pair<size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>>> pipelines_worker;
			...
			chif::vector<const SwapChain*> swapchains;
			bool dirty_pso = {};
			chif::vector<D3D12_RAYTRACING_GEOMETRY_DESC> accelerationstructure_build_geometries;
			RenderPassInfo renderpass_info;
			chif::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_begin;
			chif::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_begin_after_discards;
			chif::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_end;
			...
			chif::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources[D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT] = {};
			chif::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources_dsv = {};
			chif::vector<D3D12_RESOURCE_BARRIER> resolve_src_barriers;
		...
		chif::vector<std::unique_ptr<CommandList_DX12>> commandlists;
		...
		chif::unordered_map<size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>> pipelines_global;
	...
	public:
		...
		struct AllocationHandler
		{
			...
			struct DescriptorAllocator
			{
				...
				chif::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> heaps;
				uint32_t descriptor_size = 0;
				chif::vector<D3D12_CPU_DESCRIPTOR_HANDLE> freelist;
			...
			chif::vector<int> free_bindless_res;
			chif::vector<int> free_bindless_sam;
