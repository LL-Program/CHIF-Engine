...
namespace chif
{
		...
		: readMode(readMode), fileName(fileName)
	{
		if (!fileName.empty())
		{
			directory = chif::helper::GetDirectoryFromPath(fileName);
			if (readMode)
			{
				if (chif::helper::FileRead(fileName, DATA))
				{
					data_ptr = DATA.data();
	...
	{
		return chif::helper::FileWrite(fileName, data_ptr, pos);
	}

	bool Archive::SaveHeaderFile(const std::string& fileName, const std::string& dataName)
	{
		return chif::helper::Bin2H(data_ptr, pos, fileName, dataName.c_str());
	}
	...
	chif::graphics::Texture Archive::CreateThumbnailTexture() const
	{
		...
		chif::texturehelper::CreateTexture(texture, rgba, (uint32_t)width, (uint32_t)height);
		stbi_image_free(rgba);
		return texture;
	}

	void Archive::SetThumbnailAndResetPos(const chif::graphics::Texture& texture)
	{
		chif::vector<uint8_t> thumbnail_data;
		chif::helper::saveTextureToMemoryFile(texture, "JPG", thumbnail_data);
	...
	chif::graphics::Texture Archive::PeekThumbnail(const std::string& filename)
	{
		chif::vector<uint8_t> filedata;

		size_t required_size = sizeof(uint64_t) * 2; // version and thumbnail data size

		chif::helper::FileRead(filename, filedata, required_size); // read only up to version and thumbnail data size
		if (filedata.empty())
			return {};

		{
			chif::Archive archive(filedata.data(), filedata.size());
			if (archive.IsOpen())
			{
		...
		chif::helper::FileRead(filename, filedata, required_size); // read up to end of thumbnail data
		if (filedata.empty())
			return {};
