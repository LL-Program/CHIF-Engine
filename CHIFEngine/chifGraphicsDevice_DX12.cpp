...
#include "Utility/D3D12MemAlloc.cpp"
...
namespace dx12_internal
{
	...
	struct RootSignatureOptimizer
	{
		...
		chif::vector<RootParameterStatistics> root_stats;
	...
	struct Resource_DX12
	{
		...
		chif::vector<SingleDescriptor> subresources_srv;
		chif::vector<SingleDescriptor> subresources_uav;
		SingleDescriptor uav_raw;

		D3D12_GPU_VIRTUAL_ADDRESS gpu_address = 0;

		UINT64 total_size = 0;
		chif::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> footprints;
		chif::vector<UINT64> rowSizesInBytes;
		chif::vector<UINT> numRows;
	...
	struct Texture_DX12 : public Resource_DX12
	{
		...
		chif::vector<SingleDescriptor> subresources_rtv;
		chif::vector<SingleDescriptor> subresources_dsv;
	...
	struct PipelineState_DX12
	{
		...
		chif::vector<uint8_t> shadercode;
		chif::vector<D3D12_INPUT_ELEMENT_DESC> input_elements;
	...
	struct BVH_DX12 : public Resource_DX12
	{
		...
		chif::vector<D3D12_RAYTRACING_GEOMETRY_DESC> geometries;
	...
	struct RTPipelineState_DX12
	{
		...
		chif::vector<std::wstring> export_strings;
		chif::vector<D3D12_EXPORT_DESC> exports;
		chif::vector<D3D12_DXIL_LIBRARY_DESC> library_descs;
		chif::vector<std::wstring> group_strings;
		chif::vector<D3D12_HIT_GROUP_DESC> hitgroup_descs;
...
#endif // PLATFORM_XBOX
		chif::vector<ComPtr<ID3D12Resource>> backBuffers;
		chif::vector<D3D12_CPU_DESCRIPTOR_HANDLE> backbufferRTV;
...
std::mutex queue_locker;
	...
	void GraphicsDevice_DX12::CopyAllocator::init(GraphicsDevice_DX12* device)
	{
		...
		if (FAILED(hr))
		{
			...
			chif::helper::messageBox(ss.str(), "Error!");
			chif::platform::Exit();
		}
	...
	GraphicsDevice_DX12::CopyAllocator::CopyCMD GraphicsDevice_DX12::CopyAllocator::allocate(uint64_t staging_size)
	{
		...
		if (!cmd.IsValid())
		{
			...
			uploadBufferDesc.size = chif::math::GetNextPowerOfTwo(staging_size);
	...
	GraphicsDevice_DX12::GraphicsDevice_DX12(ValidationMode validationMode_, GPUPreference preference)
	{
		...
		hr = chif::graphics::xbox::CreateDevice(device, dxgiAdapter, validationMode);
		...
		// Determines whether tearing support is available for fullscreen borderless windows.
		{
			...
			if (FAILED(hr) || !allowTearing)
			{
				...
				chif::helper::DebugOut("WARNING: Variable refresh rate displays not supported\n");
			}
			else
			{
		...
		{
			chif::helper::messageBox("DXGI: No capable adapter found!", "Error!");
			chif::platform::Exit();
		}
		...
		if (device == nullptr)
		{
			chif::helper::messageBox("D3D12: Device couldn't be created!", "Error!");
			chif::platform::Exit();
		}
...
#ifdef PLATFORM_XBOX
		adapterName = chif::graphics::xbox::GetAdapterName();
		...
		// Init adapter properties
		{
			...
			chif::helper::StringConvert(adapterDesc.Description, adapterName);
		...
		if (features.HighestShaderModel() < D3D_SHADER_MODEL_6_0)
		{
			...
			switch (adapterType)
			{
			case chif::graphics::AdapterType::IntegratedGpu:
				error += "Integrated GPU";
				break;
			case chif::graphics::AdapterType::DiscreteGpu:
				error += "Discrete GPU";
				break;
			case chif::graphics::AdapterType::VirtualGpu:
				error += "Virtual GPU";
				break;
			case chif::graphics::AdapterType::Cpu:
				error += "CPU";
				break;
			default:
				error += "Unknown";
			...
			error += "\nExiting.";
			chif::helper::messageBox(error, "Error!");
			chif::backlog::post(error, chif::backlog::LogLevel::Error);
			chif::platform::Exit();
		}
		...
		{
			assert(0);
			chif::helper::messageBox("DX12: Root signature version 1.1 not supported!", "Error!");
			chif::platform::Exit();
		}
...
#ifdef PLATFORM_XBOX
			chif::graphics::xbox::FillDispatchMeshIndirectArgumentDesc(dispatchMeshArgs[0], cmd_desc);
#else
		...
		if (FAILED(hr))
		{
			...
			chif::helper::messageBox(ss.str(), "Warning!");
		}

		chif::backlog::post("Created GraphicsDevice_DX12 (" + std::to_string((int)std::round(timer.elapsed())) + " ms)\nAdapter: " + adapterName);
	}
	...
	bool GraphicsDevice_DX12::CreateSwapChain(const SwapChainDesc* desc, chif::platform::window_type window, SwapChain* swapchain) const
	{
...
#ifdef PLATFORM_XBOX
		chif::graphics::xbox::ApplyBufferCreationFlags(*desc, resourceDesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
...
#ifdef PLATFORM_XBOX
		chif::graphics::xbox::ApplyTextureCreationFlags(texture->desc, resourcedesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
	...
	bool GraphicsDevice_DX12::CreatePipelineState(const PipelineStateDesc* desc, PipelineState* pso, const RenderPassInfo* renderpass_info) const
	{
		...
		{
			chif::helper::hash_combine(internal_state->hash, renderpass_info->get_hash());
	...
	bool GraphicsDevice_DX12::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		chif::vector<D3D12_STATE_SUBOBJECT> subobjects;
		...
		for(auto& x : desc->shader_libraries)
		{
			...
			chif::helper::StringConvert(x.function_name, internal_state->export_strings.emplace_back());
		...
		{
			chif::helper::StringConvert(x.name, internal_state->group_strings.emplace_back());
	...
	bool GraphicsDevice_DX12::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		chif::vector<GUID> profiles(video_decode_profile_count.ProfileCount);
		...
		chif::vector<DXGI_FORMAT> formats(video_decode_format_count.FormatCount);
	...
	{
		wchar_t text[256];
		if (chif::helper::StringConvert(name, text, arraysize(text)) > 0)
		{
	...
	void GraphicsDevice_DX12::OnDeviceRemoved()
	{
		...
		if (SUCCEEDED(device->QueryInterface(PPV_ARGS(pDred))))
		{
			...
			if (SUCCEEDED(pDred->GetAutoBreadcrumbsOutput1(&pDredAutoBreadcrumbsOutput)))
			{
				...
				while (pNode && pNode->pLastBreadcrumbValue)
				{
					...
					{
						chif::helper::StringConvert(pNode->pCommandListDebugNameW, commandlistname);
					}
					if (pNode->pCommandQueueDebugNameW != nullptr)
					{
						chif::helper::StringConvert(pNode->pCommandQueueDebugNameW, commandqueuename);
					}
					...
					for (int op = firstOp; op <= lastOp; ++op)
					{
						...
						{
							chif::helper::StringConvert(it->second, contextString);
						}
			...
			if (SUCCEEDED(pDred->GetPageFaultAllocationOutput1(&DredPageFaultOutput)) && DredPageFaultOutput.PageFaultVA != 0)
			{
				...
				if (pNode)
				{
					...
					while (pNode)
					{
						...
						{
							chif::helper::StringConvert(pNode->ObjectNameW, objectname);
						}
		...
		if (!log.empty())
		{
			chif::backlog::post(log, chif::backlog::LogLevel::Error);
		}

		std::string message = "D3D12: device removed, cause: ";
		message += removedReasonString;
		chif::helper::messageBox(message, "Error!");
		chif::platform::Exit();
#endif // PLATFORM_WINDOWS_DESKTOP
	}
	...
	void GraphicsDevice_DX12::SparseUpdate(QUEUE_TYPE queue, const SparseUpdateCommand* commands, uint32_t command_count)
	{
		...
		thread_local chif::vector<D3D12_TILED_RESOURCE_COORDINATE> tiled_resource_coordinates;
		thread_local chif::vector<D3D12_TILE_REGION_SIZE> tiled_region_sizes;
		thread_local chif::vector<D3D12_TILE_RANGE_FLAGS> tile_range_flags;
		thread_local chif::vector<uint32_t> range_start_offsets;
	...
	void GraphicsDevice_DX12::EventBegin(const char* name, CommandList cmd)
	{
		...
		if (commandlist.queue == QUEUE_VIDEO_DECODE)
			return;
		wchar_t text[128];
		if (chif::helper::StringConvert(name, text) > 0)
		{
