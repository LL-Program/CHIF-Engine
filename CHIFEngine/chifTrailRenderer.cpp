...
namespace chif
{
	...
	void TrailRenderer::Draw(const CameraComponent& camera, CommandList cmd) const
	{
		...
		sb.g_xTrailTextureIndex1 = device->GetDescriptorIndex(texture.IsValid() ? &texture : chif::texturehelper::getWhite(), SubresourceType::SRV);
		sb.g_xTrailTextureIndex2 = device->GetDescriptorIndex(texture2.IsValid() ? &texture : chif::texturehelper::getWhite(), SubresourceType::SRV);
		...
		while (next_cut <= cuts.size())
		{
			...
			for (; i < count; ++i)
			{
				...
				for (uint32_t j = 0; j < segment_resolution; ++j)
				{
					...
					XMVECTOR P = cap ? XMVectorLerp(P1, P2, t) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t);
					XMVECTOR P_prev = cap ? XMVectorLerp(P0, P1, t - resolution_rcp) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t - resolution_rcp);
					XMVECTOR P_next = cap ? XMVectorLerp(P1, P2, t + resolution_rcp) : chif::math::CatmullRomCentripetal(P0, P1, P2, P3, t + resolution_rcp);
					float width_interpolated = chif::math::Lerp(width_current, width_next, t);
					XMFLOAT4 color_interpolated = chif::math::Lerp(color_current, color_next, t);
	...
	namespace TrailRenderer_Internal
	{
		void LoadShaders()
		{
			chif::renderer::LoadShader(ShaderStage::VS, vertexShader, "trailVS.cso");
			chif::renderer::LoadShader(ShaderStage::PS, pixelShader, "trailPS.cso");
	...
	void TrailRenderer::Initialize()
	{
		...
		static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { TrailRenderer_Internal::LoadShaders(); });
		TrailRenderer_Internal::LoadShaders();

		chif::backlog::post("chif::TrailRenderer Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
}
