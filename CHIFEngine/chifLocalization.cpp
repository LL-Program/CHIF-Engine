...
namespace chif
{
	...
	bool Localization::Import(const std::string& filename)
	{
		...
		{
			chif::backlog::post("XML error in " + filename + " at offset = " + std::to_string(result.offset) + ": " + result.description(), chif::backlog::LogLevel::Warning);
			return false;
		}
	...
	bool Localization::Export(const std::string& filename) const
	{
		...
		doc.append_child(pugi::node_comment).set_value("You can import this file with the chif::Localization::Import() function.");
