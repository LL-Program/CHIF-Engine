#ifndef CHIF_ENTITY_COMPONENT_SYSTEM_H
#define CHIF_ENTITY_COMPONENT_SYSTEM_H
...
namespace chif::ecs
{
	...
	{
		chif::jobsystem::context ctx; // allow components to spawn serialization subtasks
		chif::unordered_map<uint64_t, Entity> remap;
		...
		chif::unordered_set<std::string> resource_registration; // register for resource manager serialization
		ComponentLibrary* componentlibrary = nullptr;
		chif::unordered_map<std::string, uint64_t> library_versions;

		~EntitySerializer()
		{
			chif::jobsystem::Wait(ctx); // automatically wait for all subtasks after serialization
		}
	...
	inline void SerializeEntity(chif::Archive& archive, Entity& entity, EntitySerializer& seri)
	{
...
#pragma GCC diagnostic pop
	...
	public:
		...
		virtual void Serialize(chif::Archive& archive, EntitySerializer& seri) = 0;
		virtual void Component_Serialize(Entity entity, chif::Archive& archive, EntitySerializer& seri) = 0;
		...
		virtual const chif::vector<Entity>& GetEntityArray() const = 0;
	};
	...
	public:
		...
		inline void Serialize(chif::Archive& archive, EntitySerializer& seri)
		{
			if (archive.IsReadMode())
			{
		...
		inline void Component_Serialize(Entity entity, chif::Archive& archive, EntitySerializer& seri)
		{
			if(archive.IsReadMode())
			{
		...
		inline const chif::vector<Entity>& GetEntityArray() const { return entities; }

		// Returns the tightly packed [read only] component array
		inline const chif::vector<Component>& GetComponentArray() const { return components; }

	private:
		// This is a linear array of alive components
		chif::vector<Component> components;
		// This is a linear array of entities corresponding to each alive component
		chif::vector<Entity> entities;
		// This is a lookup table for entities
		chif::unordered_map<Entity, size_t> lookup;
	...
	public:
		...
		chif::unordered_map<std::string, LibraryEntry> entries;
		...
		inline void Entity_Serialize(Entity entity, chif::Archive& archive, EntitySerializer& seri)
		{
			seri.componentlibrary = this;
			if(archive.IsReadMode())
			{
...
#endif // CHIF_ENTITY_COMPONENT_SYSTEM_H
