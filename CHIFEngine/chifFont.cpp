...
namespace chif::font
{
	namespace font_internal
	{
		...
		static thread_local chif::Canvas canvas;
		...
		{
			std::string name;
			chif::vector<uint8_t> fontBuffer; // only used if loaded from file, need to keep alive
			...
			void Create(const std::string& newName, const uint8_t* data, size_t size)
			{
				...
				{
					chif::backlog::post("Failed to load font: " + name + " (file was unrecognized, it must be a .ttf file)");
				}
			...
			{
				if (chif::helper::FileRead(newName, fontBuffer))
				{
					Create(newName, fontBuffer.data(), fontBuffer.size());
				}
				else
				{
					chif::backlog::post("Failed to load font: " + name + " (file could not be opened)");
				}
			}
		};
		static chif::vector<std::unique_ptr<FontStyle>> fontStyles;
		...
		static chif::unordered_map<int32_t, Glyph> glyph_lookup;
		static chif::unordered_map<int32_t, chif::rectpacker::Rect> rect_lookup;
		struct Bitmap
		{
			int width;
			int height;
		...
		static chif::unordered_map<int32_t, Bitmap> bitmap_lookup;
		union GlyphHash
		{
			struct
			{
		...
		static chif::unordered_set<uint32_t> pendingGlyphs;
		...
		static thread_local chif::vector<FontVertex> vertexList;
		...
		ParseStatus ParseText(const char* text, size_t text_length, const Params& params)
		{
			...
			chif::helper::StringConvert(char_temp_buffer, wchar_temp_buffer);
	...
	void LoadShaders()
	{
		chif::renderer::LoadShader(ShaderStage::VS, vertexShader, "fontVS.cso");
		chif::renderer::LoadShader(ShaderStage::PS, pixelShader, "fontPS.cso");
		...
		for (int d = 0; d < DEPTH_TEST_MODE_COUNT; ++d)
		{
			...
			chif::graphics::GetDevice()->CreatePipelineState(&desc, &PSO[d]);
		}
	}
	void Initialize()
	{
		...
		static chif::eventhandler::Handle handle1 = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
		LoadShaders();

		chif::backlog::post("chif::font Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
	...
	void UpdateAtlas(float upscaling)
	{
		...
		if (!pendingGlyphs.empty())
		{
			...
			static thread_local chif::rectpacker::State packer;
			packer.clear();
			...
			if (packer.pack(4096))
			{
				...
				chif::vector<uint8_t> atlas(size_t(atlasWidth) * size_t(atlasHeight));
				...
				chif::texturehelper::CreateTexture(texture, atlas.data(), atlasWidth, atlasHeight, Format::R8_UNORM);
				GetDevice()->SetName(&texture, "chif::font::texture");
			}
			else
			{
	...
	Cursor Draw_internal(const T* text, size_t text_length, const Params& params, CommandList cmd)
	{
		...
		if (status.quadCount > 0)
		{
			...
			{
				// Asserts will check that a proper canvas was set for this cmd with chif::image::SetCanvas()
	...
	void SetCanvas(const chif::Canvas& current_canvas)
	{
