...
namespace chif::scene
{
	...
	struct TransformComponent
	{
		...
		XMFLOAT4X4 world = chif::math::IDENTITY_MATRIX;
	...
	{
		chif::ecs::Entity parentID = chif::ecs::INVALID_ENTITY;
	...
	struct MaterialComponent
	{
		...
		inline static const chif::vector<std::string> shaderTypeDefines[] = {
			{}, // SHADERTYPE_PBR,
		...
		chif::enums::STENCILREF engineStencilRef = chif::enums::STENCILREF_DEFAULT;
		uint8_t userStencilRef = 0;
		chif::enums::BLENDMODE userBlendMode = chif::enums::BLENDMODE_OPAQUE;
		...
		chif::graphics::ShadingRate shadingRate = chif::graphics::ShadingRate::RATE_1X1;
		...
		struct TextureMap
		{
			...
			uint32_t uvset = 0;
			const chif::graphics::GPUResource* GetGPUResource() const
			{
		...
		inline chif::enums::BLENDMODE GetBlendMode() const { if (userBlendMode == chif::enums::BLENDMODE_OPAQUE && (GetFilterMask() & chif::enums::FILTER_TRANSPARENT)) return chif::enums::BLENDMODE_ALPHA; else return userBlendMode; }
		...
		void WriteTextures(const chif::graphics::GPUResource** dest, int count) const;

		// Returns the bitwise OR of all the chif::enums::FILTER flags applicable to this material
		uint32_t GetFilterMask() const;

		chif::resourcemanager::Flags GetTextureSlotResourceFlags(TEXTURESLOT slot);
	...
	struct MeshComponent
	{
		...
		chif::vector<XMFLOAT3> vertex_positions;
		chif::vector<XMFLOAT3> vertex_normals;
		chif::vector<XMFLOAT4> vertex_tangents;
		chif::vector<XMFLOAT2> vertex_uvset_0;
		chif::vector<XMFLOAT2> vertex_uvset_1;
		chif::vector<XMUINT4> vertex_boneindices;
		chif::vector<XMFLOAT4> vertex_boneweights;
		chif::vector<XMUINT4> vertex_boneindices2;
		chif::vector<XMFLOAT4> vertex_boneweights2;
		chif::vector<XMFLOAT2> vertex_atlas;
		chif::vector<uint32_t> vertex_colors;
		chif::vector<uint8_t> vertex_windweights;
		...
		struct MeshSubset
		{
			...
			chif::ecs::Entity materialID = chif::ecs::INVALID_ENTITY;
		...
		chif::vector<MeshSubset> subsets;

		float tessellationFactor = 0.0f;
		chif::ecs::Entity armatureID = chif::ecs::INVALID_ENTITY;

		struct MorphTarget
		{
			chif::vector<XMFLOAT3> vertex_positions;
			chif::vector<XMFLOAT3> vertex_normals;
			chif::vector<uint32_t> sparse_indices_positions; // optional, these can be used to target vertices indirectly
			chif::vector<uint32_t> sparse_indices_normals; // optional, these can be used to target vertices indirectly
			float weight = 0;
		...
		chif::vector<MorphTarget> morph_targets;

		uint32_t subsets_per_lod = 0; // this needs to be specified if there are multiple LOD levels

		// Non-serialized attributes:
		chif::primitive::AABB aabb;
		chif::graphics::GPUBuffer generalBuffer; // index buffer + all static vertex buffers
		chif::graphics::GPUBuffer streamoutBuffer; // all dynamic vertex buffers
		struct BufferView
		{
		...
		chif::vector<chif::graphics::RaytracingAccelerationStructure> BLASes; // one BLAS per LOD
		enum BLAS_STATE
		{
		...
		chif::vector<chif::primitive::AABB> bvh_leaf_aabbs;
		chif::BVH bvh;
		...
		chif::vector<SubsetClusterRange> cluster_ranges;
		...
		inline chif::graphics::IndexBufferFormat GetIndexFormat() const { return chif::graphics::GetIndexBufferFormat((uint32_t)vertex_positions.size()); }
		inline size_t GetIndexStride() const { return GetIndexFormat() == chif::graphics::IndexBufferFormat::UINT32 ? sizeof(uint32_t) : sizeof(uint16_t); }
		inline bool IsSkinned() const { return armatureID != chif::ecs::INVALID_ENTITY; }
		...
		chif::primitive::Sphere GetBoundingSphere() const;
		...
		struct Vertex_POS16
		{
			...
			constexpr void FromFULL(const chif::primitive::AABB& aabb, XMFLOAT3 pos, uint8_t wind)
			{
				pos = chif::math::InverseLerp(aabb._min, aabb._max, pos); // UNORM remap
				x = uint16_t(pos.x * 65535.0f);
				y = uint16_t(pos.y * 65535.0f);
				z = uint16_t(pos.z * 65535.0f);
				w = uint16_t((float(wind) / 255.0f) * 65535.0f);
			}
			inline XMVECTOR LoadPOS(const chif::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = GetPOS(aabb);
				return XMLoadFloat3(&v);
			}
			constexpr XMFLOAT3 GetPOS(const chif::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = XMFLOAT3(
					float(x) / 65535.0f,
					float(y) / 65535.0f,
				...
				return chif::math::Lerp(aabb._min, aabb._max, v);
			}
			constexpr uint8_t GetWind() const
			{
				return uint8_t((float(w) / 65535.0f) * 255);
			}
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R16G16B16A16_UNORM;
		};
		struct Vertex_POS32
		{
			float x = 0;
			...
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R32G32B32_FLOAT;
		};
		struct Vertex_POS32W
		{
			...
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R32G32B32A32_FLOAT;
		};
		chif::graphics::Format position_format = Vertex_POS16::FORMAT; // CreateRenderData() will choose the appropriate format
		...
		struct Vertex_TEX
		{
			...
			{
				x = uint16_t(chif::math::InverseLerp(uv_range_min.x, uv_range_max.x, uv.x) * 65535.0f);
				y = uint16_t(chif::math::InverseLerp(uv_range_min.y, uv_range_max.y, uv.y) * 65535.0f);
			}
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R16G16_UNORM;
		};
		struct Vertex_UVS
		{
			Vertex_TEX uv0;
		...
		{
			uint32_t color = 0;
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R8G8B8A8_UNORM;
		};
		struct Vertex_NOR
		{
			int8_t x = 0;
			...
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R8G8B8A8_SNORM;
		};
		struct Vertex_TAN
		{
			int8_t x = 0;
	...
	struct ObjectComponent
	{
		...
		chif::graphics::Texture lightmap_render;
		chif::graphics::Texture lightmap;
		mutable uint32_t lightmapIterationCount = 0;
		chif::graphics::GPUBuffer vb_ao;
		int vb_ao_srv = -1;
		chif::graphics::GPUBuffer wetmap;
		...
		{
			uint8_t value = 0;
			static constexpr chif::graphics::Format FORMAT = chif::graphics::Format::R8_UNORM;
		};
	};
	...
	struct SoftBodyPhysicsComponent
	{
		...
		chif::vector<uint32_t> physicsIndices; // physics vertex connectivity
		chif::vector<uint32_t> physicsToGraphicsVertexMapping; // maps graphics vertex index to physics vertex index of the same position
		chif::vector<float> weights; // weight per physics vertex controlling the mass. (0: disable weight (no physics, only animation), 1: default weight)

		// Non-serialized attributes:
		std::shared_ptr<void> physicsobject = nullptr; // You can set to null to recreate the physics object the next time phsyics system will be running.
		XMFLOAT4X4 worldMatrix = chif::math::IDENTITY_MATRIX;
		uint32_t gpuBoneOffset = 0;
		chif::vector<ShaderTransform> boneData; // simulated soft body nodes as bone matrices that can be fed into skinning
		chif::primitive::AABB aabb;
	...
	struct ArmatureComponent
	{
		...
		chif::vector<chif::ecs::Entity> boneCollection;
		chif::vector<XMFLOAT4X4> inverseBindMatrices;

		// Non-serialized attributes:
		chif::primitive::AABB aabb;
		uint32_t gpuBoneOffset = 0;
		chif::vector<ShaderTransform> boneData;
	...
	struct LightComponent
	{
		...
		chif::vector<float> cascade_distances = { 8,80,800 };
		chif::vector<std::string> lensFlareNames;
		...
		chif::vector<chif::Resource> lensFlareRimTextures;
		...
		inline void BackCompatSetEnergy(float energy)
		{
			switch (type)
			{
			case chif::scene::LightComponent::POINT:
				intensity = energy * 20;
				break;
			case chif::scene::LightComponent::SPOT:
	...
	struct EnvironmentProbeComponent
	{
		...
		chif::Resource resource; // if texture is coming from an asset
		XMFLOAT3 position;
		float range;
	...
	struct DecalComponent
	{
		...
		chif::Resource texture;
		chif::Resource normal;
		chif::Resource surfacemap;
		chif::Resource displacementmap;
	...
	struct AnimationDataComponent
	{
		...
		chif::vector<float> keyframe_times;
		chif::vector<float> keyframe_data;
	...
	struct AnimationComponent
	{
		...
		struct AnimationChannel
		{
			...
			chif::ecs::Entity target = chif::ecs::INVALID_ENTITY;
			int samplerIndex = -1;
			int retargetIndex = -1;
		...
		struct AnimationSampler
		{
			...
			chif::ecs::Entity data = chif::ecs::INVALID_ENTITY;
		...
		{
			chif::ecs::Entity source = chif::ecs::INVALID_ENTITY;
			XMFLOAT4X4 dstRelativeMatrix = chif::math::IDENTITY_MATRIX;
			XMFLOAT4X4 srcRelativeParentMatrix = chif::math::IDENTITY_MATRIX;
		};

		chif::vector<AnimationChannel> channels;
		chif::vector<AnimationSampler> samplers;
		chif::vector<RetargetSourceData> retargets;

		// Non-serialzied attributes:
		chif::vector<float> morph_weights_temp;
		float last_update_time = 0;

		// Root Motion
		XMFLOAT3 rootTranslationOffset;
		XMFLOAT4 rootRotationOffset;
		chif::ecs::Entity rootMotionBone;
		...
		inline chif::ecs::Entity GetRootMotionBone() const { return rootMotionBone; }
		inline void SetRootMotionBone(chif::ecs::Entity _rootMotionBone) { rootMotionBone = _rootMotionBone; }
	...
	struct WeatherComponent
	{
		...
		chif::Ocean::OceanParameters oceanParameters;
		...
		chif::Resource skyMap;
		chif::Resource colorGradingMap;
		chif::Resource volumetricCloudsWeatherMapFirst;
		chif::Resource volumetricCloudsWeatherMapSecond;
	...
	struct VideoComponent
	{
		...
		std::string filename;
		chif::Resource videoResource;
		chif::video::VideoInstance videoinstance;
	...
	struct InverseKinematicsComponent
	{
		...
		chif::ecs::Entity target = chif::ecs::INVALID_ENTITY; // which entity to follow (must have a transform component)
	...
	struct SpringComponent
	{
		...
		chif::vector<SpringComponent*> children;
		chif::ecs::Entity entity;
	...
	struct ColliderComponent
	{
		...
		chif::primitive::Capsule capsule;
		chif::primitive::Plane plane;
		uint32_t layerMask = ~0u;
	...
	struct ScriptComponent
	{
		...
		// Non-serialized attributes:
		chif::vector<uint8_t> script; // compiled script binary data
		chif::Resource resource;
	...
	struct ExpressionComponent
	{
		...
		struct Expression
		{
			...
			chif::vector<MorphTargetBinding> morph_target_bindings;
		...
		chif::vector<Expression> expressions;
	...
	struct HumanoidComponent
	{
		...
		chif::ecs::Entity bones[size_t(HumanoidBone::Count)] = {};
		...
		chif::vector<RagdollBodypart> ragdoll_bodyparts;
		chif::primitive::AABB ragdoll_bounds;
	...
	struct MetadataComponent
	{
		...
		{
			chif::unordered_map<std::string, size_t> lookup; // name -> value hash lookup
			chif::vector<std::string> names; // ordered names
			chif::vector<T> values; // ordered values
	...
	struct CharacterComponent
	{
		...
		chif::ecs::Entity humanoidEntity = chif::ecs::INVALID_ENTITY;
		chif::ecs::Entity left_foot = chif::ecs::INVALID_ENTITY;
		chif::ecs::Entity right_foot = chif::ecs::INVALID_ENTITY;
		float root_offset = 0;
		bool foot_placement_enabled = true;
		chif::PathQuery pathquery; // completed
		chif::vector<chif::ecs::Entity> animations;
		chif::ecs::Entity currentAnimation = chif::ecs::INVALID_ENTITY;
		float anim_amount = 1;
		...
		struct PathfindingThreadContext
		{
			...
			chif::PathQuery pathquery_work; // working
		...
		const chif::VoxelGrid* voxelgrid = nullptr;
		...
		void AddAnimation(chif::ecs::Entity entity);
		void PlayAnimation(chif::ecs::Entity entity);
		void StopAnimation();
		...
		chif::primitive::Capsule GetCapsule() const;
		...
		void SetPathGoal(const XMFLOAT3& goal, const chif::VoxelGrid* voxelgrid);
