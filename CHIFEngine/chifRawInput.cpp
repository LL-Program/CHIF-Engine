...
namespace chif::input::rawinput
{
	...
	chif::vector<uint8_t*> input_messages;
	...
	void ParseRawInputBlock(const RAWINPUT& raw)
	{
		...
		else if (raw.header.dwType == RIM_TYPEHID)
		{
			...
			if (!internal_controller.is_xinput) // xinput enabled controller will be handled by xinput API
			{
				...
				chif::input::ControllerState& controller = internal_controller.state;

				for (ULONG i = 0; i < numberOfButtons; i++)
				{
					int button = usage[i] - pButtonCaps->Range.UsageMin;
					controller.buttons |= 1 << (button + chif::input::GAMEPAD_BUTTON_1 - chif::input::GAMEPAD_RANGE_START - 1);
				}
				...
				for (USHORT i = 0; i < Caps.NumberInputValueCaps; i++)
				{
					...
					case 0x39:  // Hat Switch
					{
						...
						case POV_UPRIGHT:
						case POV_LEFTUP:
							controller.buttons |= 1 << (chif::input::GAMEPAD_BUTTON_UP - chif::input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_UPRIGHT:
						case POV_RIGHTDOWN:
							controller.buttons |= 1 << (chif::input::GAMEPAD_BUTTON_RIGHT - chif::input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_RIGHTDOWN:
						case POV_DOWNLEFT:
							controller.buttons |= 1 << (chif::input::GAMEPAD_BUTTON_DOWN - chif::input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_DOWNLEFT:
						case POV_LEFTUP:
							controller.buttons |= 1 << (chif::input::GAMEPAD_BUTTON_LEFT - chif::input::GAMEPAD_RANGE_START - 1);
							break;
						}
					}
					break;
	...
	void Update()
	{
		keyboard = chif::input::KeyboardState();
		mouse = chif::input::MouseState();
		for (auto& internal_controller : controllers)
		{
			internal_controller.state = chif::input::ControllerState();
		}
