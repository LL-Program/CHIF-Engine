...
namespace chif::scene
{
	...
	void MaterialComponent::WriteShaderMaterial(ShaderMaterial* dest) const
	{
		...
		material.options_stencilref |= chif::renderer::CombineStencilrefs(engineStencilRef, userStencilRef) << 24u;
		...
		{
			material.sampler_descriptor = device->GetDescriptorIndex(chif::renderer::GetSampler(chif::enums::SAMPLER_OBJECTSHADER));
		}
		else
		{
	...
	void MaterialComponent::WriteTextures(const chif::graphics::GPUResource** dest, int count) const
	{
	...
	{
		if (IsCustomShader() && customShaderID < (int)chif::renderer::GetCustomShaders().size())
		{
			auto& customShader = chif::renderer::GetCustomShaders()[customShaderID];
	...
	chif::resourcemanager::Flags MaterialComponent::GetTextureSlotResourceFlags(TEXTURESLOT slot)
	{
		chif::resourcemanager::Flags flags = chif::resourcemanager::Flags::NONE;
		if (!IsPreferUncompressedTexturesEnabled())
		{
			flags |= chif::resourcemanager::Flags::IMPORT_BLOCK_COMPRESSED;
		}
		if (!IsTextureStreamingDisabled())
		{
			flags |= chif::resourcemanager::Flags::STREAMING;
		}
		switch (slot)
		{
		case NORMALMAP:
		case CLEARCOATNORMALMAP:
			flags |= chif::resourcemanager::Flags::IMPORT_NORMALMAP;
			break;
		default:
			break;
		}
	...
	void MaterialComponent::CreateRenderData(bool force_recreate)
	{
		...
		for (uint32_t slot = 0; slot < TEXTURESLOT_COUNT; ++slot)
		{
			...
			{
				chif::resourcemanager::Flags flags = GetTextureSlotResourceFlags(TEXTURESLOT(slot));
				textureslot.resource = chif::resourcemanager::Load(textureslot.name, flags);
			}
		}
	}
	uint32_t MaterialComponent::GetStencilRef() const
	{
		return chif::renderer::CombineStencilrefs(engineStencilRef, userStencilRef);
	}
	...
	void MeshComponent::CreateRenderData()
	{
		...
		for (size_t i = 0; i < vertex_positions.size(); ++i)
		{
			...
			_min = chif::math::Min(_min, pos);
			_max = chif::math::Max(_max, pos);
		}
		aabb = AABB(_min, _max);
		...
		if (!vertex_uvset_0.empty() || !vertex_uvset_1.empty())
		{
			...
			{
				uv_range_max = chif::math::Max(uv_range_max, uv0_stream[i]);
				uv_range_max = chif::math::Max(uv_range_max, uv1_stream[i]);
				uv_range_min = chif::math::Min(uv_range_min, uv0_stream[i]);
				uv_range_min = chif::math::Min(uv_range_min, uv1_stream[i]);
			}
		}
		...
		chif::vector<ShaderCluster> clusters;
		chif::vector<ShaderClusterBounds> cluster_bounds;
	...
	void MeshComponent::BuildBVH()
	{
		...
		for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset; ++subsetIndex)
		{
			...
			for (uint32_t triangleIndex = 0; triangleIndex < triangleCount; ++triangleIndex)
			{
				...
				AABB aabb = chif::primitive::AABB(chif::math::Min(p0, chif::math::Min(p1, p2)), chif::math::Max(p0, chif::math::Max(p1, p2)));
	...
	void MeshComponent::ComputeNormals(COMPUTE_NORMALS compute)
	{
		...
		if (compute != COMPUTE_NORMALS_SMOOTH_FAST)
		{
			...
			chif::vector<uint32_t> newIndexBuffer;
			chif::vector<XMFLOAT3> newPositionsBuffer;
			chif::vector<XMFLOAT3> newNormalsBuffer;
			chif::vector<XMFLOAT2> newUV0Buffer;
			chif::vector<XMFLOAT2> newUV1Buffer;
			chif::vector<XMFLOAT2> newAtlasBuffer;
			chif::vector<XMUINT4> newBoneIndicesBuffer;
			chif::vector<XMFLOAT4> newBoneWeightsBuffer;
			chif::vector<uint32_t> newColorsBuffer;
		...
		case MeshComponent::COMPUTE_NORMALS_SMOOTH:
		{
			...
			for (size_t i = 0; i < vertex_positions.size(); i++)
			{
				...
				for (size_t ind = 0; ind < indices.size() / 3; ++ind)
				{
					...
					bool match_pos0 =
						chif::math::float_equal(v_search_pos.x, v0.x) &&
						chif::math::float_equal(v_search_pos.y, v0.y) &&
						chif::math::float_equal(v_search_pos.z, v0.z);

					bool match_pos1 =
						chif::math::float_equal(v_search_pos.x, v1.x) &&
						chif::math::float_equal(v_search_pos.y, v1.y) &&
						chif::math::float_equal(v_search_pos.z, v1.z);

					bool match_pos2 =
						chif::math::float_equal(v_search_pos.x, v2.x) &&
						chif::math::float_equal(v_search_pos.y, v2.y) &&
						chif::math::float_equal(v_search_pos.z, v2.z);
				...
				for (uint32_t i = 0; i < subset.indexCount - 1; i++)
				{
					...
					for (uint32_t j = i + 1; j < subset.indexCount; j++)
					{
						...
						const bool duplicated_pos =
							chif::math::float_equal(p0.x, p1.x) &&
							chif::math::float_equal(p0.y, p1.y) &&
							chif::math::float_equal(p0.z, p1.z);

						const bool duplicated_uv0 =
							chif::math::float_equal(u00.x, u01.x) &&
							chif::math::float_equal(u00.y, u01.y);

						const bool duplicated_uv1 =
							chif::math::float_equal(u10.x, u11.x) &&
							chif::math::float_equal(u10.y, u11.y);

						const bool duplicated_atl =
							chif::math::float_equal(at0.x, at1.x) &&
							chif::math::float_equal(at0.y, at1.y);
	...
	size_t MeshComponent::GetMemoryUsageBVH() const
	{
		return
			...
			bvh_leaf_aabbs.size() * sizeof(chif::primitive::AABB);
	}
	...
	void ObjectComponent::SaveLightmap()
	{
		...
		if (lightmap.IsValid() && has_flag(lightmap.desc.bind_flags, BindFlag::UNORDERED_ACCESS))
		{
			...
			bool success = chif::helper::saveTextureToMemory(lightmap, lightmapTextureData);
			assert(success);

#ifdef OPEN_IMAGE_DENOISE
			if (success)
			{
				chif::vector<uint8_t> texturedata_dst(lightmapTextureData.size());
				...
				size_t height = (size_t)lightmapHeight;
				{
					...
					{
						chif::backlog::post(std::string("[OpenImageDenoise error] ") + errorMessage);
					}
					else
					{
			...
			chif::texturehelper::CreateTexture(lightmap, lightmapTextureData.data(), lightmapWidth, lightmapHeight, lightmap.desc.format);
			chif::graphics::GetDevice()->SetName(&lightmap, "lightmap");
		}
	}
	void ObjectComponent::CompressLightmap()
	{
		if (IsLightmapDisableBlockCompression())
		{
			...
			chif::vector<uint8_t> packed_data;
		...
		else
		{
			...
			chif::renderer::BlockCompress(lightmap, bc6tex, cmd);
			chif::helper::saveTextureToMemory(bc6tex, lightmapTextureData); // internally waits for GPU completion
			lightmap.desc = desc;
		}
	}
	...
	void EnvironmentProbeComponent::CreateRenderData()
	{
		...
		{
			resource = chif::resourcemanager::Load(textureName);
		}
		if (resource.IsValid())
		{
		...
		resolution = chif::math::GetNextPowerOfTwo(resolution);
	...
	AnimationComponent::AnimationChannel::PathDataType AnimationComponent::AnimationChannel::GetPathDataType() const
	{
		switch (path)
		{
		case chif::scene::AnimationComponent::AnimationChannel::Path::TRANSLATION:
			return PathDataType::Float3;
		case chif::scene::AnimationComponent::AnimationChannel::Path::ROTATION:
			return PathDataType::Float4;
		case chif::scene::AnimationComponent::AnimationChannel::Path::SCALE:
			return PathDataType::Float3;
		case chif::scene::AnimationComponent::AnimationChannel::Path::WEIGHTS:
			return PathDataType::Weights;

		case chif::scene::AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
			return PathDataType::Float3;
		case chif::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
		case chif::scene::AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
		case chif::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
		case chif::scene::AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
			return PathDataType::Float;

		case chif::scene::AnimationComponent::AnimationChannel::Path::SOUND_PLAY:
		case chif::scene::AnimationComponent::AnimationChannel::Path::SOUND_STOP:
			return PathDataType::Event;
		case chif::scene::AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
			return PathDataType::Float;

		case chif::scene::AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
			return PathDataType::Float;

		case chif::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
		case chif::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
		case chif::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
			return PathDataType::Float;
		case chif::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
			return PathDataType::Float2;

		case chif::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_PLAY:
		case chif::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_STOP:
			return PathDataType::Event;

		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
			return PathDataType::Float4;
		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
		case chif::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
			return PathDataType::Float;
	...
	void SoftBodyPhysicsComponent::CreateFromMesh(MeshComponent& mesh)
	{
		...
		if (physicsIndices.empty())
		{
			...
			if (pinning_required)
			{
				...
				chif::vector<XMFLOAT4> vertices(mesh.vertex_positions.size());
				...
				chif::vector<uint32_t> shadow_indices(source.size());
			...
			chif::unordered_map<uint32_t, size_t> physicsVertices;
			...
			chif::jobsystem::Dispatch(ctx, (uint32_t)mesh.vertex_positions.size(), 64, [&](chif::jobsystem::JobArgs args) {
				...
				for (size_t physicsInd = 0; physicsInd < physicsToGraphicsVertexMapping.size(); ++physicsInd)
				{
					...
					const float dist = chif::math::DistanceSquared(position, position2);
	...
	void CameraComponent::Lerp(const CameraComponent& a, const CameraComponent& b, float t)
	{
		...
		width = chif::math::Lerp(a.width, b.width, t);
		height = chif::math::Lerp(a.height, b.height, t);
		zNearP = chif::math::Lerp(a.zNearP, b.zNearP, t);
		zFarP = chif::math::Lerp(a.zFarP, b.zFarP, t);
		fov = chif::math::Lerp(a.fov, b.fov, t);
		focal_length = chif::math::Lerp(a.focal_length, b.focal_length, t);
		aperture_size = chif::math::Lerp(a.aperture_size, b.aperture_size, t);
		aperture_shape = chif::math::Lerp(a.aperture_shape, b.aperture_shape, t);
	}

	void ScriptComponent::CreateFromFile(const std::string& filename)
	{
		this->filename = filename;
		resource = chif::resourcemanager::Load(filename);
	...
	{
		_flags |= PLAYING;
		chif::audio::Play(&soundinstance);
	}
	void SoundComponent::Stop()
	{
		_flags &= ~PLAYING;
		chif::audio::Stop(&soundinstance);
	}
	void SoundComponent::SetLooped(bool value)
	{
		...
		{
			_flags |= LOOPED;
			chif::audio::CreateSoundInstance(&soundResource.GetSound(), &soundinstance);
		}
		else
		{
			_flags &= ~LOOPED;
			chif::audio::ExitLoop(&soundinstance);
		}
	}
	...
	{
		return chif::math::Lerp(position_prev, position, alpha);
	}
	...
	void CharacterComponent::SetPathGoal(const XMFLOAT3& goal, const chif::VoxelGrid* voxelgrid)
	{
		this->goal = goal;
