...
namespace chif::audio
{
	...
	struct AudioInternal
	{
		...
		AudioInternal()
		{
			...
			if (!SUCCEEDED(hr))
			{
				...
				chif::backlog::post(ss.str(), chif::backlog::LogLevel::Error);
				return;
			}
			...
			success = true;
			chif::backlog::post("chif::audio Initialized [XAudio2] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		}
		~AudioInternal()
		{
	...
	struct SoundInstanceInternal : public IXAudio2VoiceCallback
	{
		...
		chif::vector<float> outputMatrix;
		chif::vector<float> channelAzimuths;
	...
	bool CreateSound(const std::string& filename, Sound* sound)
	{
		...
		bool success = chif::helper::FileRead(filename, filedata);
		if (!success)
		{
			return false;
		}
...
namespace chif::audio
{
	...
	struct AudioInternal{
		...
		AudioInternal(){
			...
			success = true;
			chif::backlog::post("chif::audio Initialized [FAudio] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		}
		~AudioInternal(){
			if(reverbSubmix != nullptr)
