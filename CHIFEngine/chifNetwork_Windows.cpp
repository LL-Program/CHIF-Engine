...
namespace chif::network
{
	struct NetworkInternal
	{
		...
		NetworkInternal()
		{
			...
			if (result)
			{
				...
				chif::backlog::post("chif::network Initialization FAILED with error: " + std::to_string(error));
				assert(0);
			}

			chif::backlog::post("chif::network Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		}
		~NetworkInternal()
		{
			WSACleanup();
	...
	bool CreateSocket(Socket* sock)
	{
		...
		{
			int error = WSAGetLastError();
			chif::backlog::post("chif::network error in CreateSocket: " + std::to_string(error));
			return false;
		}
	...
	bool Send(const Socket* sock, const Connection* connection, const void* data, size_t dataSize)
	{
		if (socket != nullptr && sock->IsValid())
		{
			...
			{
				int error = WSAGetLastError();
				chif::backlog::post("chif::network error in Send: " + std::to_string(error));
				return false;
			}
	...
	bool ListenPort(const Socket* sock, uint16_t port)
	{
		if (socket != nullptr && sock->IsValid())
		{
			...
			{
				int error = WSAGetLastError();
				chif::backlog::post("chif::network error in ListenPort: " + std::to_string(error));
				return false;
			}
	...
	bool CanReceive(const Socket* sock, long timeout_microseconds)
	{
		if (socket != nullptr && sock->IsValid())
		{
			...
			{
				int error = WSAGetLastError();
				chif::backlog::post("chif::network error in CanReceive: " + std::to_string(error));
				assert(0);
				return false;
			}
	...
	bool Receive(const Socket* sock, Connection* connection, void* data, size_t dataSize)
	{
		if (socket != nullptr && sock->IsValid())
		{
			...
			{
				int error = WSAGetLastError();
				chif::backlog::post("chif::network error in Receive: " + std::to_string(error));
				return false;
			}
