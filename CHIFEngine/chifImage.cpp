...
namespace chif::image
{
	...
	void Draw(const Texture* texture, const Params& params, CommandList cmd)
	{
		...
		if (params.angular_softness_outer_angle > 0)
		{
			...
			image.angular_softness_mad = chif::math::pack_half2(lightAngleScale, lightAngleOffset);
		}
			...
			if (params.isCornerRoundingEnabled())
			{
				...
				for (int i = 0; i < arraysize(params.corners_rounding); ++i)
				{
					...
					for (int j = 0; j < rounding.segments; ++j)
					{
						...
						XMVECTOR bezier = chif::math::GetQuadraticBezierPos(start, B, end, t);
		...
		{
			stencilRef = chif::renderer::CombineStencilrefs(STENCILREF_EMPTY, (uint8_t)stencilRef);
		}
	...
	void LoadShaders()
	{
		chif::renderer::LoadShader(ShaderStage::VS, vertexShader, "imageVS.cso");
		chif::renderer::LoadShader(ShaderStage::PS, pixelShader, "imagePS.cso");
	...
	void Initialize()
	{
		...
		chif::backlog::post("chif::image Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
