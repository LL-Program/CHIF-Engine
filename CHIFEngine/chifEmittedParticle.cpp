...
namespace chif
{
	...
	void EmittedParticleSystem::CreateSelfBuffers()
	{
		...
		if (particleBuffer.desc.size < MAX_PARTICLES * sizeof(Particle))
		{
			...
			primitiveBuffer = chif::renderer::GetIndexBufferForQuads(MAX_PARTICLES);
		}
	...
	void EmittedParticleSystem::UpdateCPU(const TransformComponent& transform, float dt)
	{
		...
		if ((uint)emit > 0)
		{
			...
			location.color = chif::Color::White();
		...
		const uint32_t oldest_stat_index = chif::graphics::GetDevice()->GetBufferIndex();
	...
	void EmittedParticleSystem::Burst(int num, const XMFLOAT3& position, const chif::Color& color)
	{
		if (IsPaused() || num <= 0)
			return;
	...
	void EmittedParticleSystem::Burst(int num, const XMFLOAT4X4& transform, const chif::Color& color)
	{
		if (IsPaused() || num <= 0)
			return;
	...
	void EmittedParticleSystem::UpdateGPU(uint32_t instanceIndex, const MeshComponent* mesh, CommandList cmd) const
	{
		...
		if (!IsPaused() && dt > 0)
		{
			...
			cb.xEmitterRandomness = chif::random::GetRandom(0.0f, 1.0f);
			...
			if (IsSPHEnabled())
			{
				auto range = chif::profiler::BeginRangeGPU("SPH - Simulation", cmd);
		...
		if (IsSorted())
		{
			chif::gpusortlib::Sort(MAX_PARTICLES, distanceBuffer, counterBuffer, PARTICLECOUNTER_OFFSET_CULLEDCOUNT, culledIndirectionBuffer, cmd);
		}
	...
	void EmittedParticleSystem::Draw(const MaterialComponent& material, CommandList cmd, const PARTICLESHADERTYPE* shadertype_override) const
	{
		...
		if (chif::renderer::IsWireRender())
		{
			device->BindPipelineState(&PSO_wire, cmd);
		}
		else
		{
			const chif::enums::BLENDMODE blendMode = material.GetBlendMode();
	...
	namespace EmittedParticleSystem_Internal
	{
		void LoadShaders()
		{
			chif::renderer::LoadShader(ShaderStage::VS, vertexShader, "emittedparticleVS.cso");

			if (ALLOW_MESH_SHADER && chif::graphics::GetDevice()->CheckCapability(GraphicsDeviceCapability::MESH_SHADER))
			{
				chif::renderer::LoadShader(ShaderStage::MS, meshShader, "emittedparticleMS.cso");
			}

			chif::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT], "emittedparticlePS_soft.cso");
			chif::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_DISTORTION], "emittedparticlePS_soft_distortion.cso");
			chif::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SIMPLE], "emittedparticlePS_simple.cso");
			chif::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_LIGHTING], "emittedparticlePS_soft_lighting.cso");

			chif::renderer::LoadShader(ShaderStage::CS, kickoffUpdateCS, "emittedparticle_kickoffUpdateCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, finishUpdateCS, "emittedparticle_finishUpdateCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, emitCS, "emittedparticle_emitCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, emitCS_VOLUME, "emittedparticle_emitCS_volume.cso");
			chif::renderer::LoadShader(ShaderStage::CS, emitCS_FROMMESH, "emittedparticle_emitCS_FROMMESH.cso");
			chif::renderer::LoadShader(ShaderStage::CS, sphpartitionCS, "emittedparticle_sphpartitionCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, sphcellallocationCS, "emittedparticle_sphcellallocationCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, sphbinningCS, "emittedparticle_sphbinningCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, sphdensityCS, "emittedparticle_sphdensityCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, sphforceCS, "emittedparticle_sphforceCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, simulateCS, "emittedparticle_simulateCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING, "emittedparticle_simulateCS_SORTING.cso");
			chif::renderer::LoadShader(ShaderStage::CS, simulateCS_DEPTHCOLLISIONS, "emittedparticle_simulateCS_DEPTHCOLLISIONS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING_DEPTHCOLLISIONS, "emittedparticle_simulateCS_SORTING_DEPTHCOLLISIONS.cso");
	...
	void EmittedParticleSystem::Initialize()
	{
		...
		static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { EmittedParticleSystem_Internal::LoadShaders(); });
		EmittedParticleSystem_Internal::LoadShaders();

		chif::backlog::post("chif::EmittedParticleSystem Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}


	void EmittedParticleSystem::Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
			archive >> (uint32_t&)shaderType;
			chif::ecs::SerializeEntity(archive, meshID, seri);
