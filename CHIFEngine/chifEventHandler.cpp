...
namespace chif::eventhandler
{
	struct EventManager
	{
		// Note: list is used because events can also add events from within and that mustn't cause invalidation like vector
		chif::unordered_map<int, std::list<std::function<void(uint64_t)>*>> subscribers;
		chif::unordered_map<int, std::list<std::function<void(uint64_t)>>> subscribers_once;
		std::mutex locker;
	};
