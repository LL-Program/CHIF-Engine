...
namespace chif
{
	...
	{
	protected:
		std::unique_ptr<chif::graphics::GraphicsDevice> graphicsDevice;
		chif::eventhandler::Handle swapChainVsyncChangeEvent;
		...
		chif::graphics::Texture rendertarget;
	...
	public:
		...
		bool allow_hdr = true;
		chif::graphics::SwapChain swapChain;
		chif::Canvas canvas;
		chif::platform::window_type window;
		...
		void ActivatePath(RenderPath* component, float fadeSeconds = 0, chif::Color fadeColor = chif::Color(0, 0, 0, 255));
		...
		virtual void Compose(chif::graphics::CommandList cmd);

		// You need to call this before calling Run() or Initialize() if you want to render
		void SetWindow(chif::platform::window_type);
