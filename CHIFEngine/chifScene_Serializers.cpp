...
namespace chif::scene
{
	...
	{
		void Serialize(chif::Archive& archive, chif::ecs::EntitySerializer& seri) { /*this never serialized any data*/ }
	};

	void NameComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
	...
	void LayerComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> layerMask;
	...
	void TransformComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void HierarchyComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
	...
	void MaterialComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		...
		else
		{
			...
			archive << chif::helper::GetPathRelative(dir, textures[BASECOLORMAP].name);
			archive << chif::helper::GetPathRelative(dir, textures[SURFACEMAP].name);
			archive << chif::helper::GetPathRelative(dir, textures[NORMALMAP].name);
			archive << chif::helper::GetPathRelative(dir, textures[DISPLACEMENTMAP].name);

			if (archive.GetVersion() >= 24)
			{
				archive << chif::helper::GetPathRelative(dir, textures[EMISSIVEMAP].name);
			}

			if (archive.GetVersion() >= 28)
			{
				archive << chif::helper::GetPathRelative(dir, textures[OCCLUSIONMAP].name);
			...
			{
				archive << transmission;
				archive << chif::helper::GetPathRelative(dir, textures[TRANSMISSIONMAP].name);
				archive << textures[TRANSMISSIONMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 61)
			{
				archive << sheenColor;
				archive << sheenRoughness;
				archive << chif::helper::GetPathRelative(dir, textures[SHEENCOLORMAP].name);
				archive << chif::helper::GetPathRelative(dir, textures[SHEENROUGHNESSMAP].name);
				archive << textures[SHEENCOLORMAP].uvset;
				archive << textures[SHEENROUGHNESSMAP].uvset;

				archive << clearcoat;
				archive << clearcoatRoughness;
				archive << chif::helper::GetPathRelative(dir, textures[CLEARCOATMAP].name);
				archive << chif::helper::GetPathRelative(dir, textures[CLEARCOATROUGHNESSMAP].name);
				archive << chif::helper::GetPathRelative(dir, textures[CLEARCOATNORMALMAP].name);
				archive << textures[CLEARCOATMAP].uvset;
				archive << textures[CLEARCOATROUGHNESSMAP].uvset;
				archive << textures[CLEARCOATNORMALMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 68)
			{
				archive << chif::helper::GetPathRelative(dir, textures[SPECULARMAP].name);
				archive << textures[SPECULARMAP].uvset;
			}
			...
			if (seri.GetVersion() >= 2)
			{
				archive << anisotropy_strength;
				archive << anisotropy_rotation;
				archive << chif::helper::GetPathRelative(dir, textures[ANISOTROPYMAP].name);
				archive << textures[ANISOTROPYMAP].uvset;
			}

			if (seri.GetVersion() >= 3)
			{
				archive << chif::helper::GetPathRelative(dir, textures[TRANSPARENCYMAP].name);
				archive << textures[TRANSPARENCYMAP].uvset;
			}
			if (seri.GetVersion() >= 4)
			{
	...
	void MeshComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ImpostorComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ObjectComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void RigidBodyPhysicsComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void SoftBodyPhysicsComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		chif::vector<uint32_t> graphicsToPhysicsVertexMapping;
		...
		if (archive.IsReadMode())
		{
			...
			else
			{
				...
				chif::vector<float> weights2(graphicsToPhysicsVertexMapping.size());
	...
	void ArmatureComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void LightComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
			...
			chif::jobsystem::Execute(seri.ctx, [&](chif::jobsystem::JobArgs args) {
				...
				for (size_t i = 0; i < lensFlareNames.size(); ++i)
				{
					if (!lensFlareNames[i].empty())
					{
						...
						lensFlareRimTextures[i] = chif::resourcemanager::Load(lensFlareNames[i]);
					}
				}
			});
		}
		else
		{
			...
			chif::vector<std::string> lensFlareNamesRelative = lensFlareNames;
			if (!dir.empty())
			{
				for (size_t i = 0; i < lensFlareNamesRelative.size(); ++i)
				{
					chif::helper::MakePathRelative(dir, lensFlareNamesRelative[i]);
				}
			}
	...
	void CameraComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void EnvironmentProbeComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		else
		{
			...
			{
				archive << resolution;
				archive << chif::helper::GetPathRelative(dir, textureName);
			}
		}
	}
	void ForceFieldComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void DecalComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationDataComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void WeatherComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		...
		if (archive.IsReadMode())
		{
			...
			if (archive.GetVersion() >= 32)
			{
				...
				if (!skyMapName.empty())
				{
					...
					skyMap = chif::resourcemanager::Load(skyMapName);
				}
			}
			...
			if (archive.GetVersion() >= 62)
			{
				...
				if (!colorGradingMapName.empty())
				{
					...
					colorGradingMap = chif::resourcemanager::Load(colorGradingMapName, chif::resourcemanager::Flags::IMPORT_COLORGRADINGLUT);
				}
			}
			...
			if (archive.GetVersion() >= 86)
			{
				...
				if (!volumetricCloudsWeatherMapFirstName.empty())
				{
					...
					volumetricCloudsWeatherMapFirst = chif::resourcemanager::Load(volumetricCloudsWeatherMapFirstName);
				}
			}
			...
			if (archive.GetVersion() >= 88)
			{
				...
				if (!volumetricCloudsWeatherMapSecondName.empty())
				{
					...
					volumetricCloudsWeatherMapSecond = chif::resourcemanager::Load(volumetricCloudsWeatherMapSecondName);
				}
		...
		else
		{
			...
			if (archive.GetVersion() >= 32)
			{
				archive << chif::helper::GetPathRelative(dir, skyMapName);
			}
			if (archive.GetVersion() >= 40)
			{
				archive << windSpeed;
			...
			if (archive.GetVersion() >= 62)
			{
				archive << chif::helper::GetPathRelative(dir, colorGradingMapName);
			}
			...
			if (archive.GetVersion() >= 86)
			{
				archive << chif::helper::GetPathRelative(dir, volumetricCloudsWeatherMapFirstName);
			}

			if (archive.GetVersion() >= 88)
			{
				archive << chif::helper::GetPathRelative(dir, volumetricCloudsWeatherMapSecondName);
	...
	void SoundComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			chif::jobsystem::Execute(seri.ctx, [&](chif::jobsystem::JobArgs args) {
				if (!filename.empty())
				{
					...
					soundResource = chif::resourcemanager::Load(filename);
		...
		else
		{
			...
			archive << _flags;
			archive << chif::helper::GetPathRelative(dir, filename);
			archive << volume;
	...
	void VideoComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			chif::jobsystem::Execute(seri.ctx, [&](chif::jobsystem::JobArgs args) {
				...
				{
					filename = dir + filename;
					videoResource = chif::resourcemanager::Load(filename);
					chif::video::CreateVideoInstance(&videoResource.GetVideo(), &videoinstance);
				}
			});
		}
		else
	...
	void InverseKinematicsComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
	...
	void SpringComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ColliderComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ScriptComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
	...
	void ExpressionComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void HumanoidComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
	...
	void MetadataComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void CharacterComponent::Serialize(chif::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void Scene::Serialize(chif::Archive& archive)
	{
		chif::Timer timer;
		...
		chif::resourcemanager::ResourceSerializer resource_seri;
		if (archive.IsReadMode() && archive.GetVersion() >= 63)
		{
			chif::resourcemanager::Serialize_READ(archive, resource_seri);
			if (archive.GetVersion() >= 90)
			{
		...
		seri.ctx.priority = chif::jobsystem::Priority::Low; // serialization tasks will be low priority to not block rendering if scene loading is asynchronous
		...
		else
		{
			...
			ComponentManager<chif::primitive::AABB> aabbs_tmp; // no longer needed from serializer
		...
		chif::jobsystem::Wait(seri.ctx); // This is needed before emitter material fixup that is below, because material CreateRenderDatas might be pending!
		...
		if (archive.GetVersion() >= 90)
		{
			...
			else
			{
				...
				chif::resourcemanager::Serialize_WRITE(archive, seri.resource_registration);
	...
	void Scene::DDGI::Serialize(chif::Archive& archive)
	{
		...
		if (archive.IsReadMode())
		{
			...
			if(!data.empty())
			{
				...
				{
					chif::backlog::post("The serialized DDGI irradiance data structure is different from current version, discarding irradiance data.", chif::backlog::LogLevel::Warning);
				}
			}
			...
			if(!data.empty())
			{
				...
				{
					chif::backlog::post("The serialized DDGI probe offset structure is different from current version, discarding probe offset data.", chif::backlog::LogLevel::Warning);
				}
			}
		}
		else
		{
			...
			{
				bool success = chif::helper::saveTextureToMemory(color_texture, data);
				assert(success);
			}
			archive << data;
			...
			{
				bool success = chif::helper::saveTextureToMemory(depth_texture, data);
				assert(success);
			}
			archive << data;
			...
			{
				bool success = chif::helper::saveTextureToMemory(offset_texture, data);
				assert(success);
			}
			archive << data;
		}
		...
		else
		{
			...
			if (archive.IsReadMode())
			{
				...
				}
				{
					...
					if (component_exists)
					{
						auto component = chif::primitive::AABB(); // no longer needed to be serialized
						component.Serialize(archive, seri);
					}
				}
				{
