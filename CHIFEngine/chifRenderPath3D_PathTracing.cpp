...
namespace chif
{
	...
	void RenderPath3D_PathTracing::ResizeBuffers()
	{
		...
		chif::renderer::CreateLuminanceResources(luminanceResources, internalResolution);
		chif::renderer::CreateBloomResources(bloomResources, internalResolution);
	...
	void RenderPath3D_PathTracing::Update(float dt)
	{
		...
		if (sam == target)
		{
			if (!denoiserResult.IsValid() && !chif::jobsystem::IsBusy(denoiserContext))
			{
				//chif::helper::saveTextureToFile(denoiserAlbedo, "C:/PROJECTS/WickedEngine/Editor/_albedo.png");
				//chif::helper::saveTextureToFile(denoiserNormal, "C:/PROJECTS/WickedEngine/Editor/_normal.png");
				...
				if (chif::helper::saveTextureToMemory(traceResult, texturedata_src))
				{
					chif::helper::saveTextureToMemory(denoiserAlbedo, texturedata_albedo);
					chif::helper::saveTextureToMemory(denoiserNormal, texturedata_normal);

					texturedata_dst.resize(texturedata_src.size());

					chif::jobsystem::Execute(denoiserContext, [&](chif::jobsystem::JobArgs args) {
	...
	void RenderPath3D_PathTracing::Render() const
	{
		...
		if (sam < target)
		{
			...
			chif::jobsystem::Execute(ctx, [this, cmd](chif::jobsystem::JobArgs args) {
				...
				if (chif::renderer::GetRaytraceDebugBVHVisualizerEnabled())
				{
					RenderPassImage rp[] = {
					...
					chif::renderer::RayTraceSceneBVH(*scene, cmd);

					device->RenderPassEnd(cmd);
				}
				else
				{
					auto range = chif::profiler::BeginRangeGPU("Traced Scene", cmd);

					chif::renderer::RayTraceScene(
						*scene,
						traceResult,
						sam,
						cmd,
					...
					chif::profiler::EndRange(range); // Traced Scene
				}
		...
		chif::jobsystem::Execute(ctx, [this, cmd](chif::jobsystem::JobArgs args) {
			...
			chif::renderer::Postprocess_Lineardepth(traceDepth, rtLinearDepth, cmd);
			...
			// Composite other effects on top:
			{
				...
				// Clear to trace result:
				{
					...
					fx.enableFullScreen();
					fx.blendFlag = chif::enums::BLENDMODE_OPAQUE;
					if (denoiserResult.IsValid() && !chif::jobsystem::IsBusy(denoiserContext))
					{
						chif::image::Draw(&denoiserResult, fx, cmd);
					}
					else
					{
						chif::image::Draw(&traceResult, fx, cmd);
					}
					device->EventEnd(cmd);
				}
				...
				if (scene->weather.IsRealisticSky() && scene->weather.IsRealisticSkyAerialPerspective())
				{
					...
					chif::image::Params fx;
					fx.enableFullScreen();
					fx.blendFlag = chif::enums::BLENDMODE_PREMULTIPLIED;
				...
				if (getLightShaftsEnabled() && XMVectorGetX(XMVector3Dot(sunDirection, camera->GetAt())) > 0)
				{
					...
					chif::image::Params fx;
					fx.enableFullScreen();
					fx.blendFlag = chif::enums::BLENDMODE_ADDITIVE;
			...
			// GUI Background blurring:
			{
				...
				chif::renderer::Postprocess_Downsample4x(rtPostprocess, rtGUIBlurredBackground[0], cmd);
	...
	void RenderPath3D_PathTracing::Compose(CommandList cmd) const
	{
		...
		chif::image::Draw(&rtPostprocess, fx, cmd);
