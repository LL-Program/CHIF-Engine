...
namespace chif::gui
{
	...
	struct EventArgs
	{
		...
		chif::Color color;		// color value of color picker operation
	...
	struct Theme
	{
		// Reduced version of chif::image::Params, excluding position, alignment, etc.
		struct Image
		{
			inline static const chif::image::Params params; // prototype for default values
			XMFLOAT4 color = params.color;
			chif::enums::BLENDMODE blendFlag = params.blendFlag;
			chif::image::SAMPLEMODE sampleFlag = params.sampleFlag;
			chif::image::QUALITY quality = params.quality;
			bool background = params.isBackgroundEnabled();
			bool corner_rounding = params.isCornerRoundingEnabled();
			chif::image::Params::Rounding corners_rounding[arraysize(params.corners_rounding)];

			void Apply(chif::image::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const chif::image::Params& params)
			{
				color = params.color;
		...
		// Reduced version of chif::font::Params, excluding position, alignment, etc.
		struct Font
		{
			inline static const chif::font::Params params; // prototype for default values
			chif::Color color = params.color;
			chif::Color shadow_color = params.shadowColor;
			...
			void Apply(chif::font::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const chif::font::Params& params)
			{
				color = params.color;
		...
		chif::Color shadow_color = chif::Color::Shadow(); // shadow color for whole widget

		Image tooltipImage;
		Font tooltipFont;
		chif::Color tooltip_shadow_color = chif::Color::Shadow();
	};
	...
	{
	private:
		chif::vector<Widget*> widgets;
		bool focus = false;
		bool visible = true;
	public:

		void Update(const chif::Canvas& canvas, float dt);
		void Render(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const;
		...
		void SetColor(chif::Color color, int id = -1);
		void SetShadowColor(chif::Color color);
		void SetTheme(const Theme& theme, int id = -1);

		void ExportLocalization(chif::Localization& localization) const;
		void ImportLocalization(const chif::Localization& localization);
	};

	class Widget : public chif::scene::TransformComponent
	{
	private:
		int tooltipTimer = 0;
	protected:
		...
		chif::Color shadow_color = chif::Color::Shadow();
		WIDGETSTATE state = IDLE;
		float tooltip_shadow = 1; // shadow radius
		chif::Color tooltip_shadow_color = chif::Color::Shadow();
		mutable chif::Sprite tooltipSprite;
		mutable chif::SpriteFont tooltipFont;
		mutable chif::SpriteFont scripttipFont;
	...
	public:
		...
		chif::Color GetColor() const;
		float GetShadowRadius() const { return shadow; }
		void SetShadowRadius(float value) { shadow = value; }

		virtual void ResizeLayout() {};
		virtual void Update(const chif::Canvas& canvas, float dt);
		virtual void Render(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const;
		virtual void RenderTooltip(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const;
		...
		virtual void SetColor(chif::Color color, int id = -1);
		virtual void SetShadowColor(chif::Color color);
		virtual void SetImage(chif::Resource textureResource, int id = -1);
		virtual void SetTheme(const Theme& theme, int id = -1);
		virtual const char* GetWidgetTypeName() const { return "Widget"; }

		chif::Sprite sprites[WIDGETSTATE_COUNT];
		chif::SpriteFont font;

		XMFLOAT3 translation = XMFLOAT3(0, 0, 0);
		XMFLOAT3 scale = XMFLOAT3(1, 1, 1);

		chif::primitive::Hitbox2D hitBox;
		chif::graphics::Rect scissorRect;
		...
		void ApplyScissor(const chif::Canvas& canvas, const chif::graphics::Rect rect, chif::graphics::CommandList cmd, bool constrain_to_parent = true) const;
		chif::primitive::Hitbox2D GetPointerHitbox(bool constrained = true) const;

		chif::primitive::Hitbox2D active_area; // Pointer hitbox constrain area
		void HitboxConstrain(chif::primitive::Hitbox2D& hb) const;
		...
		virtual void ExportLocalization(chif::Localization& localization) const;
		virtual void ImportLocalization(const chif::Localization& localization);
	...
	public:
		...
		chif::SpriteFont font_description;
		void SetDescription(const std::string& desc) { font_description.SetText(desc); }

		void Update(const chif::Canvas& canvas, float dt) override;
		void Render(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const override;
	...
	public:
		...
		chif::Sprite sprites_knob[SCROLLBAR_STATE_COUNT];
		XMFLOAT2 knob_inset_border = {};

		void Update(const chif::Canvas& canvas, float dt) override;
		void Render(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const override;
		void SetColor(chif::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		static chif::SpriteFont font_input;
	...
	public:
		...
		chif::Sprite sprites_knob[WIDGETSTATE_COUNT];
		...
		void Update(const chif::Canvas& canvas, float dt) override;
		void Render(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const override;
		void RenderTooltip(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const override;
		void SetColor(chif::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		chif::vector<Item> items;

		chif::Color drop_color = chif::Color::Ghost();
		std::wstring invalid_selection_text;

		float GetDropOffset(const chif::Canvas& canvas) const;
		float GetDropX(const chif::Canvas& canvas) const;
		float GetItemOffset(const chif::Canvas& canvas, int index) const;
	public:
		void Create(const std::string& name);
		...
		chif::SpriteFont selected_font;

		void ExportLocalization(chif::Localization& localization) const override;
		void ImportLocalization(const chif::Localization& localization) override;
	...
	public:
		...
		void RenderTooltip(const chif::Canvas& canvas, chif::graphics::CommandList cmd) const override;
		void SetColor(chif::Color color, int id = -1) override;
		void SetShadowColor(chif::Color color) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	public:
		...
		chif::Color GetPickColor() const;
		void SetPickColor(chif::Color value);
	...
	protected:
		...
		chif::primitive::Hitbox2D GetHitbox_ListArea() const;
		chif::primitive::Hitbox2D GetHitbox_Item(int visible_count, int level) const;
		chif::primitive::Hitbox2D GetHitbox_ItemOpener(int visible_count, int level) const;
...
template<>
struct enable_bitmask_operators<chif::gui::Window::WindowControls> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<chif::gui::Window::AttachmentOptions> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<chif::gui::LocalizationEnabled> {
	static const bool enable = true;
};
