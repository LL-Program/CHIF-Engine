...
namespace chif::lua
{
	...
	void PostErrorMsg(lua_State* L)
	{
		...
		ss += str;
		chif::backlog::post(ss, chif::backlog::LogLevel::Error);
	...
	uint32_t AttachScriptParameters(std::string& script, const std::string& filename, uint32_t PID, const std::string& customparameters_prepend, const std::string& customparameters_append)
	{
		...
		dynamic_inject += "local function script_dir() return \"" + chif::helper::GetDirectoryFromPath(filepath) + "\" end;";
	...
	int Internal_DoBinaryFile(lua_State* L)
	{
		...
		{
			std::string filename = SGetString(L, 1);
			if (chif::lua::RunBinaryFile(filename))
			{
				return 0;
			}
			chif::lua::SError(L, "dobinaryfile(string filename): File could not be read!");
		}
		chif::lua::SError(L, "dobinaryfile(string filename): Not enough arguments!");
		return 0;
	}
	int Internal_CompileBinaryFile(lua_State* L)
	{
		...
		if (argc > 1)
		{
			...
			if (chif::lua::CompileFile(filename_src, data))
			{
				chif::helper::FileWrite(filename_dst, data.data(), data.size());
				return 0;
			}
			chif::lua::SError(L, "compilebinaryfile(string filename_src, filename_dst): Source file could not be read, or compilation failed!");
		}
		chif::lua::SError(L, "compilebinaryfile(string filename_src, filename_dst): Not enough arguments!");
		return 0;
	}

	chif::Application* editorApplication = nullptr;
	chif::RenderPath* editorRenderPath = nullptr;
	int IsThisEditor(lua_State* L)
	{
		bool ret = editorApplication != nullptr && editorRenderPath != nullptr;
		chif::lua::SSetBool(L, ret);
		return 1;
	}
	...
	void Initialize()
	{
		...
		char text[256] = {};
		snprintf(text, arraysize(text), "chif::lua Initialized [Lua %s.%s] (%d ms)", LUA_VERSION_MAJOR, LUA_VERSION_MINOR, (int)std::round(timer.elapsed()));
	...
	bool CompileFile(const char* filename, chif::vector<uint8_t>& dst)
	{
		chif::vector<uint8_t> filedata;
		if (chif::helper::FileRead(filename, filedata))
		{
	...
	{
		chif::vector<uint8_t>& dst = *(chif::vector<uint8_t>*)ud;
	...
	bool CompileText(const char* script, chif::vector<uint8_t>& dst)
	{
		if(luaL_loadstring(lua_internal().m_luaState, script) != LUA_OK)
		{
			PostErrorMsg();
	...
	void EnableEditorFunctionality(chif::Application* application, chif::RenderPath* renderpath)
	{
