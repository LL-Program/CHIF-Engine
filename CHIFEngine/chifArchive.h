...
namespace chif
{
	...
	private:
		...
		chif::vector<uint8_t> DATA; // data suitable for read/write operations
	...
	public:
		...
		void WriteData(chif::vector<uint8_t>& dest) const { dest.resize(pos); std::memcpy(dest.data(), data_ptr, pos); }
		...
		chif::graphics::Texture CreateThumbnailTexture() const;

		// Set a Texture as the thumbnail. This resets the archive, so you should usually do this before writing data:
		void SetThumbnailAndResetPos(const chif::graphics::Texture& texture);

		// Open just the tumbnail data from an archive, and return it as a Texture:
		static chif::graphics::Texture PeekThumbnail(const std::string& filename);
		...
		inline Archive& operator<<(const chif::Color& data)
		{
			_write(data.rgba);
			return *this;
		}
		...
		template<typename T>
		inline Archive& operator<<(const chif::vector<T>& data)
		{
		...
		inline Archive& operator<<(const chif::Archive& other)
		{
			// Here we will use the << operator so that non-specified types will have compile error!
		...
		inline Archive& operator>>(chif::Color& data)
		{
			_read(data.rgba);
			return *this;
		}
		...
		template<typename T>
		inline Archive& operator>>(chif::vector<T>& data)
		{
			// Here we will use the >> operator so that non-specified types will have compile error!
			size_t count;
			(*this) >> count;
