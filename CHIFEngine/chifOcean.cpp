...
namespace chif
{
	namespace ocean_internal
	{
		...
		void LoadShaders()
		{
			chif::renderer::LoadShader(ShaderStage::CS, updateSpectrumCS, "oceanSimulatorCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, updateDisplacementMapCS, "oceanUpdateDisplacementMapCS.cso");
			chif::renderer::LoadShader(ShaderStage::CS, updateGradientFoldingCS, "oceanUpdateGradientFoldingCS.cso");

			chif::renderer::LoadShader(ShaderStage::VS, oceanSurfVS, "oceanSurfaceVS.cso");

			chif::renderer::LoadShader(ShaderStage::PS, oceanSurfPS, "oceanSurfacePS.cso");
			chif::renderer::LoadShader(ShaderStage::PS, wireframePS, "oceanSurfaceSimplePS.cso");
...
#define GRAV_ACCEL	981.0f	// The acceleration of gravity, cm/s^2
	...
	void Ocean::Create(const OceanParameters& params)
	{
		...
		chif::vector<XMFLOAT2> h0_data(height_map_size);
		chif::vector<float> omega_data(height_map_size);
		...
		chif::vector<char> zero_data(3 * output_size * sizeof(float) * 2);
		...
		chif::vector<XMFLOAT4> displacementdata(tex_desc.width * tex_desc.height); // zero init the heightmap to be valid before first simulation
	...
	void Ocean::UpdateDisplacementMap(CommandList cmd) const
	{
		...
		// ------------------------------------ Perform FFT -------------------------------------------
		chif::fftgenerator::fft_512x512_c2c(buffer_Float_Dxyz, buffer_Float_Dxyz, buffer_Float2_Ht, cmd);
		...
		chif::renderer::GenerateMipChain(gradientMap, chif::renderer::MIPGENFILTER_LINEAR, cmd);

		device->EventEnd(cmd);
	}

	void Ocean::CopyDisplacementMapReadback(chif::graphics::CommandList cmd) const
	{
		GraphicsDevice* device = chif::graphics::GetDevice();
	...
	void Ocean::RenderForOcclusionTest(const CameraComponent& camera, CommandList cmd) const
	{
		...
		{
			chif::vector<uint16_t> index_data(index_count);
			size_t counter = 0;
	...
	void Ocean::Render(const CameraComponent& camera, CommandList cmd) const
	{
		...
		bool wire = chif::renderer::IsWireRender();
		...
		{
			chif::vector<uint32_t> index_data(index_count);
			size_t counter = 0;
	...
	void Ocean::Initialize()
	{
		...
		static chif::eventhandler::Handle handle = chif::eventhandler::Subscribe(chif::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); chif::fftgenerator::LoadShaders(); });

		LoadShaders();
		chif::fftgenerator::LoadShaders();

		chif::backlog::post("chif::Ocean Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
