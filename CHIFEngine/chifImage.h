...
namespace chif::image
{
	...
	struct Params
	{
		...
		chif::enums::BLENDMODE blendFlag = chif::enums::BLENDMODE_ALPHA;
		SAMPLEMODE sampleFlag = SAMPLEMODE_MIRROR;
		QUALITY quality = QUALITY_LINEAR;

		const chif::graphics::Texture* maskMap = nullptr;
		const chif::graphics::Texture* backgroundMap = nullptr;
		...
		constexpr void setMaskMap(const chif::graphics::Texture* tex) { maskMap = tex; }
		...
		//	If you don't set this per image, then chif::image::SetBackground() will be used instead
		constexpr void setBackgroundMap(const chif::graphics::Texture* tex) { backgroundMap = tex; }
		...
		//	the background tex should be bound with chif::image::SetBackground() beforehand
		...
		Params(
			const chif::primitive::Hitbox2D& hitbox, const XMFLOAT4& color = XMFLOAT4(1, 1, 1, 1)
		) :
	...
	void SetBackground(const chif::graphics::Texture& texture);

	// Draw the specified texture with the specified parameters
	void Draw(const chif::graphics::Texture* texture, const Params& params, chif::graphics::CommandList cmd);
