...
class FontManager(Singleton):
    ...
    def render_text(self,text, canvas_width, canvas_height):
        if RenderOption.RENDER_FONT and self.show:
            self.text_render_data.set_text(text, self.ascii, font_size=12, skip_check=True)
            self.core_manager.renderer.render_text(self.text_render_data, canvas_width/2, (canvas_height / 2) - self.text_render_data.font_size, canvas_width, canvas_height)
    def render_Transskript(self, text, canvas_width, canvas_height):
        if RenderOption.RENDER_FONT and self.show:
            self.text_render_data.set_text(text, self.ascii, font_size=20, skip_check=True)
            self.core_manager.renderer.render_text(self.text_render_data, canvas_width/2, (canvas_height / 2) - (self.text_render_data.font_size * 2), canvas_width, canvas_height)
